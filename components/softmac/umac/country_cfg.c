...
s32 get_5g_listidx(u8 ch)
{
    int idx;
    
    for(idx = 0; idx < (sizeof(channel_tbl)/sizeof(CHANNEL_INFO)); idx++)
    {
        if(ch == channel_tbl[idx].ch)
        {
            break;
        }
    }

    if(idx == (sizeof(channel_tbl)/sizeof(CHANNEL_INFO)))
...
s32 check_2Gchannel_availavble(u8 ch)
{
    ...
    if(gwifistatus.available_2Gchannel & (0x1 << (ch - 1)))
        ret = 1;

exit:
    return ret;
}
extern bool is_5g_band_support(void);
...
void set_5G_availch(u32 start, u32 end, u32 flag)
{
    ...
    while(1)
    {
        ...
        if(frequpper <= end)
        {
            ...
            gwifistatus.available_5Gchannel |= 0x1 << index;
            ch += 4;
        }
        else
        {
...
void set_2G_availch(u32 start, u32 end)
{
    ...
    while (1)
    {
        ...
        if(frequpper <= end)
        {
            gwifistatus.available_2Gchannel |= 0x1 << (ch - 1);
            ch++;
        }
        else
        {
...
int config_channel(COUNTRY_CODE index)
{
    ...
    gwifistatus.available_2Gchannel = 0;
    gwifistatus.available_5Gchannel = 0;
...
//    printf("2G:%x\n", gwifistatus.available_2Gchannel);
//    printf("5G:%x\n", gwifistatus.available_5Gchannel);
    return 0;
}
