...
u8 attach_sniffer_cb(SNIFFER_RECVINDEX recv_filter, void (*cb_fn)(packetinfo *), u16 stack_size)
{
    ...
    if (OS_TaskCreate(sniffer_task, "sniffer_task", stack_size, (void *)cb_fn, OS_TASK_HIGH_PRIO, &g_sniffer_task_handler) == 0) {
...
u8 deattach_sniffer_cb(void)
{
    ...
    OS_TaskCreate(stop_sniffer_task, "disable_sniffer", 256, NULL, OS_TASK_MIDDLE_PRIO, NULL);
