...
/* Command Line: */
static char sgCmdBuffer[CLI_BUFFER_SIZE+1] ATTRIBUTE_SECTION_CLI_BUF;
static char *sgArgV[CLI_ARG_SIZE];
static u32 sgArgC;
static u32 sgCurPos = 0;

#ifdef SUPPORT_CUSTOM_CMD   
extern const CLICmds gCustomCmdTable[];
#endif
...
ringbuf_st g_uart_rx_ringbuf ATTRIBUTE_SECTION_CLI_BUF = {
    0,
    0,
    {0},
};
...
static inline char __get_char (void)
{
    ...
    ret = drv_comport_read_fifo(rx_buf, 1, COMPORT_BLOCKING);
    return rx_buf[0];
...
{
    drv_comport_write_fifo((unsigned char*)&c,1,COMPORT_BLOCKING);
}
...
void Cli_Reset_Buf()
{
    memset( sgCmdBuffer, 0x0, strlen((char*)sgCmdBuffer));
    sgCurPos = 0;
}
...
void uart_rx_isr(void)
{
    ...
    //while(drv_uart_is_receiver_available()) {
        if(vidx_w != vidx_w_full) {
            ...
            //printf("Queue Full!\n");
            drv_comport_register_isr(COMPORT_RX_DATA_READY_IE, NULL);
            break;
        }
    }
...
void Cli_Init( void )
{
    ...
    drv_comport_register_isr(COMPORT_RX_DATA_READY_IE, uart_rx_isr);
}
...
void Cli_Start( void )
{
    const CLICmds *CmdPtr = NULL;
    u8 ch;
    char *pch;
	int32_t res = -1;
	static uint8_t akeeta_flag = 0;
#ifndef USE_SERIAL_DRV
    if ( !dbgcon_rx_ready() )
    {
        return;
