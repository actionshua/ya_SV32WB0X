...
void query_init(struct connstruct *cn)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] query_init() - cn = %p, cn->query = %s\n", cn, cn->query));

	cn->query_tbl = (query_var *)ax_malloc(sizeof(query_var)*NUM_QUERYVAR);
	if (cn->query_tbl == NULL )
	{
        HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] query_init() - query_tbl = NULL\n"));
		return ;
	}
	...
	while( var->name && (idx<NUM_QUERYVAR) )
	{
        ...
        HTTPD_DEBUGF((
            PRINT_HTTPD_TAG, "[HTTPD] query_init() - var->name = %s, var->value = %s, idx = %d\n",
...
char *query_getvar(struct connstruct *cn, char *name)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] query_getvar() - cn = %p, name = %s\n", cn, name));
	...
	while( var->name && (idx<NUM_QUERYVAR) ) {
        HTTPD_DEBUGF((
            PRINT_HTTPD_TAG, "[HTTPD] query_getvar() - var->name = %s, var->value = %s, idx = %d\n",
            var->name, var->value, idx));
...
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] WEB_write_blk() - soc_id = %d, buff_len = %d\n", cn->networkdesc, len));
