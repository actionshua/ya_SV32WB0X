...
CGI_FUNC cgi_filename_match(char *path_info)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] cgi_filename_match() - path_info = %s\n", path_info));
...
static int apConfig(struct cgi_para *para)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] apConfig() - sd = %d, cmd = %s, cmd_len = %d\n", para->sd, para->cmd, para->cmd_len));
...
static int test2(struct cgi_para *para)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] test2() - sd = %d, cmd = %s, cmd_len = %d\n", para->sd, para->cmd, para->cmd_len));
