...
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_start_handle()\n"));
...
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_stop_handle() - reason = %d\n", reason));
...
int httpd_fb_status(HTTPD_STATUS status, HTTPD_STOP_REASON reason)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_fb_status() - httpd_fb_mbox = %p\n", httpd_fb_mbox));
    ...
    if(httpd_fb_mbox)
    {
        ...
        {
            HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_fb_status() - Time out: can't add a job to queue\n"));
            return -1;
        }
    }
...
void httpd_proc_thread(void *arg)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_proc_thread()\n"));
    ...
    for (;;)
    {
        ...
        if (OS_BufQPop(httpd_proc_mbox, (void *)parameter, portMAX_DELAY) == OS_SUCCESS)
        {
            ...
            {
                HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_proc_thread() - receive the job, fb_queue = %p\n", parameter->fb_queue));
                httpd_fb_mbox = parameter->fb_queue;
                httpd_main(parameter);
            }
            else
            {
                HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_proc_thread() - receive the job, default value\n"));
                httpd_fb_mbox = NULL;
                httpd_main(NULL);
            }
        }
...
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_set_status() - status = %d\n", status));
    httpd_status = status;
}
...
HTTPD_RESULT httpd_init(void)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_init()\n"));
    ...
    if (httpd_proc_mbox == NULL)
    {
    	...
    	if(httpd_proc_mbox == NULL)
        {
            HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_init(), xQueueCreate fail\n"));
    ...
    if (httpd_proc_task_handle == NULL)
    {
        ...
        if(httpd_proc_task_handle == NULL)
        {
            HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_init(), OS_TaskCreate fail, return vaule = %ld\n", xReturn));
            return HTTPD_RESULT_UNINIT;
        }
...
HTTPD_RESULT httpd_stop(void)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_stop() - currently status = %d\n", status));
...
HTTPD_RESULT httpd_start(httpd_para *parameter)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_start() - currently status = %d\n", status));
    ...
    {
        HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] httpd_start() - Time out: can't add a job to queue\n"));
