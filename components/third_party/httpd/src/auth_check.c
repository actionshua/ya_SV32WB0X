...
void user_config(int lan_updated, int wan_updated)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] user_config() - lan_updated = %d, wan_updated = %d \n", lan_updated, wan_updated));
	...
	for (i=0; i < USER_NUM_MAX; i++)
	{
        ...
        HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] user_config() - NVRAM username= %s, passwd = %s\n", username, passwd));
...
int auth_check(struct connstruct *cn)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] auth_check() - cn = %p, cn->authorization = %s\n", cn, cn->authorization));
    ...
    *authpass++ = '\0';
	...
	{
        HTTPD_DEBUGF((
            PRINT_HTTPD_TAG, "[HTTPD] auth_check() - (authinfo = %s, authpass = %s), (username = %s, passwd = %s)\n",
	...
	if(i == USER_NUM_MAX)
	{
        HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] auth_check() - no match username and password\n"));
		goto send_auth;
	}
...
static void send_authenticate(struct connstruct *cn, char *realm)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] send_authenticate() - cn = %p, realm = %s\n", cn, realm));
