...
int CGI_do_cmd(struct connstruct *cn)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_do_cmd() - cn = %p\n", cn));
...
bad:
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_do_cmd() - cgi result=%d\n", rc));
    return rc;
}

int CGI_set_sta_info(struct connstruct *cn)
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "\n=============CGI_set_sta_info===============\n"));
    char *Ssid, *WpaPsk;
    char *key1 = "Ssid";
    char *key2 = "WpaPsk";
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_set_sta_info()\n"));
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_set_sta_info() - Ssid = %s, Psk = %s\n", Ssid,  WpaPsk));
    return 0;
}
...
int CGI_set_ap_info(struct connstruct *cn)
{
    ...
    char *key1 = "Ssid";
    char *key2 = "WpaPsk";
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_set_ap_info()\n"));
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_set_ap_info() - Ssid = %s, Psk = %s\n", Ssid,  WpaPsk));
    return 0;
}
...
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_get_ap_ssid() - cn = %p\n", cn));
    char ap_ssid[33] = {0};
...
void CGI_get_ap_passwd(struct connstruct *cn)
{
    ...
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_get_ap_passwd() - cn = %p\n", cn));
    char ap_pwd[65] = {0} ;
    uint8_t pwdlen = 0;
...
int CGI_sys_reboot(struct connstruct *cn)
{
    HTTPD_DEBUGF((PRINT_HTTPD_TAG, "[HTTPD] CGI_config_sys_reload() - cn = %p\n", cn));
