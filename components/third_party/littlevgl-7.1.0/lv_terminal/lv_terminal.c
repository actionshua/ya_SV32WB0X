 ...
 *      DEFINES
 *********************/
#define TERMINAL_ANIM_TIME   10 /*[ms]*/
...
lv_obj_t * terminal_create(void)
{
    ...
    lv_win_set_scrollbar_mode(win, LV_SCRLBAR_MODE_AUTO);
    ...
    clr_btn = lv_label_create(win, NULL);
    lv_label_set_text(clr_btn, "-----------");
...
void terminal_add_char(const char txt_in)
{
    if(win == NULL) return;                 /*Check if the window is exists*/
