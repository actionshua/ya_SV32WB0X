...
static void  joylink_smnt_finish(void)
{
	...
	if (pSmnt && (pSmnt->state== SMART_FINISH) ){
        ...
        log_i(PRINT_JOYLINK_TAG, "enc_len=%d, padding=%d\n", enc_len, padding);
		...
		if(joylink_smnt_gobal.get_result_callback == NULL){
			log_i(PRINT_JOYLINK_TAG, "ERROR:joylink_smnt_finish->get_result_callback NULL\n");
			goto RET;
		}
...
int joylink_smnt_cyclecall(void)
{
	...
	if(joylink_smnt_gobal.switch_channel_callback == NULL){
		log_i(PRINT_JOYLINK_TAG, "switch channel function NULL\n");
		return 50;
	}
	...
	log_i(PRINT_JOYLINK_TAG, "CH=%d, T=%d\n", pSmnt->chCurrentIndex, 50);
	return 50;
}
			...
			if (index == 0){
				*broadindex = 0;
				...
				if (rCrc == crc){
					...
					log_i(PRINT_JOYLINK_TAG, "Version RX:%x\n",pSmnt->broadcastVersion);
				}
			}
		}
	} 
...
void joylink_smnt_datahandler(PHEADER_802_11 pHeader, int length)
{
	...
	if(pSmnt->state == SMART_CH_LOCKING)
	{
				...
				if(isUplink == 1)
				{
					...
					if ( ( length == expectLength ) || isStep)
					{
						...
						//if (pSmnt->syncCount >= 4)	// Achive SYNC count!
						{
							...
							log_i(PRINT_JOYLINK_TAG, "SYNC:(%02X%02X%02X%02X%02X%02X-%02X%02X%02X%02X%02X%02X)------->:CH=%d, WD=%d\n",
							...
							log_i(PRINT_JOYLINK_TAG, "--->locked by uplink\n");
						}
						return;
					}
					if (pSmnt->syncCount)
					{
						...
						log_i(PRINT_JOYLINK_TAG, "SYNC LOST\n");
					}
				}
				else
				{
					...
					if ( ( length == expectLength ) || isStep)
					{
						...
						//if (pSmnt->syncCount_downlink>= 4)	// Achive SYNC count!
						{
							...
							if(pSmnt->chCurrentProbability < 20)
								pSmnt->chCurrentProbability = 20;
							log_i(PRINT_JOYLINK_TAG, "--->locked by downlink\n");
						}
						return;
					}
		...
		pSmnt->syncFirst = length;
		pSmnt->syncFirst_downlink = length;
		log_i(PRINT_JOYLINK_TAG, "Try to SYNC!\n");
		return;
	}
	...
	{
		log_i(PRINT_JOYLINK_TAG, "SMART_FINISH-1\n");
	}
	else
	{
		pSmnt->state = SMART_CH_LOCKING;
		...
		pSmnt->syncStepPoint = 0;
		pSmnt->syncCount = 0;
		log_i(PRINT_JOYLINK_TAG, "Reset All State\n");
	}
	return;
}
...
static void joylink_smnt_bufferprintf(uint8* p, int split, int len)
{
	...
	log_i(PRINT_JOYLINK_TAG, "Len=%d:%s\n",len, buf);
#endif
}
