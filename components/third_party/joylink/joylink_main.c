...
{
    log_i(PRINT_JOYLINK_TAG, "=========\n");
    log_i(PRINT_JOYLINK_TAG, "joylinkwificbfunc wifistatus = %d\n", msg->wifistatus);
    ...
    if(msg->wifistatus == 1)
    {
        ...
        log_i(PRINT_JOYLINK_TAG, "Wifi Connect\n");
        log_i(PRINT_JOYLINK_TAG, "STA%d:\n", msg->id);
        log_i(PRINT_JOYLINK_TAG, "mac addr        - %02x:%02x:%02x:%02x:%02x:%02x\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        log_i(PRINT_JOYLINK_TAG, "ip addr         - %d.%d.%d.%d\n", ipaddr.u8[0], ipaddr.u8[1], ipaddr.u8[2], ipaddr.u8[3]);
        log_i(PRINT_JOYLINK_TAG, "netmask         - %d.%d.%d.%d\n", submask.u8[0], submask.u8[1], submask.u8[2], submask.u8[3]);
        log_i(PRINT_JOYLINK_TAG, "default gateway - %d.%d.%d.%d\n", gateway.u8[0], gateway.u8[1], gateway.u8[2], gateway.u8[3]);
        log_i(PRINT_JOYLINK_TAG, "DNS server      - %d.%d.%d.%d\n", dnsserver.u8[0], dnsserver.u8[1], dnsserver.u8[2], dnsserver.u8[3]);

    }
    else
    {
        log_i(PRINT_JOYLINK_TAG, "Wifi Disconnect\n");
    }
}
...
{
    int ret = 0;
    log_i(PRINT_JOYLINK_TAG, "scan finish\n");
...
void joylink_get_result_cb(joylink_smnt_result_t result)
{
    int ret = 0;
    log_i(PRINT_JOYLINK_TAG, "==============================\n");
    log_i(PRINT_JOYLINK_TAG, "flag %d\n", result.smnt_result_status);
    log_i(PRINT_JOYLINK_TAG, "ssid %s, len %d\n", result.jd_ssid, result.jd_ssid_len);
    log_i(PRINT_JOYLINK_TAG, "pswd %s, len %d\n", result.jd_password, result.jd_password_len);
    log_i(PRINT_JOYLINK_TAG, "==============================\n");
    joylink_stop();
...
void sniffer_cb(packetinfo *packet)
{
    ...
    if(data == NULL && packet->len>=sizeof(struct ieee80211_hdr_3addr)){
		...
		((PHEADER_802_11)data)->FC.ToDs = 1;
    }

    joylink_smnt_datahandler((PHEADER_802_11)data, packet->len);
}
