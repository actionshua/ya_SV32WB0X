...
int FreeRTOS_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
    ...
    if (ret < 0) {
        log_e(PRINT_MQTT_TAG, "mqtt write fail");
        return -1;
    } else if (ret == 0) {
        log_e(PRINT_MQTT_TAG, "mqtt write timeout");
        return -2;
    } else if (ret == 1) {
...
int NetworkConnect(Network *n, char *addr,  char *port)
{
    ...
    if (rc == 0) {
        ...
        if (n->my_socket < 0) {
            log_e(PRINT_MQTT_TAG, "mqtt socket create fail");
            return -1;
        }
        ...
        if (rc < 0) {
            close(n->my_socket);
            log_e(PRINT_MQTT_TAG, "mqtt socket connect fail:rc=%d,socket = %d", rc, n->my_socket);
            return -2;
        }
