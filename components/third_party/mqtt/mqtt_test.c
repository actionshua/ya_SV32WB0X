...
static void messageArrived(MessageData *md)
{
    ...
    //log_i(PRINT_MQTT_TAG, "Message arrived: qos %d, retained %d, dup %d, packetid %d\n", 
        //message->qos, message->retained, message->dup, message->id);
    *((char *)(message->payload +(size_t)message->payloadlen)) = '\0';
    log_i(PRINT_MQTT_TAG, "\n--Payload:%s--\n", (char *)(message->payload));
...
void mqtt_client_test(void)
{
    ...
    MQTTMessage message;
    log_i(PRINT_MQTT_TAG, "mqtt_client_test begin\n");
    //Initialize MQTT network structure
    NetworkInit(&n);
    
    //Connect to remote server
    //log_i(PRINT_MQTT_TAG, "Connect to %s:%s\n", MQTT_SERVER, MQTT_PORT);
    for(cnt = 0; cnt < 10; cnt++) {
        rc = NetworkConnect(&n, MQTT_SERVER, MQTT_PORT);
        if (rc != 0) {
            log_e(PRINT_MQTT_TAG, "TCP connect failed,status -%4X\n", -rc);
        } else {
            break;
        }
    }
    ...
    //log_i(PRINT_MQTT_TAG, "MQTTClientInit\n");
    ...
    data.cleansession = 1;
    //log_i(PRINT_MQTT_TAG, "MQTTConnect\n");
    //Send MQTT connection request to the remote MQTT server
    rc = MQTTConnect(&c, &data);

    if (rc != 0) {
        log_e(PRINT_MQTT_TAG, "MQTT connect failed,status -%4d\n", -rc);
        goto exit;
    }
    //log_i(PRINT_MQTT_TAG, "MQTTSubscribe\n");
    ...
    //log_i(PRINT_MQTT_TAG, "MQTTPublish\n");
    rc = MQTTPublish(&c, topic, &message);
    
    while (arrivedcount < 1) {
        //log_i(PRINT_MQTT_TAG, "MQTTYield\n");
        MQTTYield(&c, 1000);
    }
    ...
    if ((rc = MQTTUnsubscribe(&c, topic)) != 0) {
        log_e(PRINT_MQTT_TAG, "The return from unsubscribe was %d\n", rc);
    }
    //log_i(PRINT_MQTT_TAG, "MQTT unsubscribe done\n");

    if ((rc = MQTTDisconnect(&c)) != 0) {
        log_e(PRINT_MQTT_TAG, "The return from disconnect was %d\n", rc);
    }
    //log_i(PRINT_MQTT_TAG, "MQTT disconnect done\n");

    n.disconnect(&n);
    //log_i(PRINT_MQTT_TAG, "Network disconnect done\n");


exit:
    log_i(PRINT_MQTT_TAG, "mqtt_client_test end\n");
    OS_TaskDelete(NULL);
}
