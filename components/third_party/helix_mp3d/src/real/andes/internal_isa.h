...
ISA_FUNC_PRFX uint32_t NDS_ISA_ABS(q31_t data)
{
    ...
    //need consider:
    //V2 and bsp320 V3 has no __nds32__abs() delclaration but bsp400 has (4.4.4 vs. 4.8.2)
...
#define NDS_ISA_SATS_168(X, Y)  __nds32__clips((X), (Y - 1))      //only for Y=16 or Y=8
...
#define NDS_ISA_SATS_168(X, Y)      sat_q31_to_any(X, Y)
...
#define NDS_ISA_CLOV        __nds32__clov
