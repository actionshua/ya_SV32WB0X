...
int nds_dsp_ov_flag = 0;

DSP_FUNC_PRFX q31_t clip_q63_to_q31(q63_t x)
{
...
//DSP_FUNC_PRFX uint32_t NDS_DSP_SAT16(uint32_t x, IMM uint8_t num_bit)
//{
//    union32u_t ret;
//
//    ret.w0 = x;
...
////signed clipping for two 16 at bit bit_idx separately.
////num_bit: from 1 to 16 bits for half-word.
//DSP_FUNC_PRFX q31_t NDS_DSP_SATS16(q31_t x, IMM uint8_t num_bit)
//{
//    union32_t ret;
//
//    ret.w0 = x;
...
//Signed CoMPare EQual to the corresponding four 8-bit between two operands.
...
//Signed CoMPare EQual to the corresponding two 16-bit between two operands.
...
//Unsigned CoMPare EQual to the corresponding four 8-bit between two operands.
//return: four 8-bit 0xFF/0x00 results. if eq then 0xFF else 0x00.
...
//Unsigned CoMPare EQual to the corresponding two 16-bit between two operands.
//return: two 16-bit 0xFFFF/0x0000 results. if eq then 0xFFFF else 0x0000.
...
//Signed CoMPare Less Than the corresponding four 8-bit between two operands.
...
//Signed CoMPare Less Than the corresponding two 16-bit between two operands.
...
//Unsigned CoMPare Less Than the corresponding four 8-bit between two operands.
//return: four 8-bit 0xFF/0x00 results. if LT then 0xFF else 0x00.
...
//Unsigned CoMPare Less Than the corresponding two 16-bit between two operands.
//return: two 16-bit 0xFFFF/0x0000 results. if LT then 0xFFFF else 0x0000.
...
//Signed CoMPare Less than or Equal to the corresponding four 8-bit between two operands.
...
//Signed CoMPare Less than or Equal to the corresponding two 16-bit between two operands.
...
//Unsigned CoMPare Less than or Equal to the corresponding four 8-bit between two operands.
//return: four 8-bit 0xFF/0x00 results. if LE then 0xFF else 0x00.
...
//Unsigned CoMPare Less than or Equal to the corresponding two 16-bit between two operands.
//return: two 16-bit 0xFFFF/0x0000 results. if LE then 0xFFFF else 0x0000.
...
DSP_FUNC_PRFX q31_t NDS_DSP_KSLLI16(q31_t x, IMM uint8_t sh)
{
    ...
    temp.w0 = x;
    ret.b16.h1 = (q15_t) NDS_DSP_SAT((q31_t)temp.b16.h1 << sh, 16);
    ret.b16.h0 = (q15_t) NDS_DSP_SAT((q31_t)temp.b16.h0 << sh, 16);
