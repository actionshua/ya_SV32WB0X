...
//COS_RES_N controls the resolution of cosine tables in nds32_cos_xx and nds32_sin_xx functions.
//The value of COS_RES_N is identical with RES_N in default configuration.
//
//There is an exception case when user needs to calculate nds32_cos_xx and
...
/**
 * nds32_truncate_(source type)_to_(destination type);
...
static inline q31_t nds32_truncate_q63_to_q31(q63_t src)
{
...
#if 0
static inline q15_t nds32_truncate_q63_to_q15(q63_t src)
{
    q31_t a = (src >> 32);
    q31_t b = (src >> 15);
...
static inline q7_t nds32_truncate_q63_to_q7(q63_t src)
{
    q31_t a = (src >> 32);
    q31_t b = (src >> 7);
...
static inline q15_t nds32_truncate_q31_to_q15(q31_t src)
{
    q15_t a = (src >> 16);
    q15_t b = (src >> 15);
...
static inline q7_t nds32_truncate_q31_to_q7(q31_t src)
{
    q15_t a = (src >> 16);
    q15_t b = (src >> 7);
...
static inline q7_t nds32_truncate_q15_to_q7(q15_t src)
{
    q15_t a = (src >> 8);
    q15_t b = (src >> 7);
...
/**
 * nds32_saturate(q31_t src, uin32_t (destination_type))
...
#define nds32_saturate(X, Y) (q31_t)__nds32__clips(X, Y - 1)

#else

static inline q31_t nds32_saturate(q31_t src, const uint32_t type)
{
    q31_t max, min;
...
//Define 64-bit over 32-bit division
#if !(( __GNUC__ == 4) && (__GNUC_MINOR__ == 4))
//BSP400 and after

// ?div_qrnnd is better than __divdi3 both in cycles and code size.
// Result 64 bits:
//      High word is quotient
//      Low word is remainder
extern int64_t sdiv_qrnnd(int64_t, int32_t);
extern uint64_t udiv_qrnnd(uint64_t, uint32_t);

#define nds32_sdivq_64_32(X, Y)       ((q31_t)(sdiv_qrnnd((X), (Y)) >> 32))
#define nds32_udivq_64_32(X, Y)       ((uint32_t)(udiv_qrnnd((X), (Y)) >> 32))

#else   //!(( __GNUC__ == 4) && (__GNUC_MINOR__ == 4))
//Before BSP400

//This will use __divdi3
#define nds32_sdivq_64_32(X, Y)       ((q31_t)((X) / (Y)))
#define nds32_udivq_64_32(X, Y)       ((uint32_t)((X) / (Y)))
