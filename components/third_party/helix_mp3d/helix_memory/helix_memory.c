...
int mp3data_total_size = sizeof(_home_minghung_test_mp3_mp3);

// 10K mp3 buffer
//char g_mh_buffer[1024*10];
...
int helix_memory_tester()
{
...
#ifdef READ_MP3DATA_FROM_ARRAY
    mp3data_read_ptr = (char *)_home_minghung_test_mp3_mp3;
#endif
    ...
    nFrames = 0;
    int mh_start;
    int mh_total_time_start = OS_GetSysTick();
    int mh_fs_time = 0;
    do
    {
        /* somewhat arbitrary trigger to refill buffer - should always be enough for a full frame */
        mh_start = OS_GetSysTick();
        ...
        mh_fs_time += OS_GetSysTick() - mh_start;
...
//        totalDecTime += diffTime;
        ...
        else
        {
            ...
            //drv_i2s_tx_write_data(mp3FrameInfo.bitsPerSample / 8 * mp3FrameInfo.outputSamps, (uint8_t *)outBuf, 100);
#ifdef WRITE_OUT_PCM
            if (outfile)
...
//#endif
    ...
    int mh_finish = OS_GetSysTick();
    printf("total ticks %d\n", mh_finish - mh_total_time_start);
    printf("total fs ticks %d\n", mh_fs_time);
    printf("\nDecoded %d frames\n", nFrames);
