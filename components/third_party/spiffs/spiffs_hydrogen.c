...
s32_t prvSPIFFS_format(spiffs *fs) {
  ...
  while (bix < fs->block_count) {
    ...
    {
    	log_i(LOG_TAG_FS, "erase block %d done\n",bix);
    }
...
void spiff_erase_flash_wtih_fs(unsigned int erase_addr, unsigned int erase_size)
{
...
s32_t prvSPIFFS_format_fast(spiffs *fs, unsigned int addr, unsigned int size) {
#if SPIFFS_READ_ONLY
  (void)fs;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
  ...
  spiff_erase_flash_wtih_fs(addr, addr+size);
  while (bix < fs->block_count) {
    fs->max_erase_count = 0;
    res = spiffs_erase_block_fast(fs, bix);
    if (res != SPIFFS_OK) {
      res = SPIFFS_ERR_ERASE_FAIL;
    }
    else
    {
    	printf("write block info %d done\n",bix);
    }
    SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
    bix++;
  }
...
s32_t prvSPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
  ...
  log_d(LOG_TAG_FS, "page index byte len:         %"PRIu32"\n", SPIFFS_CFG_LOG_PAGE_SZ(fs));
  log_d(LOG_TAG_FS, "object lookup pages:         %lu\n", SPIFFS_OBJ_LOOKUP_PAGES(fs));
  log_d(LOG_TAG_FS, "page pages per block:        %"PRIu32"\n", SPIFFS_PAGES_PER_BLOCK(fs));
  log_d(LOG_TAG_FS, "page header length:          %lu\n", sizeof(spiffs_page_header));
  log_d(LOG_TAG_FS, "object header index entries: %lu\n", SPIFFS_OBJ_HDR_IX_LEN(fs));
  log_d(LOG_TAG_FS, "object index entries:        %lu\n", SPIFFS_OBJ_IX_LEN(fs));
  log_d(LOG_TAG_FS, "available file descriptors:  %"PRIu32"\n", fs->fd_count);
  log_d(LOG_TAG_FS, "free blocks:                 %"PRIu32"\n", fs->free_blocks);
