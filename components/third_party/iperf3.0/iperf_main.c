...
void iperf_peer_update(u8* mac)
{
    ...
    //log_i(PRINT_IPERF3_TAG, "%s ip=%s,val=0x%x\r\n",__func__,netstack_inet_ntoa(ipaddr),netstack_ipaddr_addr(netstack_inet_ntoa(ipaddr)));
    if(g_iperf_test1)
    {
        log_i(PRINT_IPERF3_TAG, "g_iperf_test1 hostname=%s, Server:%s\r\n",
        ...
        {
            log_i(PRINT_IPERF3_TAG, "g_iperf_test2 hostname=%s, Server:%s\r\n",
            ...
            {
                log_i(PRINT_IPERF3_TAG, "g_iperf_test3 hostname=%s, Server:%s\r\n",
                ...
                {
                    log_i(PRINT_IPERF3_TAG, "g_iperf_test4 hostname=%s, Server:%s\r\n",
...
void iperf_list()
{
    ...
    if (g_iperf_test1){
        log_i(PRINT_IPERF3_TAG, "iperf task%d: %s, server port: %d \r\n", ++i,
    ...
    if (!g_iperf_test1 && !g_iperf_test2 && !g_iperf_test3 && !g_iperf_test4){
        log_i(PRINT_IPERF3_TAG, "No iperf thread running. \r\n");
    }
    iperf_api_mutex_lock(0);
}
...
void iperf_query()
{
    ...
    #if 0
    log_i(PRINT_IPERF3_TAG, "\r\nMEMP_TCP_PCB\r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "avail: %"U32_F"\r\n\t", (u32_t)tcp_pcb_mem->avail); 
    log_i(PRINT_IPERF3_TAG, "used: %"U32_F"\r\n\t", (u32_t)tcp_pcb_mem->used); 
    log_i(PRINT_IPERF3_TAG, "max: %"U32_F"\r\n\t", (u32_t)tcp_pcb_mem->max); 
    log_i(PRINT_IPERF3_TAG, "err: %"U32_F"\r\n", (u32_t)tcp_pcb_mem->err);
    ...
    #if 0
    log_i(PRINT_IPERF3_TAG, "STA mode \r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "Client mode :\r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "TCP TX : %d\r\n\t", netconn_mem->avail - 1); 
    log_i(PRINT_IPERF3_TAG, "UDP TX : %d\r\n\t", netconn_mem->avail - 2); 

    log_i(PRINT_IPERF3_TAG, "Server mode :\r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "TCP RX : %d\r\n\t", netconn_mem->avail - 2); 
    log_i(PRINT_IPERF3_TAG, "UDP RX : %d\r\n\t", netconn_mem->avail - 3); 

    log_i(PRINT_IPERF3_TAG, "AP mode \r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "Client mode :\r\n\t"); 
    log_i(PRINT_IPERF3_TAG, "TCP TX : %d\r\n\t", netconn_mem->avail - 2); 
    log_i(PRINT_IPERF3_TAG, "UDP TX : %d\r\n\t", netconn_mem->avail - 2); 
...
#if 1
    if (g_iperf_test1 && g_iperf_test2 && g_iperf_test3 && g_iperf_test4){
        log_i(PRINT_IPERF3_TAG, "ERROR -- there have already been four iperf threads running! \r\n");
        result = false;
    }
    else{    
        ...
        if (result){
            ...
            log_i(PRINT_IPERF3_TAG, "ERROR -- there has already been an iperf thread with server port %d! \r\n", test->server_port);
            log_i(PRINT_IPERF3_TAG, "Please use option '-p <port>' to designate another server port.\r\n");
        }
    }
#else
    if (g_iperf_test1 && g_iperf_test2){
        log_i(PRINT_IPERF3_TAG, "ERROR -- there have already been two iperf threads running! \r\n");
        result = false;
    }
    else{    
        if (g_iperf_test1 && g_iperf_test1->server_port == test->server_port)
...
net_app_iperf3(int argc, char **argv)
{
    ...
    if (argc == 3 && STRCMP(argv[1], "arp") == 0)
    {
        ...
        if (retval != 6){
            log_i(PRINT_IPERF3_TAG, "sscanf get MAC address: %02x:%02x:%02x:%02x:%02x:%02x \r\n",
                dst_mac[0],dst_mac[1],dst_mac[2],
                dst_mac[3],dst_mac[4],dst_mac[5]);
            log_i(PRINT_IPERF3_TAG, "Invalid MAC address [%s], retval:%d \r\n", argv[2], retval);
            return;
        }
        for(i=0;i<6;i++)
            mac[i]=(u8)dst_mac[i];
        retval = netmgr_send_arp_unicast(mac);
        log_i(PRINT_IPERF3_TAG, "arp unicast request return %s \r\n", retval ? "FAILURE" : "SUCCESS!");
        return ;
    }
#endif
    ...
    if (argc == 5 && STRCMP(argv[1], "udp") == 0)
    {
        ...
        {
            log_e(PRINT_IPERF3_TAG, "udp speed pkg size > 60k \r\n");
            return;
        }
        else
        {
        ...
        if (!sendBuf)
        {
            log_e(PRINT_IPERF3_TAG, "malloc %d failed\r\n", udp_speed_pkg_size);
            return;
        }
        ...
        for(;((tick2-tick1)*OS_TICK_RATE_MS) < (u32)(udp_speed_time*1000);)
        {
            ...
            if (len <=0)
            {
                log_i(PRINT_IPERF3_TAG, "sendto err\r\n ");
                break;
            }
            ...
            {
                tick3 = tick2;
                log_i(PRINT_IPERF3_TAG, "send speed=%dk \r\n ", (int)((send_count * len)>>10));
        ...
        {
            log_i(PRINT_IPERF3_TAG, "send end,tol time = %d(ms),tol pkg =%d, tol data=%d(k),speed=%d(k/s)\r\n"
        ...
        {
            log_i(PRINT_IPERF3_TAG, "send end,time is < 1s\r\n");
        }

        shutdown(fd,SHUT_RDWR);
        closesocket(fd);
        OS_MemFree(sendBuf);
        log_i(PRINT_IPERF3_TAG, "iperf udp test end\r\n");
        return;
    }
#endif

    if (!iperf_api_mutex && iperf_init()){
        log_i(PRINT_IPERF3_TAG, "iperf init failed! \r\n");
        return;
    }
    ...
    if (argc == 2 && STRCMP(argv[1], "stop") == 0)
    {
        if (iperf_get_runflag()){
            iperf_clear_runflag();
            log_i(PRINT_IPERF3_TAG, "The iperf threads stopped! \r\n");
            sys_msleep(2000);

            #ifdef IPERF_DEBUG_MEM
            log_i(PRINT_IPERF3_TAG, "iperf mallocs:%d, frees:%d, delta:%d, delta size:%d\r\n", iperf_mallocs, iperf_frees, iperf_mallocs - iperf_frees, iperf_msize);
            log_i(PRINT_IPERF3_TAG, "iperf max mallocs:%d, max size:%d \r\n", iperf_max_mallocs, iperf_max_msize);
        ...
        log_i(PRINT_IPERF3_TAG, "float_var = %f \r\n \r\n", float_var);
        log_i(PRINT_IPERF3_TAG, "double_var = %g \r\n \r\n", double_var);
        log_i(PRINT_IPERF3_TAG, "u64_var = %llu \r\n", u64_var);

        sprintf( str, "%lld", u64_var);
        log_i(PRINT_IPERF3_TAG, "u64_var=%s after sprintf \r\n", str);
        return;
    }
    test = iperf_new_test();
    if (!test)
    {
        log_e(PRINT_IPERF3_TAG, "create new test error - %s \r\n", iperf_strerror(IENEWTEST));
        return;
    }
    ...
    if (retval < 0) {
        log_e(PRINT_IPERF3_TAG, "parameter error - %s \r\n", iperf_strerror(test->i_errno));
        log_i(PRINT_IPERF3_TAG, "\r\n");
    ...
    {
        log_i(PRINT_IPERF3_TAG, "Notes: iperf3 win32 version not support udp rx more than one stream\r\n");	
    }
    
    if (iperf_add_test(test)){
        if (run(test) < 0)
            log_e(PRINT_IPERF3_TAG, "error - %s \r\n", iperf_strerror(test->i_errno));
    }else{
        log_i(PRINT_IPERF3_TAG, "Please run command \"iperf3 list\" to show the running iperf threads. \r\n");	
        log_i(PRINT_IPERF3_TAG, "Please run command \"iperf3 stop\" to stop the running threads. \r\n");	
    }
            ...
            for (; iperf_get_runflag(); ) {
    		    if (iperf_run_server(test) < 0) {
    		        ...
    		        if (consecutive_errors >= 5) {
    		            log_e(PRINT_IPERF3_TAG, "too many errors, exiting\r\n");
    			        break;
    		        }
                } else
    		        consecutive_errors = 0;
