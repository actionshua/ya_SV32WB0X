...
iperf_server_listen(struct iperf_test *test)
{
    ...
    if((test->listener = netannounce(test->settings->domain, Ptcp, test->bind_address, test->server_port)) < 0) {
    	if (test->t_errno == EAFNOSUPPORT && test->settings->domain != AF_INET) {
    	    ...
    	    log_i(PRINT_IPERF3_TAG, "listener = netannounce return %d \r\n", test->listener);
...
iperf_accept(struct iperf_test *test)
{
    ...
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf_accept new sock:%d, test->ctrl_sck:%d \r\n", s, test->ctrl_sck);
...
iperf_handle_message_server(struct iperf_test *test)
{
    ...
    if ((rval = Nread(test->ctrl_sck, (char*) &test->state, sizeof(signed char), Ptcp)) <= 0) {
        if (rval == 0) {
    	    log_e(PRINT_IPERF3_TAG, "the client has unexpectedly closed the connection \r\n");
    ...
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf_handle_message_server read state: %d \r\n", test->state);
...
//                sp->socket = -1;
        ...
        case CLIENT_TERMINATE:
    	    ...
    	    log_i(PRINT_IPERF3_TAG, "the client has terminated \r\n");
            SLIST_FOREACH(sp, &test->streams, streams) {
                FD_CLR(sp->socket, &test->read_set);
                FD_CLR(sp->socket, &test->write_set);
                close(sp->socket);
...
iperf_run_server(struct iperf_test *test)
{
    ...
    while (test->state != IPERF_DONE && iperf_get_runflag()) {
        ...
        //log_i(PRINT_IPERF3_TAG, "iperf_run_server: state:%d \r\n", (int)test->state);
        //if (timeout != NULL)
        //    log_i(PRINT_IPERF3_TAG, "iperf_run_server: select timeout: %ld,%ld\r\n", timeout->tv_sec, timeout->tv_usec);
        ...
        //log_i(PRINT_IPERF3_TAG, "select test->max_fd:%d, result: %d \r\n", test->max_fd, result);
    	...
    	if (result == 0 || (timeout != NULL && timeout->tv_sec == 0 && timeout->tv_usec == 0)) {
            ...
            if (test->state == CREATE_STREAMS)
            {
                ...
                strm_timeout_cnt++;
                if(test->debug)
                    log_i(PRINT_IPERF3_TAG, "iperf_stream create timeout counter ticks = %d \r\n", strm_timeout_cnt);
                if(strm_timeout_cnt > create_strm_timeo_cnt) //timeout of select - 50ms, counter 20 times for 1 seconds
                {
                    if (iperf_set_send_state(test, SERVER_TERMINATE) != 0) 
                    {
                        ...
                        log_i(PRINT_IPERF3_TAG, "the server has terminated \r\n");
                        SLIST_FOREACH(sp, &test->streams, streams) 
                        {
                            FD_CLR(sp->socket, &test->read_set);
                            FD_CLR(sp->socket, &test->write_set);
