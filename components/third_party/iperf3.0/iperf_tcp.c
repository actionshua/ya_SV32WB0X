    ...
    if (test->no_delay || test->settings->mss || test->settings->socket_bufsize) {
        ...
        if (test->no_delay) {
            ...
            if( getsockopt(s, IPPROTO_TCP, TCP_NODELAY, &opt, &optlen) >= 0)
            {
                if (test->debug){
                    if( opt )
                        log_e(PRINT_IPERF3_TAG,  " sock(%d) TCP_NODELAY : ENABLED \r\n", s);
                    else
                        log_e(PRINT_IPERF3_TAG, " sock(%d) TCP_NODELAY : DISABLED \r\n", s);
                }
            }
            else if (test->debug)
                    log_e(PRINT_IPERF3_TAG,  " sock(%d) getsockopt failed \r\n", s);
        }
    	...
    	if (test->debug) {
    	    ...
    	    log_i(PRINT_IPERF3_TAG, "SO_SNDBUF is %u\r\n", opt);
    	    */
    	}
