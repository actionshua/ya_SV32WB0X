...
iperf_handle_message_client(struct iperf_test *test)
{
    ...
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf_handle_message_client read state: %d\r\n", test->state);
    switch (test->state) {
        case PARAM_EXCHANGE:
    ...
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf_handle_message_client done \r\n");
...
iperf_connect(struct iperf_test *test)
{
    ...
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf connect to server:%s, port:%u success \r\n", test->server_hostname, test->server_port);
...
iperf_run_client(struct iperf_test * test)
{
    ...
    while (test->state != IPERF_DONE && iperf_get_runflag()) {
        if (retry>= 10) { // over 10s, if not get report with remote, exit iperf
            printf("test end, but state can not change to next, exit iperf\n");
            iperf_client_end(test);
            return -1;
        }
        //log_i(PRINT_IPERF3_TAG, "test->state: %d \r\n", test->state);
        ...
        /* fixed bug, if layer2 disconnect, after timeout, exit iperf*/
        if ((test->state == TEST_END || test->state == EXCHANGE_RESULTS)&&(timeout->tv_sec == 0&&timeout->tv_usec==0)) {
            timeout->tv_sec = 1;
            retry++;
        }
