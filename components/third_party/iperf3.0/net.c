...
Nread(int fd, char *buf, size_t count, int prot)
{
    ...
    while (nleft > 0) {
        ...
        if (r < 0) {
            ...
            {
                log_e(PRINT_IPERF3_TAG, "Nread err = %d r = %d\r\n", sock_err, r);
    ...
    *read_len = 0;
    while (nleft > 0) {
        ...
        if (r < 0) {
            ...
            {
                log_e(PRINT_IPERF3_TAG, "Nread_partly err = %d r = %d\r\n", sock_err, r);
                return NET_HARDERROR;
            }
        } else if (r == 0)
            break;
    ...
    while (nleft > 0) {
    	...
    	if (r < 0) {
    	    ...
    	    switch (sock_err) {
        		...
        		default:
                {
                    log_e(PRINT_IPERF3_TAG, "Nwrite err = %d r = %d\r\n", sock_err, r);
            		return NET_HARDERROR;
        		}
    	    }
    	} else if (r == 0)
    ...
    *written = 0;
    while (nleft > 0) {
    	...
    	if (r < 0) {
    		...
    		default:
                {
                    log_e(PRINT_IPERF3_TAG, "Nwrite_partly err = %d r = %d\r\n", sock_err, r);
                    return NET_HARDERROR;
                }
    	    }
    	} else if (r == 0)
...
setnonblocking(int fd, int nonblocking)
{
    ...
    //log_i(PRINT_IPERF3_TAG, "setnonblocking: fd:%d, flag:%d \n", fd, nonblocking);
    flags = lwip_fcntl(fd, F_GETFL, 0);
    if (flags < 0) {
        log_e(PRINT_IPERF3_TAG, "fcntl(F_GETFL)");
        return -1;
    }
    if (nonblocking)
    ...
    if (newflags != flags)
    	if (lwip_fcntl(fd, F_SETFL, newflags) < 0) {
    	    log_e(PRINT_IPERF3_TAG, "lwip_fcntl(F_SETFL)");
    	    return -1;
    	}
    return 0;
}
