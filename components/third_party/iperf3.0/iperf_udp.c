...
iperf_udp_recv(struct iperf_stream *sp)
{
     ...
     * processing.
     */
    //log_i(PRINT_IPERF3_TAG, "iperf_udp_recv r:%d\r\n", r);
    if (r <= 0)
    {
        return r;
    }
    ...
    if (pcount >= sp->packet_count + 1) {
        ...
        if (sp->test->debug)
        {
            if (sp->packet_count == pcount)
                log_i(PRINT_IPERF3_TAG, "DUPLICATES - received packet = %d \r\n", (int)sp->packet_count);
            else
        	    log_i(PRINT_IPERF3_TAG, "OUT OF ORDER - received packet = %d AND SP = %d, incoming packet = %u \r\n", (int)sp->packet_count, sp->socket, (int)pcount);
    	}
    }
    ...
    if (sp->prev_transit.tv_sec != 0 || sp->prev_transit.tv_usec != 0)
    {
        ...
        if (sp->test->debug)
        {
            //log_i(PRINT_IPERF3_TAG, "arr_sec:%ld, arr_usec:%ld, pre_sec:%ld, pre_usec:%ld \r\n", arrival_time.tv_sec, arrival_time.tv_usec, sp->prev_receive.tv_sec, sp->prev_receive.tv_usec);
            //log_i(PRINT_IPERF3_TAG, "snt_sec:%ld, snt_usec:%ld, pre_sec:%ld, pre_usec:%ld \r\n", sent_time.tv_sec, sent_time.tv_usec, sp->prev_transit.tv_sec, sp->prev_transit.tv_usec);
            //log_i(PRINT_IPERF3_TAG, "J=%ld, jitter:%ld \r\n", J, sp->jitter);
        }
    }
    ...
    if (sp->test->debug) {
    	//log_i(PRINT_IPERF3_TAG, "packet_count %d\r\n", sp->packet_count);
    }
