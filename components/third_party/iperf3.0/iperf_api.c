...
Iperf_cJSON *Iperf_cJSON_Parse( const char *value )
{
	...
	//log_i(PRINT_IPERF3_TAG, "cJSON_Parse :%s  \r\n", value);
...
static char *Iperf_print_value( Iperf_cJSON *item, int depth, int fmt )
{
    ...
    //log_i(PRINT_IPERF3_TAG, "print_value:: type:%d \r\n", item->type&0xff);
    //if (item->string)
    //    log_i(PRINT_IPERF3_TAG, "print_value:: type:%s \r\n", item->string);
...
{    
    //log_i(PRINT_IPERF3_TAG, "cJSON_AddItemToObject ...%s \r\n", string);
	if ( ! item )
		return;
	if ( item->string )
    ...
    //log_i(PRINT_IPERF3_TAG, "cJSON_AddItemToObject ...end\r\n");
}
...
void *iperf_malloc(size_t size)
{
    ...
    if (ptr) {
        ...
        *(int*)ptr = size;
        ...
        //log_i(PRINT_IPERF3_TAG, "iperf_malloc size: %u\r\n", size);
        ptr += 4;
    }    
    return (void*)ptr;
#else
...
void iperf_free(void *ptr)
{
    ...
    if (ptr){
        ...
        iperf_frees++;
        iperf_msize -= *(int*)ptr;
        //log_i(PRINT_IPERF3_TAG, "iperf_free size: %u\r\n", *(int*)ptr);
        FREE(ptr);
    }
#else
    if (ptr)
...
iperf_usage()
{
    log_i(PRINT_IPERF3_TAG, "Usage: iperf [-s|-c host] [options]\r\n");
    log_i(PRINT_IPERF3_TAG, "Try `iperf --help' for more information.\r\n");
}

void
usage_long()
{
    log_i(PRINT_IPERF3_TAG, "Usage: iperf3 [-s|-c host] [options]\r\n");
    log_i(PRINT_IPERF3_TAG, "       iperf3 [-h|--help] [-v|--version]\r\n\r\n");
    log_i(PRINT_IPERF3_TAG, "Server or Client:\r\n");
    log_i(PRINT_IPERF3_TAG, "  -p, --port      #         server port to listen on/connect to\r\n");
    log_i(PRINT_IPERF3_TAG, "  -i, --interval  #         seconds between periodic bandwidth reports\r\n");
    log_i(PRINT_IPERF3_TAG, "  -V, --verbose             more detailed output\r\n");
    log_i(PRINT_IPERF3_TAG, "  -d, --debug               emit debugging output\r\n");
    log_i(PRINT_IPERF3_TAG, "  -v, --version             show version information and quit\r\n");
    log_i(PRINT_IPERF3_TAG, "  -h, --help                show this message and quit\r\n");
    log_i(PRINT_IPERF3_TAG, "  stop                      kill the iperf threads \r\n");
    log_i(PRINT_IPERF3_TAG, "  list                      show the running iperf threads \r\n");
    log_i(PRINT_IPERF3_TAG, "Server specific:\r\n");
    log_i(PRINT_IPERF3_TAG, "  -s, --server              run in server mode\r\n");
    log_i(PRINT_IPERF3_TAG, "Client specific:\r\n");
    log_i(PRINT_IPERF3_TAG, "  -c, --client    <host>    run in client mode, connecting to <host>\r\n");
    log_i(PRINT_IPERF3_TAG, "  -u, --udp                 use UDP rather than TCP\r\n");
    log_i(PRINT_IPERF3_TAG, "  -b, --bandwidth #[KMG][/#] target bandwidth in bits/sec (0 for unlimited)\r\n");
    log_i(PRINT_IPERF3_TAG, "                            (default %d Mbit/sec for UDP, unlimited for TCP)\r\n", UDP_RATE / (1024*1024));
    log_i(PRINT_IPERF3_TAG, "                            (optional slash and packet count for burst mode)\r\n");
    log_i(PRINT_IPERF3_TAG, "  -t, --time      #         time in seconds to transmit for (default %d secs)\r\n", DURATION);
    log_i(PRINT_IPERF3_TAG, "  -n, --bytes     #[KMG]    number of bytes to transmit (instead of -t)\r\n");
    log_i(PRINT_IPERF3_TAG, "  -k, --blockcount #[KMG]   number of blocks (packets) to transmit (instead of -t or -n)\r\n");
    log_i(PRINT_IPERF3_TAG, "  -l, --len       #[KMG]    length of buffer to read or write\r\n");
    log_i(PRINT_IPERF3_TAG, "                            (default %d KB for TCP, %d Bytes for UDP)\r\n", DEFAULT_TCP_BLKSIZE/1024, DEFAULT_UDP_BLKSIZE);
    log_i(PRINT_IPERF3_TAG, "  -R, --reverse             run in reverse mode (server sends, client receives)\r\n");
    //log_i(PRINT_IPERF3_TAG, "  -w, --window    #[KMG]    set window size / socket buffer size\r\n");
    log_i(PRINT_IPERF3_TAG, "  -N, --no-delay            set TCP no delay, disabling Nagle's Algorithm\r\n");
    log_i(PRINT_IPERF3_TAG, "  -P, --parallel  #         number of parallel client streams to run\r\n");
    //log_i(PRINT_IPERF3_TAG, "  -O, --omit N              omit the first n seconds [0,%d]\r\n", MAX_OMIT);
    log_i(PRINT_IPERF3_TAG, "  -T, --title str           prefix every output line with this string\r\n");
}


void warning(char *str)
{
    log_i(PRINT_IPERF3_TAG, "warning: %s\r\n", str);
}
...
{
    if (test->debug)
        log_i(PRINT_IPERF3_TAG, "iperf state changed: %d --> %d\r\n", test->state, state);
...
iperf_check_throttle(struct iperf_stream *sp, struct timeval *nowP)
{
    ...
    if (sp->test->debug){
        //log_i(PRINT_IPERF3_TAG, "check_id:%ld, msec:%ld, bits_per_sec:%u \r\n", check_id++, msec, bits_per_second);
    }
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            if (sp->green_light &&
                ...
                if ((r = sp->snd(sp)) < 0) {
                    if (r == NET_SOFTERROR)
            			break;
            	    //log_i(PRINT_IPERF3_TAG, "iperf_send retval=%d \r\n", r);
...
send_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
    	...
    	if (test->debug) {
    	    log_i(PRINT_IPERF3_TAG, "send_parameters:\r\n%s\r\n", Iperf_cJSON_Print(j));
    	}
...
get_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
    	...
    	if (test->debug) {
    	    log_i(PRINT_IPERF3_TAG, "get_parameters:\r\n%s\r\n", Iperf_cJSON_Print(j));
    	}
        ...
        if ((j_p = Iperf_cJSON_GetObjectItem(j, "sctp")) != NULL)
        {
            ...
            #else
            log_i(PRINT_IPERF3_TAG, "NOT support SCTP yet!\r\n");
            test->i_errno = IERECVPARAMS;
            Iperf_cJSON_Delete(j);
            return -1;
            #endif
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "omit")) != NULL){
    	    ...
    	    if (test->omit < 0 || test->omit > MAX_OMIT){
   	            log_i(PRINT_IPERF3_TAG, "omit time range: [0,%d] \r\n", MAX_OMIT);
                test->i_errno = IERECVPARAMS;
                Iperf_cJSON_Delete(j);
                return -1;
    	    }
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "MSS")) != NULL) 
    	{
            ...
            #else
            log_i(PRINT_IPERF3_TAG, "NOT support setting MSS option!\r\n");
            test->i_errno = IERECVPARAMS;
            Iperf_cJSON_Delete(j);
            return -1;
            #endif    	    
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "parallel")) != NULL)	{
    	    ...
    	    if (test->num_streams > MAX_STREAMS)
    	    {
   	            log_i(PRINT_IPERF3_TAG, "max streams in parallel allowed: %d \r\n", MAX_STREAMS);
                test->i_errno = IERECVPARAMS;
                Iperf_cJSON_Delete(j);
                return -1;
    	    }
...
#if 0
    	    {
   	            log_i(PRINT_IPERF3_TAG, "NOT support reverse option in server mode \r\n");
                test->i_errno = IERECVPARAMS;
                cJSON_Delete(j);
                return -1;
    	    }
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "window")) != NULL){    	    
    	    ...
    	    if (test->settings->socket_bufsize > MAX_TCP_BUFFER)
    	    {
   	            log_i(PRINT_IPERF3_TAG, "max tcp window: %d \r\n", MAX_TCP_BUFFER);
                test->i_errno = IERECVPARAMS;
                Iperf_cJSON_Delete(j);
                return -1;
    	    }
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "len")) != NULL){
    	    ...
    	    if (test->settings->blksize > MAX_BLOCKSIZE){
   	            log_i(PRINT_IPERF3_TAG, "max block size allowed: %d \r\n", MAX_BLOCKSIZE);
    	        test->settings->blksize = MAX_BLOCKSIZE;
    	    }else if (test->settings->blksize < MIN_BLOCKSIZE){
   	            log_i(PRINT_IPERF3_TAG, "min block size allowed: %d \r\n", MIN_BLOCKSIZE);
    	...
    	if ((j_p = Iperf_cJSON_GetObjectItem(j, "burst")) != NULL){    	
    	    ...
    	    if (test->settings->burst > MAX_BURST || test->settings->burst <= 0){
   	            log_i(PRINT_IPERF3_TAG, "burst range: [1, %d] \r\n", MAX_BURST);
                test->i_errno = IERECVPARAMS;
                Iperf_cJSON_Delete(j);
                return -1;
            }
...
send_results(struct iperf_test *test)
{
    ...
    if (j == NULL) {
    	...
    	if (j_streams == NULL) {
    	    ...
    	    if (r == 0 && test->debug) {
        		log_i(PRINT_IPERF3_TAG, "send_results\r\n%s\r\n", Iperf_cJSON_Print(j));
    	    }
...
get_results(struct iperf_test *test)
{
    ...
    if (j == NULL) {
    	...
    	if (j_sender_has_retransmits == NULL) {
    	    ...
    	    if (test->debug) {
        		log_i(PRINT_IPERF3_TAG, "get_results:\r\n%s\r\n", Iperf_cJSON_Print(j));
    	    }
...
JSON_write(int fd, Iperf_cJSON *json)
{
    ...
    if (str == NULL)
    {
        log_i(PRINT_IPERF3_TAG, "cJSON_PrintUnformatted failed\r\n");
    	r = -1;
    }
    else {
    	hsize = STRLEN(str);
...
iperf_stats_callback(struct iperf_test *test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
    	...
    	if (test->protocol->id == Ptcp) {
    	    ...
    	    if (test->debug){
        	    //log_i(PRINT_IPERF3_TAG, "sp->jit:%ld, tmp.packet count:%ld\r\n", sp->jitter, temp.interval_packet_count);
        	    //if (irp){
        	    //    log_i(PRINT_IPERF3_TAG, "irp->jit:%ld, irp->packet cout:%ld\r\n", irp->jitter, irp->interval_packet_count);
        	    //}
        	    //log_i(PRINT_IPERF3_TAG, "jitter: %ld\r\n", temp.jitter);
        	}
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams) {
	    ...
	    if (irp == NULL) {
	        log_e(PRINT_IPERF3_TAG, "iperf_print_intermediate error: interval_results is NULL \r\n");
	        return;
	    }
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, Iperf_cJSON *json_interval_streams)
{
    ...
    if (irp == NULL) {
    	log_e(PRINT_IPERF3_TAG, "print_interval_results error: interval_results is NULL \r\n");
        return;
    }
	/* First stream? */
...
iperf_printf(struct iperf_test *test, const char* format, ...)
{
    ...
    if (test->role == 'c') {
    	if (test->title)
    	    log_i(PRINT_IPERF3_TAG, "%s:  ", test->title);
    	va_start(argp, format);
    	r = vsnprintf(linebuffer, sizeof(linebuffer), format, argp);
    	va_end(argp);
    	log_i(PRINT_IPERF3_TAG, "%s", linebuffer);
    }
    else if (test->role == 's') {
    	va_start(argp, format);
    	r = vsnprintf(linebuffer, sizeof(linebuffer), format, argp);
