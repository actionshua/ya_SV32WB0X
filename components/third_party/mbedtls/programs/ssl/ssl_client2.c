...
#define MAX_REQUEST_SIZE_STR "1200"
...
typedef struct options
{
    ...
    double debug_time;          /* debug_time mode */
} options;
...
int ssl_client2( int argc, char *argv[] )
{
    ...
    int debug_exit = 0;
    unsigned int cnt = 0;
    unsigned int tran_len = 0, read_len = 0, rand_len;
    int ret = 0, len, tail_len, i, written, frags, retry_left;
    mbedtls_net_context server_fd;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info_get = NULL;
    unsigned char buf[MAX_REQUEST_SIZE + 1], send_buf[MAX_REQUEST_SIZE + 1], read_buf[MAX_REQUEST_SIZE + 1];
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        *q++ = '\0';
        ...
        else if( strcmp( p, "debug" ) == 0 )
        {
            switch( atoi( q ) )
            {
                case 0: opt.debug = 0; break;
                case 1: opt.debug= 1; break;
                case 2: opt.debug= 2; break;
                case 3: opt.debug= 3; break;
                case 4: opt.debug= 4; break;
                default: goto usage;
            }
        } else if( strcmp( p, "debug_time" ) == 0 )
        {
            opt.debug_time = atoi( q );
        }
        else
            goto usage;
        }
...
send_request:
    ...
    if( opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM )
    {
        ...
        {
            if(opt.debug == 3) {
                rand_len = (int)(OS_Random() & 0x3ff);
                if(rand_len <= 1)
                    rand_len = 2;
                for(cnt = 0; cnt < rand_len; cnt++)
                    buf[cnt] = cnt;
                buf[rand_len -1] = '\r';
                len = rand_len;
                if(debug_exit == 1) {
            ...
            if(opt.debug == 3) {
                ...
                time_val = second_time.tv_sec - first_time.tv_sec;
 
                if(time_val >= opt.debug_time) {
                    debug_exit = 1;
        }
                if(msg_dump_cnt > 1000) {
                    OS_EnterCritical();
                    printf("send cipher: %s time: %d tran size: %d\n", ciphersuite_info_get->name, time_val, tran_len);
                    OS_ExitCritical();
                    msg_dump_cnt = 0;
                }
                msg_dump_cnt++;
        ...
        while( (written < len && opt.debug != 3) || ((len != 1) && (opt.debug == 3)));
    }
    ...
    buf[written] = '\0';
    if(opt.debug == 2) {
        memcpy(send_buf, buf, written);
    } else if(opt.debug == 3) { 
        printf( "%d seconds tran: %dKBps avg: %d KBps\n", time_val, tran_len, (tran_len /time_val)/1000);
    } else {
    ...
    if( opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM )
    {
        ...
        do
        {
            ...
            tran_len += ret;
            buf[len] = '\0';
            memcpy(read_buf+read_len, buf, len);
            read_len +=len;
            gettimeofday(&second_time, NULL);
            if(opt.debug == 2) {
                mbedtls_printf( "debug %d bytes read\n\n", len);
            } else if(opt.debug == 4) {
                time_val = second_time.tv_sec - first_time.tv_sec;
                if(msg_dump_cnt > 5) {
                    OS_EnterCritical();
                    printf("recv cipher: %s time: %d tran size: %d\n", ciphersuite_info_get->name, time_val, tran_len);
                    OS_ExitCritical();
                    msg_dump_cnt = 0;
                }
                msg_dump_cnt++;
            ...
            if(opt.debug == 4) {
                //dump(read_buf, read_len);
                if(buf[len - 1] == '\r') {
                    for(cnt = 0; cnt < (read_len -1); cnt++) {
                        if((cnt&0xff) != read_buf[cnt]) {
                            printf("===compare not match(%d %d %d)===\n", cnt, (cnt&0xff), read_buf[cnt]);
                            dump(read_buf, read_len);
                            while(1);
                        }
                    }
...
#if 1
    if(opt.debug == 1) {
        //const mbedtls_ssl_ciphersuite_t *ciphersuite_info_get;
        ciphersuite_info_get =
            mbedtls_ssl_ciphersuite_from_id( opt.force_ciphersuite[0] );
        len = mbedtls_snprintf( (char *) buf, sizeof( buf ) - 1, HTTP_RESPONSE,
                               ciphersuite_info_get->name);
        for(cnt = 0; cnt < (unsigned int)len; cnt++) {
            if(buf[cnt] != read_buf[cnt]) {
                printf("===compare not match: %d===\n", read_len);
                dump(read_buf, read_len);
                while(1);
            }
        }
