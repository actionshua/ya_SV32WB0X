...
void ssl_mbed_test()
{
    if (OS_TaskCreate(test_task, "test_task", 4096, NULL, OS_TASK_MIDDLE_PRIO, NULL) != 1)
    {
...
void ssl_mbed_thoughtput_test(char *ip, u16 port, u8 sec)
{
    ...
    ssl_port = port;
    test_sec = sec;
    if (OS_TaskCreate(thoughtput_test, "thoughtput_test", 4096, NULL, OS_TASK_MIDDLE_PRIO, NULL) != 1)
    {
        printf("Failed to create test task\n");
    }
}
