...
exchange( char **argv ) {
    ...
    while ( top > middle && middle > bottom ) {
        if ( top - middle > middle - bottom ) {
            ...
            for ( i = 0; i < len; i++ ) {
                tem = argv[bottom + i -1];
                argv[bottom + i -1] = argv[top - (middle - bottom) + i -1];
                argv[top - (middle - bottom) + i -1] = tem;
            }
...
#define NONOPTION_P (argv[(gnu_optind-1)][0] != '-' || argv[(gnu_optind-1)][1] == '\0')
    ...
    if ( nextchar == NULL || *nextchar == '\0' ) {
       ...
       moved back by the user (who may also have changed the arguments).  */
        ...
        if ( ordering == PERMUTE ) {
            ...
            while ( gnu_optind < (argc+1) && NONOPTION_P )
                gnu_optind++;
            last_nonopt = gnu_optind;
        }
       ...
       then skip everything else like a non-option.  */
        ...
        if ( gnu_optind != (argc+1) && !strcmp (argv[(gnu_optind-1)], "--") ) {//if ( gnu_optind != argc && !strcmp (argv[gnu_optind], "--") ) {
            gnu_optind++;
       ...
       either stop the scan or describe it to the caller and pass it by.  */
        ...
        if ( NONOPTION_P ) {
            if ( ordering == REQUIRE_ORDER )
                return -1;
            gnu_optarg = argv[(gnu_optind-1)];//gnu_optarg = argv[gnu_optind++];
            gnu_optind++;
            return 1;
        }
       ...
       Skip the initial punctuation.  */
        ...
        nextchar = (argv[(gnu_optind-1)] + 1
                    + (longopts != NULL && argv[(gnu_optind-1)][1] == '-'));
    }
    ...
    if ( longopts != NULL
         && (argv[(gnu_optind-1)][1] == '-'
             || (long_only && (argv[(gnu_optind-1)][2] || !my_index (optstring, argv[(gnu_optind-1)][1])))) ) {
        char *nameend;
                    ...
                    if ( gnu_opterr ) {
                        if ( argv[gnu_optind - 2][1] == '-' ) {
                            ...
                            printf (
                                     ...
                                     argv[0], argv[gnu_optind - 2][0], pfound->name);
                        }
                    }
       ...
       Otherwise interpret it as a short option.  */
        if ( !long_only || argv[(gnu_optind-1)][1] == '-'
            ...
            if ( gnu_opterr ) {
                if ( argv[(gnu_optind-1)][1] == '-' )
                    ...
                    printf ( _("%s: unrecognized option `%c%s'\n"),
                             argv[0], argv[(gnu_optind-1)][0], nextchar);
            }
            nextchar = (char *) "";
            gnu_optind++;
            gnu_optopt = 0;
