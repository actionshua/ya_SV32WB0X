    ...
    do{
        ...
        else //correct packet type
        {
            ...
            if(packet->data.blockNumber == blockNumber)
            {
...
#else
                //log_i(PRINT_TFTP_TAG, "packet.data.dataSize = %d, blockNumber = %d \n", packet->data.dataSize, blockNumber);
                blockNumber++;        
#endif
                if (DEBUG) log_i(PRINT_TFTP_TAG, "Sending Ack: [%u] ",packet->data.blockNumber);
                ...
                re_ack = 0;
                re_try = 0;
                if (DEBUG) log_i(PRINT_TFTP_TAG, "Sent\n");
            }
            else
            {
                if (DEBUG) log_i(PRINT_TFTP_TAG, "Error: expected block [%u] but got [%u]\n",blockNumber,packet->data.blockNumber);
                ...
                {
                    if (DEBUG) log_i(PRINT_TFTP_TAG, "Re-Sending Ack: [%u] ",packet->data.blockNumber);
                    //this should be mvoed ot of the else to be a genreric send
                    if (!send_ack(sockfd, (struct sockaddr *) &serv_addr, packet->data.blockNumber))
                    {
                        closesocket(sockfd);
