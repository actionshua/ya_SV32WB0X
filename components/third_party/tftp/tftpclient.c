...
void getFile(int port, char *filename)
{
  ...
  {
    log_e(PRINT_TFTP_TAG, "We do not support file transfer out of the current working directory\n");
    return;
  }
  ...
  if(sockfd < 0)
  {
    log_e(PRINT_TFTP_TAG, "Couldn't open socket\n");
    return;
  }

  if(!send_RRQ(sockfd, (struct sockaddr *) &serv_addr, filename, TFTP_SUPORTED_MODE))
  {
    log_e(PRINT_TFTP_TAG, "Error: couldn't send RRQ\n");
    return;
  }
  if(!recvFile(sockfd, (struct sockaddr *) &serv_addr, file,filename))
  {
    log_e(PRINT_TFTP_TAG, "Error: didn't receive file\n");
    return;
  }
  fclose(file);
  return;
...
void putFile(int port, char *filename)
{
  ...
  {
    log_e(PRINT_TFTP_TAG, "Error: couldn't send WRQ to server\n");
    return;
  }
  while (timeout_counter < MAX_TFTP_TIMEOUTS)
  {
    ...
    if (result < 0)
    {
      log_e(PRINT_TFTP_TAG, "Error: Timeout sending packet to server\n");
      if(!send_WRQ(sockfd, (struct sockaddr *) &serv_addr, filename, TFTP_SUPORTED_MODE))
      {
        log_e(PRINT_TFTP_TAG, "Error: couldn't send WRQ to server\n");
        return;
  ...
  {
    //we still timed out
    log_e(PRINT_TFTP_TAG, "Timed out after %d tries, is the server running\n",MAX_TFTP_TIMEOUTS);
    fclose(fileh);
    return;
  }
  ...
  }else
  {
    ...
    {
      log_e(PRINT_TFTP_TAG, "Unable to send file to server\n");
    }
  }
  fclose(fileh);
  return;
  ...
  if(( argc == 3 || argc == 5) && argv[argOffset][0] == '-')
  {
    switch(argv[argOffset][1])
    {
      ...
      case 'r':
        ...
        return;
      default: 
        log_e(PRINT_TFTP_TAG, "Unknown action [%s] \n",argv[argOffset]);
        break;
    }
    //print usage
  }
  log_i(PRINT_TFTP_TAG, "Usage: %s [-p port] (-w putfile || -r getFile)\n",argv[0]);
}
