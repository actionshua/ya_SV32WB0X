...
int waitForPacket(int sockfd, struct sockaddr* cli_addr, uint16_t optcode, PACKET* packet)
{
  ...
  while (true)
  {
    if (DEBUG) log_i(PRINT_TFTP_TAG, "Waiting for response..");
    FD_ZERO(&readSet); 
    ...
    if (rc == 0) {
      if (DEBUG) log_i(PRINT_TFTP_TAG, "Timeout detected\n");
      return -1; 
    }
    else if (rc < 0) {
      if (DEBUG) log_i(PRINT_TFTP_TAG, "Errno with value[%d]\n",errno);
    }
    
    n = recvfrom(sockfd, buffer, BUFSIZE, 0, cli_addr, (socklen_t *)&cli_size);
    if (DEBUG) log_i(PRINT_TFTP_TAG, "done\n");
    if (errno != 0)
    {
      if (errno == EINTR)
      {
