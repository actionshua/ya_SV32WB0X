...
void run_child(struct sockaddr cli_addr, PACKET * packet)
{
  ...
  switch (packet->optcode)
  {
    case TFTP_OPTCODE_RRQ:
      log_i(PRINT_TFTP_TAG, "Prossessng client READ request for %s\n",packet->read_request.filename);
      result = server_send(child_sockfd,&cli_addr,packet);
      break;
    case TFTP_OPTCODE_WRQ:
      log_i(PRINT_TFTP_TAG, "Prossessng client WRITE request for %s\n",packet->read_request.filename);
  ...
  if (result == false)
  {
    log_e(PRINT_TFTP_TAG, "Failed to handle client request\n");
  }
  log_i(PRINT_TFTP_TAG, "Done processing client request\n");
}
...
void packet_recieve_loop(int sockfd)
{
  ...
  log_i(PRINT_TFTP_TAG, "Server started\n");
  ...
  while (true)
  {
    ...
    if (recv_len <= 0)
    {
      log_i(PRINT_TFTP_TAG, "recvfrom error\n");
      perror("Socket read:");
      if (recv_len == 0)
      {
        log_e(PRINT_TFTP_TAG, "Socket closed\n");
      }
      exit(1);
    }
    ...
    if (childCount < MAX_TFTP_CLIENTS)
    {
      ...
      log_e(PRINT_TFTP_TAG, "Error: Server too busy to accept new clients\n");
    ...
    if (fork_id < 0)
    {
      log_e(PRINT_TFTP_TAG, "Error: could not fork child process\n");
    ...
    {
      //main server code
      if (DEBUG) log_i(PRINT_TFTP_TAG, "spawning fork: %u to handle new request\n",fork_id);
    }
...
int main(int argc, char *argv[])
{
  ...
  {
    //print usage
    log_i(PRINT_TFTP_TAG, "Usage: %s [-p port]\n",argv[0]);
    return 0;
  }
  ...
  //bind to our socket
  log_i(PRINT_TFTP_TAG, "Using Port %d\n",port);
