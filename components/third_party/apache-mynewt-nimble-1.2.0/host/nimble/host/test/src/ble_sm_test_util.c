                                  ...
                                  struct ble_sm_test_util_entity *out_entity)
{
    ...
    if (initiator) {
        ...
        out_entity->key_dist = params->pair_rsp.rssv_key_dist;

        out_entity->addr_type = params->rssv_addr_type;
        out_entity->id_addr = params->rssv_id_addr;
        out_entity->rpa = params->rssv_rpa;
                           ...
                           struct ble_sm_test_util_entity *out_peer)
{
    ...
    if (we_are_initiator) {
        ...
        ble_hs_cfg.sm_their_key_dist = out_us->pair_cmd->rssv_key_dist;
    } else {
        ble_hs_cfg.sm_our_key_dist = out_us->pair_cmd->rssv_key_dist;
    ...
    struct ble_sm_pair_cmd *exp_cmd)
{
    ...
    TEST_ASSERT(cmd.rssv_key_dist == exp_cmd->rssv_key_dist);
}
                         ...
                         int we_are_initiator)
{
    ...
    if (we_are_initiator) {
        peer_key_dist = params->pair_rsp.rssv_key_dist;
                                ...
                                int we_are_initiator)
{
    ...
    if (we_are_initiator) {
        ...
        our_key_dist = params->pair_rsp.rssv_key_dist;
...
ble_sm_test_util_us_lgcy_good(struct ble_sm_test_params *params)
{
    ...
    peer_addr.type = ble_hs_misc_addr_type_to_id(params->rssv_addr_type);
    memcpy(peer_addr.val, params->rssv_id_addr, sizeof peer_addr.val);
...
ble_sm_test_util_us_fail_inval(struct ble_sm_test_params *params)
{
    ...
    ble_sm_test_util_init();
    ble_hs_id_set_pub(params->rssv_id_addr);
