...
void
ble_mqueue_showinfo(struct ble_mqueue *mq)
{
    struct os_mbuf_pkthdr *mp;
    struct os_mbuf *om;
    os_sr_t sr;
	int count = 0;
	 struct os_mbuf_pkthdr *next;
    ...
    while (next) {
		count++;
		om = OS_MBUF_PKTHDR_TO_MBUF(next);
		printf("showinfo %d om_len %d\n", count, om->om_len);
        next = STAILQ_NEXT(next, omp_next);
    }
    OS_EXIT_CRITICAL(sr);
