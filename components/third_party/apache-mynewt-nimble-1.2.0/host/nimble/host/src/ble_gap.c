...
ble_gap_adv_stop_no_lock(void)
{
    ...
    BLE_HS_LOG(DEBUG, "GAP procedure initiated: stop advertising.\n");
...

{
    ...
    rc = ble_hs_hci_cmd_build_le_set_adv_params(&hci_adv_params,
                                                buf, sizeof buf);
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s build_l error rc = %d\n", __func__, rc);
    ...
    rc = ble_hs_hci_cmd_tx_empty_ack(BLE_HCI_OP(BLE_HCI_OGF_LE,
                                     ...
                                     buf, sizeof(buf));
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s tx_empty error rc = %d\n", __func__, rc);
        return rc;
    }
                     ...
                     const struct ble_gap_adv_params *adv_params)
{
    ...
    if (ble_gap_slave[0].op != BLE_GAP_OP_NULL) {
        BLE_HS_LOG(ERROR, "%s ble_gap_slave[0].op = %d\n", __func__, ble_gap_slave[0].op);
    ...
    case BLE_GAP_CONN_MODE_DIR:
         ...
         */
        if (!ble_hs_conn_can_alloc()) {
            BLE_HS_LOG(ERROR, "%s return BLE_HS_ENOMEM\n", __func__);
            return BLE_HS_ENOMEM;
        }
        break;
                  ...
                  ble_gap_event_fn *cb, void *cb_arg)
{
...
#if !NIMBLE_BLE_ADVERTISE || MYNEWT_VAL(BLE_EXT_ADV)
    BLE_HS_LOG(ERROR, "%s return BLE_HS_ENOTSUP\n", __func__);
    return BLE_HS_ENOTSUP;
#else
    ...
    if (rc != 0) {
        if (rc == BLE_HS_ENOMEM)
            BLE_HS_LOG(DEBUG, "%s adv_validate error rc = %d\n", __func__, rc);
        else
            BLE_HS_LOG(ERROR, "%s adv_validate error rc = %d\n", __func__, rc);
        goto done;
    }
    ...
    if (duration_ms != BLE_HS_FOREVER) {
        ...
        if (rc != 0) {
            /* Duration too great. */
            rc = BLE_HS_EINVAL;
            BLE_HS_LOG(ERROR, "%s Duration too great error rc = %d\n", __func__, rc);
            goto done;
        }
    }
    ...
    if (!ble_hs_is_enabled()) {
        rc = BLE_HS_EDISABLED;
        BLE_HS_LOG(ERROR, "%s hs_is_enabled error rc = %d\n", __func__, rc);
        goto done;
    }

    if (ble_gap_is_preempted()) {
        rc = BLE_HS_EPREEMPTED;
        BLE_HS_LOG(ERROR, "%s preempted error rc = %d\n", __func__, rc);
        goto done;
    }

    rc = ble_hs_id_use_addr(own_addr_type);
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s id_use_addr error rc = %d\n", __func__, rc);
        goto done;
    }

    BLE_HS_LOG(DEBUG, "GAP adv disc_mode %d, itvl_min %d, itvl_max %d\n", adv_params->disc_mode, adv_params->itvl_min, adv_params->itvl_max);
    ...
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s adv_params_tx error rc = %d\n", __func__, rc);
        goto done;
    }
...
ble_gap_adv_set_data(const uint8_t *data, int data_len)
{
    ...
    if (data_len > BLE_HCI_MAX_ADV_DATA_LEN) {
        printf("error ble_gap_adv_set_data data_len %d\n", data_len);
        ASSERT(0);
    }
    ...
    rc = ble_hs_hci_cmd_build_le_set_adv_data(data, data_len, buf,
                                              sizeof(buf));
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s build error rc = %d\n", __func__, rc);
        goto done;
    }
...
ble_gap_adv_rsp_set_data(const uint8_t *data, int data_len)
{
    ...
    rc = ble_hs_hci_cmd_build_le_set_scan_rsp_data(data, data_len,
                                                   buf, sizeof(buf));
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s build fail rc %d\n", __func__, rc);
    ...
    rc = ble_hs_hci_cmd_tx_empty_ack(opcode, buf, sizeof(buf));
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "%s tx_emptyd fail rc %d\n", __func__, rc);
        goto done;
    }
...
ble_gap_adv_set_fields(const struct ble_hs_adv_fields *adv_fields)
{
    ...
    if (buf_sz > BLE_HCI_MAX_ADV_DATA_LEN) {
        printf("error ble_gap_adv_set_fields buf_sz %d\n", buf_sz);
        ASSERT(0);
    }
             ...
             ble_gap_event_fn *cb, void *cb_arg)
{
    ...
    BLE_HS_LOG(INFO, "scan itvl 0x%x, win 0x%x ", params.itvl, params.window);
    BLE_HS_LOG(INFO, "\n");
                ...
                ble_gap_event_fn *cb, void *cb_arg)
{
    ...
    rc = ble_gap_conn_create_tx(own_addr_type, peer_addr,
                                conn_params);
    printf("ble_gap_conn_create_tx rc %d\n", rc);
    if (rc != 0) {
        ble_gap_master_reset_state();
        goto done;
    }
