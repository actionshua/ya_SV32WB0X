                       ...
                       struct ble_hs_hci_ack *out_ack)
{
    ...
    default:
        BLE_HS_DBG_ASSERT(0);
        rc = BLE_HS_EUNKNOWN;
        printf("%s rc = BLE_HS_EUNKNOWN\n", __func__);
        if (event_len > 4)
            printf("%s %02x%02x%02x%02x%02x\n", __func__,
                ble_hs_hci_ack[0], ble_hs_hci_ack[1], ble_hs_hci_ack[2], ble_hs_hci_ack[3], ble_hs_hci_ack[4]);
        break;
    }
                  ...
                  uint8_t *out_evt_buf_len)
{
    ...
    if (rc != 0) {
        printf("%s send_buf rc %d\n", __func__, rc);
        goto done;
    }

    rc = ble_hs_hci_wait_for_ack();
    if (rc != 0) {
        printf("%s wait_for_ack rc %d\n", __func__, rc);
    ...
    rc = ack.bha_status;
    if (rc)
        printf("%s bha_status rc %d, opcode %02X\n", __func__, ack.bha_status, opcode);
done:
                           ...
                           uint8_t pb_flag)
{
    ...
    om2 = os_mbuf_prepend(om, sizeof hci_hdr);//enlarge om sizeof hci_hdr
    if (om2 == NULL) {
        return NULL;
    }
...
{
    return ble_hs_hci_version;//todo fw report version 5.0
}
