...
ble_att_svr_rx_read_type(uint16_t conn_handle, struct os_mbuf **rxom)
{
    ...
    //printf("%s start_handle %d, end_handle %d\n", __func__, start_handle, end_handle); //werner debug
...
ble_att_svr_start(void)
{
    ...
    if (ble_hs_max_attrs > 0) {
        ble_att_svr_entry_mem = osi_malloc(
