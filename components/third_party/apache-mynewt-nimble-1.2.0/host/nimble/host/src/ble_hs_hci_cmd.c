...
ble_hs_hci_cmd_send(uint16_t opcode, uint8_t len, const void *cmddata)
{
    ...
    if (NULL == buf) {
        printf("!!!!!![%s] ble_hci_trans_buf_alloc NULL\n", __func__);
    }
    BLE_HS_DBG_ASSERT_EVAL(buf != NULL);
                                      ...
                                      uint8_t *dst)
{
    ...
    if ((adv->adv_itvl_min > adv->adv_itvl_max) ||
        ...
        printf("%s 1 parameters are not valid \n", __func__);
        return -1;
    }
 ...
 */
    ...
    if (adv->adv_type != BLE_HCI_ADV_TYPE_ADV_DIRECT_IND_HD) {
        if ((adv->adv_itvl_min < itvl) ||
            ...
            printf("%s adv_itvl_min %d < itvl %d return -1\n", __func__, adv->adv_itvl_min, itvl);
            return -1;
        }
    }
                                    ...
                                    uint8_t *dst)
{
    ...
    if (((data == NULL) && (len != 0)) || (len > BLE_HCI_MAX_ADV_DATA_LEN)) {
        if (data == NULL)
            printf("le_set_adv_data data == NULL\n");
