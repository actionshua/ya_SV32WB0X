...
{
    BLE_HS_LOG(INFO, "GATTC procedure initiated: %s", name);
}
                        ...
                        struct os_mbuf *txom)
{
    ...
    if (txom == NULL) {
        ...
        //printf("%s 1\n", __func__);
        //printf("%s %u bytes: %s\n", __func__, txom->om_len, bt_hex(txom->om_data, txom->om_len));
    ...
    //printf("%s 4 %u bytes: %s\n", __func__, txom->om_len, bt_hex(txom->om_data, txom->om_len));
    txom = NULL;
    if (rc != 0) {
        BT_ERR("%s tx_notify fail %d\n", __func__, rc);
        goto done;
    }

done:
    if (rc != 0) {
        BT_ERR("%s done fail %d\n", rc);
