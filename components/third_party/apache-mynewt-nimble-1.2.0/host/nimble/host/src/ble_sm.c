...
ble_sm_key_dist(struct ble_sm_proc *proc,
                uint8_t *out_init_key_dist, uint8_t *out_rssv_key_dist)
{
...
ble_sm_pair_cfg(struct ble_sm_proc *proc)
{
    ...
    ble_sm_key_dist(proc, &init_key_dist, &rssv_key_dist);
    if (proc->flags & BLE_SM_PROC_F_INITIATOR) {
        rx_key_dist = rssv_key_dist;
    } else {
     ...
     */
    if (!(proc->flags & BLE_SM_PROC_F_SC) &&
        (init_key_dist == 0 && rssv_key_dist == 0)) {
...
ble_sm_pair_req_fill(struct ble_sm_proc *proc)
{
    ...
    req->rssv_key_dist = ble_hs_cfg.sm_their_key_dist;
}
...
ble_sm_pair_rsp_fill(struct ble_sm_proc *proc)
{
    ...
    rsp->rssv_key_dist = req->rssv_key_dist &
