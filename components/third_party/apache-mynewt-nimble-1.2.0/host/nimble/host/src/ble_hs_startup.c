...
ble_hs_startup_read_local_ver_tx(void)
{
    ...
    //printf("%s call ble_hs_hci_set_hci_version 0x%02X\n", __func__, hci_version);
...
ble_hs_startup_set_evmask_tx(void)
{
    ...
    if (version >= BLE_HCI_VER_BCS_4_1) {
        ...
        rc = ble_hs_hci_cmd_tx_empty_ack(BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND,
                                         ...
                                         buf, sizeof(buf));
        //todo reply EVENT_MASK2 and support 5.0 version
        if (rc != 0) {
            return rc;
        }
    }
...
static int
ble_hs_startup_set_bd_addr(void)
{
    int rc;

    uint8_t zro_array[BD_ADDR_LEN] = {0};
    uint8_t max_array[BD_ADDR_LEN] = {0};
    uint8_t mac_addr[BD_ADDR_LEN] = {0};
    void *cfg_handle = wifi_cfg_init();
    /* read bd address*/
    memset(mac_addr, 0, sizeof(mac_addr));
    memset(zro_array, 0, sizeof(zro_array));
    memset(max_array, 255, sizeof(max_array));

    get_local_mac(1/*AP, mac2*/, mac_addr, sizeof(mac_addr));

    if ((memcmp(zro_array, mac_addr, BD_ADDR_LEN) == 0) ||
        (memcmp(max_array, mac_addr, BD_ADDR_LEN) == 0)) {/* flash bt address is zero, use default*/
        BLE_HS_LOG(ERROR, "BLE MAC Read From local is 0 or 255, use fw default\n");
        return 0;
    }
    BLE_HS_LOG(ERROR, "BLE MAC Read From local\n");
    rc = ble_hs_hci_cmd_tx_empty_ack(BLE_HCI_OP(BLE_HCI_OGF_VENDOR,
                                                BLE_HCI_OCF_SSV_SET_PUBLIC_BD_ADDR),
                                     mac_addr, BD_ADDR_LEN);
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "ble_hs_startup_set_bd_addr rc %d\n", rc);
        return rc;
    }
...
ble_hs_startup_go(void)
{
    ...
    if (rc != 0) {
        printf("%s ble_hs_startup_read_sup_f_tx fail rc = %d\n", __func__, rc);
        return rc;
    }
#endif
    ...
    if (rc != 0) {
        printf("%s ble_hs_startup_set_evmask_tx fail rc = %d\n", __func__, rc);
        return rc;
    }

    rc = ble_hs_startup_le_set_evmask_tx();
    if (rc != 0) {
        printf("%s ble_hs_startup_le_set_evmask_tx fail rc = %d\n", __func__, rc);
        return rc;
    }

    rc = ble_hs_startup_read_buf_sz();
    if (rc != 0) {
        printf("%s ble_hs_startup_read_buf_sz fail rc = %d\n", __func__, rc);
        return rc;
    }

    rc = ble_hs_startup_le_read_sup_f_tx();
    if (rc != 0) {
        printf("%s ble_hs_startup_le_read_sup_f_tx fail rc = %d\n", __func__, rc);
        return rc;
    }

    rc = ble_hs_startup_read_bd_addr();
    if (rc != 0) {
        printf("%s ble_hs_startup_read_bd_addr fail rc = %d\n", __func__, rc);
        return rc;
    }
