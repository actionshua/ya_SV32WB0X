...
ble_hs_hci_evt_disconn_complete(uint8_t event_code, uint8_t *data, int len)
{
    ...
    evt.reason = data[5];
    printf("disconn handle %d, reason 0x%x\n", evt.connection_handle, evt.reason);
    ble_hs_lock();
...
ble_hs_hci_evt_le_conn_complete(uint8_t subevent, uint8_t *data, int len)
{
    ...
    if (evt.status == BLE_ERR_SUCCESS) {
        ...
        printf("\33[32mconn complete\33[0m\r\n");
    } else {
#if MYNEWT_VAL(BLE_HS_DEBUG)
        evt.connection_handle = BLE_HS_CONN_HANDLE_NONE;
        printf("\33[32mconn fail status %d\33[0m\r\n", evt.status);
#endif
    }
...
ble_hs_hci_evt_le_adv_rpt(uint8_t subevent, uint8_t *data, int len)
{
    ...
    for (i = 0; i < num_reports; i++) {
        ...
        //debug
        if(desc.length_data > BLE_HS_ADV_MAX_SZ)
            return 0;
        //
        ...
        if(desc.data[0] >= BLE_HS_ADV_MAX_SZ) {
            printf("desc.data[0] %d err\n", desc.data[0]);
            dump_hex(data, len);
            continue;
        }

        if (bt_mesh_dbg_flag & BIT(0)) {
            printf("%02x:%02x:%02x:%02x:%02x:%02x rssi=%d\n", desc.addr.val[0],
                    desc.addr.val[1],
                    desc.addr.val[2],
                    desc.addr.val[3],
                    desc.addr.val[4],
...
ble_hs_hci_evt_acl_process(struct os_mbuf *om)
{
    ...
    if (hci_hdr.hdh_len != OS_MBUF_PKTHDR(om)->omp_len) {
        rc = BLE_HS_EBADDATA;
        printf("err BLE_HS_EBADDATA hdh_len %d, omp_len %d\n",
