...
ssv_ble_mesh_model_t *ble_mesh_get_model(uint16_t model_id)
{
    ...
    switch (model_id) {
    case SSV_BLE_MESH_MODEL_ID_CONFIG_SRV:
        model = config_server_models;
        break;
#if (CONFIG_BLE_MESH_CFG_CLI)
    case SSV_BLE_MESH_MODEL_ID_CONFIG_CLI:
        model = &gen_onoff_cli_models[1];
        break;
#endif
    case SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV:
        model = &gen_onoff_srv_models[1];
        break;
#if (CONFIG_BLE_MESH_GENERIC_ONOFF_CLI)
    case SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_CLI:
        model = &gen_onoff_cli_models[3];
        break;
#endif
#if CONFIG_PERFORMANCE_CLIENT_MODEL
    case SSV_BLE_MESH_VND_MODEL_ID_TEST_PERF_CLI:
        //model = &test_perf_cli_models[0];
        model = &gen_perf_srvcli_models[1];
        break;
    case SSV_BLE_MESH_VND_MODEL_ID_TEST_PERF_SRV:
        //model = &test_perf_srv_models[0];
        model = &gen_perf_srvcli_models[0];
        break;
#endif
    }
...
uint8_t ble_mesh_node_init(void)
{
    ...
    if (!ble_mesh_node_sema) {
        SSV_LOGE(TAG, "%s failed to init, failed to create mesh node semaphore", __func__);
        return -1;
    }
    return 0;
}
...
void ble_mesh_create_send_data(char *data, uint16_t byte_num, uint16_t sequence_num, uint32_t opcode)
{
    ...
    switch (opcode) {
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_GET:
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_SET:
        ...
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_SET_UNACK:
        data[2] = VENDOR_MODEL_PERF_OPERATION_TYPE_SET_UNACK;
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_ITEM_SET:
        data[2] = VENDOR_MODEL_PERF_OPERATION_TYPE_ITEM_SET;
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST:
        data[2] = VENDOR_MODEL_PERF_OPERATION_TYPE_TEST;
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST_UNACK:
        data[2] = VENDOR_MODEL_PERF_OPERATION_TYPE_TEST_UNACK;
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST_RESULT:
        data[2] = VENDOR_MODEL_PERF_OPERATION_TYPE_TEST_RESULT;
        break;
    }
...
void ble_mesh_test_performance_client_model_get(void)
{
    ...
    SSV_LOGI(TAG, "VendorModel:Statistics,%d,%d\n",
...
void ble_mesh_test_performance_client_model_get_received_percent(void)
{
    ...
    statistics_time_percent = osi_malloc(sizeof(statistics_time_performance) * time_level_num);
    ...
    // for script match
    SSV_LOGI(TAG, "VendorModel:Statistics");
    for (j = 0; j < time_level_num; j++) {
...
int ble_mesh_test_performance_client_model_init(uint16_t node_num, uint32_t test_num, uint8_t ttl)
{
    ...
    // malloc time
    test_perf_statistics.time = osi_malloc(test_num * sizeof(uint16_t));
    if (test_perf_statistics.time == NULL) {
        SSV_LOGE(TAG, " %s %d, malloc fail\n", __func__, __LINE__);
        return 1;
    }

    test_perf_statistics.package_index = osi_malloc(test_num * sizeof(uint16_t));
...
int ble_mesh_node_statistics_init(uint16_t package_num)
{
    ...
    ble_mesh_node_statistics.package_index = osi_malloc(sizeof(uint16_t) * package_num);
...
int8_t arg_string_to_value(const char *str, uint8_t strlen, uint8_t *outbuf, uint8_t outbuflen)
{
    ...
    if (str == NULL || strlen%2 || strlen == 0 || outbuf == NULL || outbuflen == 0)
        return -1;
    ...
    for (i = 0; i <= strlen/2; i+=2){
        ...
        outbuf[i/2] = strtol(temp_str, NULL, 16);
    }
