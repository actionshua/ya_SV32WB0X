...
typedef struct {
    ...
    struct arg_int *msg_timeout;
    struct arg_int *seq_number;
    //struct arg_str *send_data;
    struct arg_end *end;
...
void ble_mesh_test_performance_client_model_throughput(void *params)
{
    ...
    // create send data
    data = osi_malloc(profile_context->length);
    if (data == NULL) {
        SSV_LOGE(TAG, " %s, %d, malloc fail\n", __func__, __LINE__);
    }
    ...
    for (i = 1; i <= profile_context->test_num; i++) {
        ...
        start_time = ssv_timer_get_time();
        ssv_ble_mesh_client_model_send_msg(profile_context->model, &ctx, profile_context->opcode,
    ...
    SSV_LOGI(TAG, "VendorModel:SendPackage,Finish");
    free(params);
    OS_TaskDelete(NULL);
    SSV_LOGD(TAG, "exit %s\n", __func__);
}
...
int ble_mesh_test_performance_client_model(int argc, char **argv)
{
    ...
    if (strcmp(test_perf_client_model.action_type->sval[0], "init") == 0) {
        ...
        result = ssv_ble_mesh_client_model_init(model);
        if (result == SSV_BM_OK) {
            SSV_LOGI(TAG, "VendorClientModel:Init,OK");
        }
    } else if (strcmp(test_perf_client_model.action_type->sval[0], "start") == 0) {
        profile_data = osi_malloc(sizeof(ble_mesh_test_perf_throughput_data));
        ...
        if (profile_data->opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_SET) {
            profile_data->need_ack = true;
        } else {
            profile_data->need_ack = false;
        }

        OS_TaskCreate(ble_mesh_test_performance_client_model_throughput, "MESHTHROUGHPUTSEND", 4048, profile_data, 1, NULL);
    }

    SSV_LOGD(TAG, "exit %s\n", __func__);
    return result;
}
#define SSV_PERF_MODEL_SEND_DATA_MAX_LEN 31
int ble_mesh_test_performance_client_model_ssv_test(int argc, char **argv)
{
    ssv_ble_mesh_model_t *model;
    ssv_err_t result = SSV_FAIL;
    ssv_ble_mesh_msg_ctx_t ctx;
    uint8_t need_ack = 0;
    uint8_t *data = NULL;
    uint8_t send_data[SSV_PERF_MODEL_SEND_DATA_MAX_LEN];
    int8_t ret = 0;
    ...
    if (strcmp(test_perf_client_model.action_type->sval[0], "init") == 0) {
        //ble_mesh_test_perf_sema = xSemaphoreCreateMutex();//need init ?
        result = ssv_ble_mesh_client_model_init(model);
        if (result == SSV_BM_OK) {
            SSV_LOGI(TAG, "VendorClientModelSSV:Init,OK");
        }
    } else if (strcmp(test_perf_client_model.action_type->sval[0], "set") == 0) {
            ctx.model = model;
            ctx.send_rel = 0;
            //arg_int_to_value(test_perf_client_model.playload_byte, profile_data->length, "length");
            //arg_int_to_value(test_perf_client_model.opcode, profile_data->opcode, "opcode");
            arg_int_to_value(test_perf_client_model.unicast_address, ctx.addr, "publish address");
            arg_int_to_value(test_perf_client_model.ttl, ctx.send_ttl, "model ttl");
            arg_int_to_value(test_perf_client_model.app_idx, ctx.app_idx, "appkey index");
            arg_int_to_value(test_perf_client_model.net_idx, ctx.net_idx, "network key index");
            //arg_int_to_value(test_perf_client_model.dev_role, profile_data->device_role, "device role");
            //arg_int_to_value(test_perf_client_model.test_num, profile_data->test_num, "test number");
            
            if (test_perf_client_model.opcode->ival[0] == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_ITEM_SET ||
                test_perf_client_model.opcode->ival[0] == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST) {
                need_ack = true;
            } else if (test_perf_client_model.opcode->ival[0] == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST_UNACK) {
                need_ack = false;
            } else {
                SSV_LOGI(TAG, "VendorClientModelSSV: opcode is not expected");
                return result;
            }
            SSV_LOGI(TAG, "VendorClientModelSSV: need_ack %d", need_ack);
#if 0
            if (test_perf_client_model.send_data->sval[0]) {/*user assign send data*/                
                ret = arg_string_to_value(test_perf_client_model.send_data->sval[0],
                        test_perf_client_model.send_data->count,
                        send_data, SSV_PERF_MODEL_SEND_DATA_MAX_LEN);

                if(ret < 0)
                    return result;
                
                ssv_ble_mesh_client_model_send_msg(model, &ctx, test_perf_client_model.opcode->ival[0],
                    ret, send_data, test_perf_client_model.msg_timeout->ival[0],
                    need_ack, test_perf_client_model.dev_role->ival[0]);
            } else {
                data = osi_malloc(test_perf_client_model.playload_byte->ival[0]);
                ble_mesh_create_send_data((char *)data, test_perf_client_model.playload_byte->ival[0],
                    test_perf_client_model.seq_number->ival[0], test_perf_client_model.opcode->ival[0]);               
                osi_free(data);
            }
#else
            data = osi_malloc(test_perf_client_model.playload_byte->ival[0]);
...
int ble_mesh_test_performance_client_model_performance(int argc, char **argv)
{
    ...
    if (strcmp(test_perf_client_model_statistics.action_type->sval[0], "init") == 0) {
        ...
        if (result == 0) {
            SSV_LOGI(TAG, "VendorPerfTest:InitStatistics,OK\n");
        }
        ...
        SSV_LOGI(TAG, "VendorPerfTest:DestroyStatistics,OK\n");
        ...
        SSV_LOGI(TAG, "VendorPerfTest:GetPercent,OK\n");
    }
...
void ble_mesh_performance_client_model_command(void)
{
    ...
    test_perf_client_model.msg_timeout = arg_int0("m", NULL, "<msg timeout>", "message timeout");
    test_perf_client_model.seq_number = arg_int0("s", NULL, "<seq_number>", "sequence number");
    //test_perf_client_model.send_data = arg_str1("d", NULL, "<send_data>", "send data");
    test_perf_client_model.end = arg_end(1);
#if 0
    const ssv_console_cmd_t test_perf_client_model_cmd = {
        .command = "bmtpcvm",
    ...
    SSV_ERROR_CHECK(ssv_console_cmd_register(&test_perf_client_model_cmd));
#endif
...
#if 0
    const ssv_console_cmd_t test_perf_client_model_performance_cmd = {
        .command = "bmcperf",
    ...
    SSV_ERROR_CHECK(ssv_console_cmd_register(&test_perf_client_model_performance_cmd));
#endif
}
