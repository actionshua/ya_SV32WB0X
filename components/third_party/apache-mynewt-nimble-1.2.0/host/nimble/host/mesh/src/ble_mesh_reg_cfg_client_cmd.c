...
void ble_mesh_configuration_client_model_cb(ssv_ble_mesh_cfg_client_cb_event_t event,
...
{
    uint32_t opcode;
    SSV_LOGD(TAG, "enter %s, event = %x\n, error_code = %x\n", __func__, event, param->error_code);

    if (!param->error_code) {
        opcode = param->params->opcode;
        switch (event) {
        case SSV_BLE_MESH_CFG_CLIENT_GET_STATE_EVT:
            switch (opcode) {
            case SSV_BLE_MESH_MODEL_OP_BEACON_GET:
                SSV_LOGI(TAG, "CfgClient:beacon,0x%x", param->status_cb.beacon_status.beacon);
                break;
            case SSV_BLE_MESH_MODEL_OP_COMPOSITION_DATA_GET:
                SSV_LOGI(TAG, "CfgClient:page,0x%x,len,0x%x", param->status_cb.comp_data_status.page, param->status_cb.comp_data_status.composition_data->len);
                break;
            case SSV_BLE_MESH_MODEL_OP_DEFAULT_TTL_GET:
                SSV_LOGI(TAG, "CfgClient:ttl,0x%x", param->status_cb.default_ttl_status.default_ttl);
                break;
            case SSV_BLE_MESH_MODEL_OP_GATT_PROXY_GET:
                SSV_LOGI(TAG, "CfgClient:proxy,0x%x", param->status_cb.gatt_proxy_status.gatt_proxy);
                break;
            case SSV_BLE_MESH_MODEL_OP_RELAY_GET:
                SSV_LOGI(TAG, "CfgClient:relay,0x%x,retransmit,0x%x", param->status_cb.relay_status.relay, param->status_cb.relay_status.retransmit);
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_PUB_GET:
                if (param->status_cb.model_pub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:PublishGet,OK,0x%x", param->status_cb.model_pub_status.publish_addr);
                } else {
                    SSV_LOGI(TAG, "CfgClient:PublishGet,Fail");
                }

                break;
            case SSV_BLE_MESH_MODEL_OP_FRIEND_GET:
                SSV_LOGI(TAG, "CfgClient:friend,0x%x", param->status_cb.friend_status.friend_state);
                break;
            case SSV_BLE_MESH_MODEL_OP_HEARTBEAT_PUB_GET:
                if (param->status_cb.heartbeat_pub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatPubGet,OK,destination:0x%x,countlog:0x%x,periodlog:0x%x,ttl:0x%x,features:0x%x,net_idx:0x%x",
                    ...
                    SSV_LOGI(TAG, "CfgClient:HeartBeatGet,Fail,%d", param->status_cb.heartbeat_pub_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_HEARTBEAT_SUB_GET:
                if (param->status_cb.heartbeat_sub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatSubGet,OK,source:0x%x,destination:0x%x, periodlog:0x%x,countlog:0x%x,minhops:0x%x,maxhops:0x%x",
                    ...
                    SSV_LOGI(TAG, "CfgClient:HeartBeatSubGet,Fail,%d", param->status_cb.heartbeat_sub_status.status);
                }
                break;
            default:
                SSV_LOGI(TAG, "Not supported config client get message opcode");
                break;
            }
            break;
        case SSV_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
            switch (opcode) {
            case SSV_BLE_MESH_MODEL_OP_BEACON_SET:
                SSV_LOGI(TAG, "CfgClient:beacon,0x%x", param->status_cb.beacon_status.beacon);
                break;
            case SSV_BLE_MESH_MODEL_OP_DEFAULT_TTL_SET:
                SSV_LOGI(TAG, "CfgClient:ttl,0x%x", param->status_cb.default_ttl_status.default_ttl);
                break;
            case SSV_BLE_MESH_MODEL_OP_GATT_PROXY_SET:
                SSV_LOGI(TAG, "CfgClient:proxy,0x%x", param->status_cb.gatt_proxy_status.gatt_proxy);
                break;
            case SSV_BLE_MESH_MODEL_OP_RELAY_SET:
                SSV_LOGI(TAG, "CfgClient:relay,0x%x, retransmit: 0x%x", param->status_cb.relay_status.relay, param->status_cb.relay_status.retransmit);
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_PUB_SET:
                if (param->status_cb.model_pub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:PublishSet,OK,0x%x", param->status_cb.model_pub_status.publish_addr);
                } else {
                    SSV_LOGI(TAG, "CfgClient:PublishSet,Fail");
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
                if (param->status_cb.model_sub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CnfClient:SubAdd,OK,%x,%x", param->status_cb.model_sub_status.element_addr, param->status_cb.model_sub_status.sub_addr);
                } else {
                    SSV_LOGI(TAG, "CnfClient:SubAdd,Fail,%x", param->status_cb.model_sub_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_DELETE:
                if (param->status_cb.model_sub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CnfClient:SubDel,OK,%x,%x", param->status_cb.model_sub_status.element_addr, param->status_cb.model_sub_status.sub_addr);
                } else {
                    SSV_LOGI(TAG, "CnfClient:SubDel,Fail,%x", param->status_cb.model_sub_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_OVERWRITE:
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_ADD:
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_DELETE:
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_SUB_VIRTUAL_ADDR_OVERWRITE:
                break;
            case SSV_BLE_MESH_MODEL_OP_NET_KEY_ADD:
                if (param->status_cb.netkey_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:NetKeyAdd,OK");
                } else {
                    SSV_LOGI(TAG, "CfgClient:NetKeyAdd,Fail,%d", param->status_cb.netkey_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_APP_KEY_ADD:
                if (param->status_cb.appkey_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CnfClient:AddAppkey,OK,%x,%x,%x", param->status_cb.appkey_status.net_idx, param->status_cb.appkey_status.app_idx, param->params->ctx.addr);
                } else {
                    SSV_LOGI(TAG, "CnfClient:AddAppkey,Fail,%x", param->status_cb.appkey_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
                if (param->status_cb.model_app_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CnfClient:AppkeyBind,OK,%x,%x,%x", param->status_cb.model_app_status.app_idx, param->status_cb.model_app_status.model_id, param->params->ctx.addr);
                } else {
                    SSV_LOGI(TAG, "CnfClient:AppkeyBind,Fail,%x", param->status_cb.model_app_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_FRIEND_SET:
                SSV_LOGI(TAG, "CfgClient:friend: 0x%x", param->status_cb.friend_status.friend_state);
                break;
            case SSV_BLE_MESH_MODEL_OP_HEARTBEAT_PUB_SET:
                if (param->status_cb.heartbeat_pub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatPubSet,OK,destination:0x%x,countlog:0x%x, periodlog:0x%x,ttl:0x%x,features:0x%x,net_idx: 0x%x",
                             param->status_cb.heartbeat_pub_status.dst, param->status_cb.heartbeat_pub_status.count, param->status_cb.heartbeat_pub_status.period,
                             param->status_cb.heartbeat_pub_status.ttl, param->status_cb.heartbeat_pub_status.features, param->status_cb.heartbeat_pub_status.net_idx);
                } else {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatSet,Fail,%d", param->status_cb.heartbeat_pub_status.status);
                }
                break;
            case SSV_BLE_MESH_MODEL_OP_HEARTBEAT_SUB_SET:
                if (param->status_cb.heartbeat_sub_status.status == SSV_BM_OK) {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatSubSet,OK,source:0x%x,destination:0x%x, periodlog:0x%x,countlog:0x%x,minhops:0x%x,maxhops:0x%x",
                             param->status_cb.heartbeat_sub_status.src, param->status_cb.heartbeat_sub_status.dst, param->status_cb.heartbeat_sub_status.period,
                             param->status_cb.heartbeat_sub_status.count, param->status_cb.heartbeat_sub_status.min_hops, param->status_cb.heartbeat_sub_status.max_hops);
                } else {
                    SSV_LOGI(TAG, "CfgClient:HeartBeatSubSet,Fail,%d", param->status_cb.heartbeat_sub_status.status);
                }
                break;
            default:
                SSV_LOGI(TAG, "Not supported config client set message opcode");
                break;
            }
            break;
        case SSV_BLE_MESH_CFG_CLIENT_PUBLISH_EVT:
            SSV_LOGI(TAG, "CnfClient:Publish,OK");
            break;
        case SSV_BLE_MESH_CFG_CLIENT_EVT_MAX:
            SSV_LOGI(TAG, "CnfClient:MaxEvt");
            break;
        case SSV_BLE_MESH_CFG_CLIENT_TIMEOUT_EVT:
            SSV_LOGI(TAG, "CfgClient:TimeOut");
            break;
        default:
            SSV_LOGI(TAG, "CfgClient:InvalidEvent");
            break;
        }
    } else {
        SSV_LOGI(TAG, "CnfClient:Fail,%d", param->error_code);
    }
    SSV_LOGD(TAG, "exit %s \n", __func__);
}
...
int ble_mesh_configuration_client_model_operation(int argc, char **argv)
{
    ...
    ssv_ble_mesh_cfg_default_ttl_set_t ttl_set;
    ssv_ble_mesh_cfg_gatt_proxy_set_t proxy_set;
    ssv_ble_mesh_cfg_app_key_add_t app_key_add;
    ssv_ble_mesh_cfg_model_pub_set_t mod_pub_set = {
        .company_id = 0xFFFF,
        .cred_flag = false,
        .publish_period = 0,
        .publish_retransmit = 0,
    };
    ssv_ble_mesh_cfg_model_sub_add_t mod_sub_add = {
        .company_id = 0xFFFF,
    };
    ssv_ble_mesh_cfg_model_sub_delete_t mod_sub_del = {
        .company_id = 0xFFFF,
    };
    ssv_ble_mesh_cfg_relay_set_t relay_set;
    ssv_ble_mesh_client_common_param_t client_common = {
    ...
    ssv_ble_mesh_cfg_model_app_bind_t mod_app_bind = {
        .company_id = 0xFFFF,
    };

    client_common.model = ble_mesh_get_model(SSV_BLE_MESH_MODEL_ID_CONFIG_CLI);
    ...
    if (configuration_client_model_operation.action_type->count != 0) {
        if (strcmp(configuration_client_model_operation.action_type->sval[0], "get") == 0) {
            err = ssv_ble_mesh_config_client_get_state(&client_common, &get_state);
            ...
            if (configuration_client_model_operation.set_state->count != 0) {
                if (strcmp(configuration_client_model_operation.set_state->sval[0], "appkey") == 0) {
                    ...
                    app_key = ssv_ble_mesh_provisioner_get_local_app_key(app_key_add.net_idx, app_key_add.app_idx);
                    if (app_key == NULL) {
                        SSV_LOGE(TAG, "CnfClient:AddAppkey,Fail,app key or network key NULL");
                        return SSV_FAIL;
                    } else {
                        memcpy(app_key_add.app_key, app_key, 16);
                    }
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&app_key_add);
#endif
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "appbind") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&mod_app_bind);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "ttl") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&ttl_set);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "proxy") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&proxy_set);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "subadd") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&mod_sub_add);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "subdel") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&mod_sub_del);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "relay") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&relay_set);
                } else if (strcmp(configuration_client_model_operation.set_state->sval[0], "pubset") == 0) {
                    err = ssv_ble_mesh_config_client_set_state(&client_common, (ssv_ble_mesh_cfg_client_set_state_t *)&mod_pub_set);
                }
            }
        } else if (strcmp(configuration_client_model_operation.action_type->sval[0], "reg") == 0) {
            err = ssv_ble_mesh_register_config_client_callback(ble_mesh_configuration_client_model_cb);
        }
    }

    if (err == SSV_BM_OK) {
        SSV_LOGI(TAG, "ConfigClient:OK");
    } else {
        SSV_LOGI(TAG, "ConfigClient:Fail");
    }

    SSV_LOGD(TAG, "exit %s %d\n", __func__, err);
    return err;
}
...
void ble_mesh_register_configuration_client_model_command(void)
{
...
#if 0
    const ssv_console_cmd_t client_stconfiguration_client_model_operationate_cmd = {
        .command = "bmccm",
    ...
    SSV_ERROR_CHECK(ssv_console_cmd_register(&client_stconfiguration_client_model_operationate_cmd));
#endif
}
#endif
