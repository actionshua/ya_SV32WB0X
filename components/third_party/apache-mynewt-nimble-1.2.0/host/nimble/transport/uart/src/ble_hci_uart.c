...
ble_hci_uart_acl_tx(struct os_mbuf *om)
{
    ...
    os_sr_t sr;
    int8_t type = BLE_HCI_UART_H4_ACL;
    struct os_mbuf *m;
    struct os_mbuf *next;
    struct os_mbuf *cur;
    int i = 0;
    ...
    acl[0] = BLE_HCI_UART_H4_ACL;

    cur = om;
    do {
        if (om->om_len > (sizeof(acl) - 1)) {
            printf("%s tx_len %d > %d @@@@@@@@@@@@\n", __func__, om->om_len, (sizeof(acl) - 1));
            break;
        }
        memcpy(&acl[1], cur->om_data, cur->om_len);
        hci_aclh(cur->om_len + 1, acl);
...
void cmd_if_ble_rx(u8 *cmd, u8 tx_len);
...
{/* h4_type  hci_ev*/
    ...
    tx_len = hci_ev[2] + BLE_HCI_CMD_HDR_LEN;

    if (hci_ev[0] == 0x0c && hci_ev[1] == 0x20)
        printf("cmd =%02X%02X%02X %02X%02X\n", hci_ev[0], hci_ev[1], hci_ev[2], hci_ev[3], hci_ev[4]);
    else
        printf("cmd =%02X%02X%02X\n", hci_ev[0], hci_ev[1], hci_ev[2]);

    cmd[0] = h4_type;

    if (tx_len > (sizeof(cmd) - 1)) {
        printf("%s tx_len %d > %d @@@@@@@@@@@@\n", __func__, tx_len, (sizeof(cmd) - 1));
        ble_hci_trans_buf_free(hci_ev);
        return 0;
    }
    memcpy(&cmd[1], hci_ev, tx_len);

    cmd_if_ble_rx(cmd, tx_len + 1);
...
{/* h4_type  hci_ev*/
    ...
    OS_EXIT_CRITICAL(sr);
    printf("%s call hal_uart_start_tx 0x%02X\n", __func__, h4_type);
    rc = hal_uart_start_tx(MYNEWT_VAL(BLE_HCI_UART_PORT));

    OS_ENTER_CRITICAL(sr);
    if (rc > 0) {
        printf("%s drv_hsuart_write_fifo tx: 0x%02X%02X%02X len = %d\n", __func__,
        ble_hci_uart_state.tx_cmd.data[0],
        ble_hci_uart_state.tx_cmd.data[1],
        ble_hci_uart_state.tx_cmd.data[2],
        ble_hci_uart_state.tx_cmd.len);
        drv_hsuart_write_fifo(&h4_type, 1, HSUART_BLOCKING);
        drv_hsuart_write_fifo(ble_hci_uart_state.tx_cmd.data,
...
ble_hci_uart_tx_char(void *arg)
{
    ...
    printf("%s ble_hci_uart_state.tx_type = 0x%02X\n",
        __func__, ble_hci_uart_state.tx_type);
    ...
    case BLE_HCI_UART_H4_EVT:
        ...
        printf("%s BLE_HCI_UART_H4_CMD ble_hci_uart_state.tx_type = 0x%02X, rc = %d\n",
        __func__, ble_hci_uart_state.tx_type, rc);
        break;
...
ble_hci_uart_rx_pkt_type(uint8_t data)
{
    ...
    case BLE_HCI_UART_H4_ACL:
        ...
        ble_hci_uart_state.om = ble_hci_trans_acl_buf_alloc();
        if (!ble_hci_uart_state.om) {
            printf("%s set SKIP_ACL\n", __func__);
        }
        break;
#endif
...
ble_hci_uart_rx_evt_cb()
{
    ...
    if (ble_hci_uart_state.rx_cmd.cur == ble_hci_uart_state.rx_cmd.len) {
        ASSERT(ble_hci_uart_rx_cmd_cb != NULL);
        //printf("evt %s\n", bt_hex(ble_hci_uart_state.rx_cmd.data, ble_hci_uart_state.rx_cmd.len));
        rc = ble_hci_uart_rx_cmd_cb(ble_hci_uart_state.rx_cmd.data,
                                    ble_hci_uart_rx_cmd_arg);
        if (rc != 0) {
            ble_hci_trans_buf_free(ble_hci_uart_state.rx_cmd.data);
...
ble_hci_uart_rx_acl(uint8_t data)
{
    ...
    ble_hci_uart_state.om->om_data[rxd_bytes] = data;
    ++rxd_bytes;
    ...
    //if (ble_hci_uart_state.rx_acl.rxd_bytes > 21)
    //    printf("%s lendata rx_acl.dptr %s\n", __func__, bt_hex(ble_hci_uart_state.rx_acl.dptr, ble_hci_uart_state.rx_acl.rxd_bytes));

    if (rxd_bytes == BLE_HCI_DATA_HDR_SZ) {
        //pktlen = ble_hci_uart_state.rx_acl.dptr[3];
        pktlen = ble_hci_uart_state.om->om_data[3];
        //pktlen = (pktlen << 8) + ble_hci_uart_state.rx_acl.dptr[2];
        pktlen = (pktlen << 8) + ble_hci_uart_state.om->om_data[2];
        ble_hci_uart_state.rx_acl.len = pktlen + BLE_HCI_DATA_HDR_SZ;
        //printf("%s ble_hci_uart_state.rx_acl.len %d, pktlen %d\n", __func__, ble_hci_uart_state.rx_acl.len, pktlen);
        //printf("%s lendata rx_acl.dptr %s\n", __func__, bt_hex(ble_hci_uart_state.rx_acl.dptr, ble_hci_uart_state.rx_acl.rxd_bytes));
        /*
        ...
        if (pktlen > MYNEWT_VAL(BLE_ACL_BUF_SIZE)) {
            os_mbuf_free_chain(ble_hci_uart_state.om);
#if MYNEWT_VAL(BLE_CONTROLLER)
            ble_hci_uart_sync_lost();
#else
        /*
    ...
    if (rxd_bytes == ble_hci_uart_state.rx_acl.len) {
        ...
        OS_MBUF_PKTLEN(ble_hci_uart_state.om) = rxd_bytes;
        ble_hci_uart_state.om->om_len = rxd_bytes;
        if (ble_hci_uart_state.om->om_len >= 30) {
            show = 1;
            //printf("%s rx len %u, om_data %s\n", __func__, ble_hci_uart_state.rx_acl.buf->om_len, bt_hex(ble_hci_uart_state.rx_acl.buf->om_data, ble_hci_uart_state.rx_acl.buf->om_len));
        }
        printf("%s rx len %u, dptr %s\n", __func__, ble_hci_uart_state.om->om_len, bt_hex(ble_hci_uart_state.om->om_data, 4/*ble_hci_uart_state.rx_acl.buf->om_len*/));
            
        ble_hci_uart_rx_acl_cb(ble_hci_uart_state.om,
                               ble_hci_uart_rx_acl_arg);
        //if (ble_hci_uart_state.rx_acl.buf->om_len >= 30) {
            //printf("%s acl_cb end\n", __func__);
            //printf("%s rx len %u, om_data %s\n", __func__, ble_hci_uart_state.rx_acl.buf->om_len, bt_hex(ble_hci_uart_state.rx_acl.buf->om_data, ble_hci_uart_state.rx_acl.buf->om_len));
        //}
        ble_hci_uart_state.rx_type = BLE_HCI_UART_H4_NONE;
...
ble_hci_uart_rx_acl(uint8_t data)
{
    ...
    rxd_bytes = ble_hci_uart_state.rx_acl.rxd_bytes;
    //ble_hci_uart_state.rx_acl.dptr[rxd_bytes] = data;
    ble_hci_uart_state.rx_acl.buf->om_data[rxd_bytes] = data;
    ++rxd_bytes;
    ble_hci_uart_state.rx_acl.rxd_bytes = rxd_bytes;
    ...
    if (rxd_bytes == BLE_HCI_DATA_HDR_SZ) {
        //pktlen = ble_hci_uart_state.rx_acl.dptr[3];
        pktlen = ble_hci_uart_state.rx_acl.buf->om_data[3];
        //pktlen = (pktlen << 8) + ble_hci_uart_state.rx_acl.dptr[2];
        pktlen = (pktlen << 8) + ble_hci_uart_state.rx_acl.buf->om_data[2];
        ble_hci_uart_state.rx_acl.len = pktlen + BLE_HCI_DATA_HDR_SZ;
        //printf("%s ble_hci_uart_state.rx_acl.len %d, pktlen %d\n", __func__, ble_hci_uart_state.rx_acl.len, pktlen);
        //printf("%s lendata rx_acl.dptr %s\n", __func__, bt_hex(ble_hci_uart_state.rx_acl.dptr, ble_hci_uart_state.rx_acl.rxd_bytes));
        /*
    ...
    if (rxd_bytes == ble_hci_uart_state.rx_acl.len) {
        ...
        rc = ble_hci_uart_rx_acl_cb(ble_hci_uart_state.rx_acl.buf,
                               ble_hci_uart_rx_acl_arg);
        ble_hci_uart_state.rx_type = BLE_HCI_UART_H4_NONE;
    }

    if (rc != 0) {
        printf("%s ble_hci_uart_rx_acl_cb err\n", __func__);
        os_mbuf_free_chain(ble_hci_uart_state.rx_acl.buf);
    }
}
#endif
...
ble_hci_uart_init(void)
{
    ...
    SYSINIT_PANIC_ASSERT(rc == 0);
    printf("**** %s os_mempool_init done\n", __func__);
    rc = ble_hci_uart_config();
    //SYSINIT_PANIC_ASSERT_MSG(rc == 0, "Failure configuring UART HCI");
    if (rc != 0)
        printf("Failure configuring UART HCI rc = %d\n", rc);
