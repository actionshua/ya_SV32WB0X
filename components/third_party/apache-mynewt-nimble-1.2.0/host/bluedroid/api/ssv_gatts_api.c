...
static ssv_err_t ssv_ble_gatts_add_char_desc_param_check(ssv_attr_value_t *char_val, ssv_attr_control_t *control);
...
ssv_err_t ssv_ble_gatts_register_callback(ssv_gatts_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_GATTS, callback) == 0 ? SSV_BM_OK : SSV_FAIL);
}
...
ssv_err_t ssv_ble_gatts_app_register(uint16_t app_id)
{
    ...
    //if (app_id < SSV_APP_ID_MIN || app_id > SSV_APP_ID_MAX) {
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t), NULL) == BT_STATUS_SUCCESS ? SSV_BM_OK : SSV_FAIL);
}
                                       ...
                                       ssv_gatt_srvc_id_t *service_id, uint16_t num_handle)
{
    ...
    memcpy(&arg.create_srvc.service_id, service_id, sizeof(ssv_gatt_srvc_id_t));
                                        ...
                                        uint8_t srvc_inst_id)
{
    ...
    arg.create_attr_tab.gatts_attr_db = (ssv_gatts_attr_db_t *)gatts_attr_db;
                                 ...
                                 ssv_attr_control_t *control)
{
    ...
    status = ssv_ble_gatts_add_char_desc_param_check(char_val, control);
    if (status != SSV_BM_OK){
        return status;
    }
    ...
    memcpy(&arg.add_char.char_uuid, char_uuid, sizeof(ssv_bt_uuid_t));

    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t), btc_gatts_arg_deep_copy) == BT_STATUS_SUCCESS ? SSV_BM_OK : SSV_FAIL);
}
                                        ...
                                        ssv_attr_control_t *control)
{
    ...
    /* parameter validation check */
    status = ssv_ble_gatts_add_char_desc_param_check(char_descr_val, control);
    if (status != SSV_BM_OK){
        return status;
    }
    ...
    memcpy(&arg.add_descr.descr_uuid, descr_uuid, sizeof(ssv_bt_uuid_t));
                                      ...
                                      uint16_t value_len, uint8_t *value, bool need_confirm)
{
    ...
    /*if (L2CA_CheckIsCongest(L2CAP_ATT_CID, conn_id)) { todo
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t),
                                 btc_gatts_arg_deep_copy) == BT_STATUS_SUCCESS ? SSV_BM_OK : SSV_FAIL);
}
...
ssv_gatt_status_t ssv_ble_gatts_get_attr_value(uint16_t attr_handle, uint16_t *length, const uint8_t **value)
{
    ...
    if (attr_handle == SSV_GATT_ILLEGAL_HANDLE) {
        *length = 0;
...
ssv_err_t ssv_ble_gatts_open(ssv_gatt_if_t gatts_if, ssv_bd_addr_t remote_bda, bool is_direct)
{
    ...
    memcpy(&arg.open.remote_bda, remote_bda, sizeof(ssv_bd_addr_t));
...
ssv_err_t ssv_ble_gatts_send_service_change_indication(ssv_gatt_if_t gatts_if, ssv_bd_addr_t remote_bda)
{
    ...
    if(remote_bda) {
        memcpy(&arg.send_service_change.remote_bda, remote_bda, sizeof(ssv_bd_addr_t));
    } else {
        memset(arg.send_service_change.remote_bda, 0, sizeof(ssv_bd_addr_t));
    }
...
static ssv_err_t ssv_ble_gatts_add_char_desc_param_check(ssv_attr_value_t *char_val, ssv_attr_control_t *control)
{
    if ((control != NULL) && ((control->auto_rsp != SSV_GATT_AUTO_RSP) && (control->auto_rsp != SSV_GATT_RSP_BY_APP))){
        #if 0
            LOG_ERROR("Error in %s, line=%d, control->auto_rsp should be set to SSV_GATT_AUTO_RSP or SSV_GATT_RSP_BY_APP\n",\
                            __func__, __LINE__);
        #endif
            return SSV_ERR_INVALID_ARG;
    }

    if ((control != NULL) && (control->auto_rsp == SSV_GATT_AUTO_RSP)){
        if (char_val == NULL){
