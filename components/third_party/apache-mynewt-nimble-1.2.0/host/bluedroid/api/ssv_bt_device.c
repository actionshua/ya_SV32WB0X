...
{
    /*if (ssv_bluedroid_get_status() != ESP_BLUEDROID_STATUS_ENABLED) {
	return NULL;
    }
...
ssv_err_t ssv_bt_dev_set_device_name(const char *name)
{
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_dev_args_t), NULL) == BT_STATUS_SUCCESS ? SSV_BM_OK : SSV_FAIL);
}
