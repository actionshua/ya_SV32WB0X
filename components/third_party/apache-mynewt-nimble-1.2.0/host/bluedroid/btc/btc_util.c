...
void uuid128_be_to_ssv_uuid(ssv_bt_uuid_t *u, uint8_t* uuid128)
{
    if (memcmp(base_uuid_be+4, uuid128 + 4, 12) != 0) {
        u->len = SSV_UUID_LEN_128;
        uint8_t *p_i = uuid128 + SSV_UUID_LEN_128 - 1;
        uint8_t *p_o = u->uuid.uuid128;
        uint8_t *p_end = p_o + SSV_UUID_LEN_128;
...
{
    ssv_bt_status_t ssv_status = SSV_BT_STATUS_FAIL;
    switch(hci_status) {
        case HCI_SUCCESS:
            ssv_status = SSV_BT_STATUS_SUCCESS;
            break;
        case HCI_ERR_HOST_TIMEOUT:
            ssv_status = SSV_BT_STATUS_TIMEOUT;
            break;
        case HCI_ERR_ILLEGAL_COMMAND:
            ssv_status = SSV_BT_STATUS_PENDING;
            break;
        case HCI_ERR_UNACCEPT_CONN_INTERVAL:
            ssv_status = SSV_BT_STATUS_UNACCEPT_CONN_INTERVAL;
            break;
        case HCI_ERR_PARAM_OUT_OF_RANGE:
            ssv_status = SSV_BT_STATUS_PARAM_OUT_OF_RANGE;
            break;
        case HCI_ERR_ILLEGAL_PARAMETER_FMT:
            ssv_status = SSV_BT_STATUS_ERR_ILLEGAL_PARAMETER_FMT;
            break;
        default:
            ssv_status = SSV_BT_STATUS_FAIL;
            break;
    }
...
ssv_bt_status_t btc_btm_status_to_ssv_status (uint8_t btm_status)
{
    ...
    switch(btm_status){
        case BTM_SUCCESS:
            ...
            break;
        case BTM_BUSY:
            ssv_status = SSV_BT_STATUS_BUSY;
            break;
        case BTM_NO_RESOURCES:
            ssv_status = SSV_BT_STATUS_NOMEM;
            break;
        case BTM_ERR_PROCESSING:
            ssv_status = SSV_BT_STATUS_PENDING;
            break;
        case BTM_PEER_LE_DATA_LEN_UNSUPPORTED:
            ssv_status = SSV_BT_STATUS_PEER_LE_DATA_LEN_UNSUPPORTED;
            break;
        case BTM_CONTROL_LE_DATA_LEN_UNSUPPORTED:
            ssv_status = SSV_BT_STATUS_CONTROL_LE_DATA_LEN_UNSUPPORTED;
            break;
...
ssv_bt_status_t btc_bta_status_to_ssv_status (uint8_t bta_status)
{
    ...
    switch(bta_status){
        ...
        case BTA_NO_RESOURCES:
            ...
            break;
        case BTA_WRONG_MODE:
            ssv_status = SSV_BT_STATUS_NOT_READY;
            break;
        case BTA_EIR_TOO_LARGE:
            ssv_status = SSV_BT_STATUS_EIR_TOO_LARGE;
            break;
        default:
            ssv_status = SSV_BT_STATUS_FAIL;
            break;
