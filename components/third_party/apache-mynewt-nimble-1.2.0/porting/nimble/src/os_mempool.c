...
struct os_memblock *fblock = NULL;/*first memory buffer for ble_hci_uart_acl_pool*/
...
os_memblock_get(struct os_mempool *mp)
{
    ...
    if (mp) {
        ...
        /* Check for any free */
		//todo werner improve acl rx buffer
		if ((strcmp(mp->name, "ble_hci_uart_acl_pool") == 0) && ((mp->mp_num_blocks - 1) == mp->mp_num_free)) {
			fblock = SLIST_FIRST(mp);
		}

		if ((strcmp(mp->name, "ble_hci_uart_acl_pool") == 0) && (!mp->mp_num_free)) {
			SLIST_NEXT(fblock, mb_next) = SLIST_FIRST(mp);
    		SLIST_FIRST(mp) = fblock;
			mp->mp_num_free++;
		}
	...
	if (block == NULL)
        printf("%s end omp->omp_pool == NULL\n", __func__);
...
os_memblock_get(struct os_mempool *mp)
{
    ...
    //if (block == NULL)//werner debug, always show
	//	printf("%s block == NULL %s\n", __func__, mp->name);
    return (void *)block;
}
