 ...
 *
 * Log library stores all tags provided to ssv_log_level_set as a linked
...
//print number of bytes per line for ssv_log_buffer_char and ssv_log_buffer_hex
...
typedef struct uncached_tag_entry_{
    ...
    uint8_t level;  // ssv_log_level_t as uint8_t
    char tag[0];    // beginning of a zero-terminated string
} uncached_tag_entry_t;

static ssv_log_level_t s_log_default_level = SSV_LOG_VERBOSE;
...
static inline bool get_cached_log_level(const char* tag, ssv_log_level_t* level);
static inline bool get_uncached_log_level(const char* tag, ssv_log_level_t* level);
static inline void add_to_cache(const char* tag, ssv_log_level_t level);
static void heap_bubble_down(int index);
static inline void heap_swap(int i, int j);
static inline bool should_output(ssv_log_level_t level_for_message, ssv_log_level_t level_for_tag);
static inline void clear_log_level_list();

uint8_t g_loglevelapp = LOG_LOCAL_LEVEL;
uint8_t g_loglevelcore = MESH_LOG_LEVEL;
...
void ssv_log_write(ssv_log_level_t level,
        const char* tag,
        const char* format, ...)
{
    char buffer[256];
    ...
    printf("%s\n", buffer);
    //(*s_log_print_func)(format, list);// s_log_print_func is vprintf
    va_end(list);
...
static inline bool get_cached_log_level(const char* tag, ssv_log_level_t* level)
{
    ...
    *level = (ssv_log_level_t) s_log_cache[i].level;
...
uint32_t ssv_log_early_timestamp()
{
    return 0;//xthal_get_ccount() / (g_ticks_per_us_pro * 1000);
}

#ifndef BOOTLOADER_BUILD

uint32_t ssv_log_timestamp()
{
    /*if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) {
        return ssv_log_early_timestamp();
    }
    static uint32_t base = 0;
    if (base == 0 && xPortGetCoreID() == 0) {
        base = ssv_log_early_timestamp();
    }
...
uint32_t ssv_log_timestamp() __attribute__((alias("ssv_log_early_timestamp")));

#endif //BOOTLOADER_BUILD

void ssv_log_buffer_hex_internal(const char *tag, const void *buffer, uint16_t buff_len,
                        ssv_log_level_t log_level)
{
    ...
    do {
        ...
        /*if ( !ssv_ptr_byte_accessible(buffer) ) {//--
        ...
        SSV_LOG_LEVEL( log_level, tag, "%s", hex_buffer );
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while( buff_len );
}

void ssv_log_buffer_char_internal(const char *tag, const void *buffer, uint16_t buff_len,
                            ssv_log_level_t log_level)
{
    if ( buff_len == 0 ) return;
    char temp_buffer[BYTES_PER_LINE+3];   //for not-byte-accessible memory
    ...
    do {
        ...
        SSV_LOG_LEVEL( log_level, tag, "%s", char_buffer );
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while( buff_len );
}
...
void ssv_log_buffer_hexdump_internal( const char *tag, const void *buffer, uint16_t buff_len, ssv_log_level_t log_level)
{
    ...
    do {
        ...
        SSV_LOG_LEVEL( log_level, tag, "%s", hd_buffer );
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while( buff_len );
}
