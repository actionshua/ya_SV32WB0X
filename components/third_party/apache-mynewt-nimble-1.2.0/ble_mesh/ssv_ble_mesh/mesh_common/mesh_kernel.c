...
s64_t k_uptime_get(void)
{
     ...
     * but ssv_timer_get_time is in microseconds
     */
    return (ssv_timer_get_time() / 1000);
}
...
u32_t k_uptime_get_32(void)
{
     ...
     * but ssv_timer_get_time is in microseconds
     */
    return (u32_t)(ssv_timer_get_time() / 1000);
}

void k_sleep(s32_t duration)
{
    //vTaskDelay(duration / portTICK_PERIOD_MS); no use
    OS_MsDelay(duration);
    return;
}
