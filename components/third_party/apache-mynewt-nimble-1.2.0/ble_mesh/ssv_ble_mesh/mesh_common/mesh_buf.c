...
#endif
{
    ...
    while (buf) {
        ...
        //NET_BUF_INFO("f %d", pool->avail_count);
...
void output_pool_detail(struct net_buf_pool *pool)
{
    u16 idx;

    NET_BUF_ERR("pool address:%p", pool);
    NET_BUF_ERR("buf size:%d, uninit size:%d", pool->buf_count, pool->uninit_count);

    for(idx = 0; idx < pool->buf_count; idx++)
    {
        NET_BUF_ERR("%d ref:%d size:%d", idx, pool->__bufs->ref, pool->__bufs->len);
    }
