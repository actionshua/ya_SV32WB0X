...
#include "ssv_fast_prov_client_model.h"
...
static example_node_info_t nodes_info[CONFIG_BLE_MESH_MAX_PROV_NODES] = {
    [0 ... (CONFIG_BLE_MESH_MAX_PROV_NODES - 1)] = {
        .reprov       = false,
        .unicast_addr = SSV_BLE_MESH_ADDR_UNASSIGNED,
        .element_num  = 0x0,
        .net_idx      = SSV_BLE_MESH_KEY_UNUSED,
        .app_idx      = SSV_BLE_MESH_KEY_UNUSED,
        .onoff        = LED_OFF,
        .lack_of_addr = false,
        .unicast_min  = SSV_BLE_MESH_ADDR_UNASSIGNED,
        .unicast_max  = SSV_BLE_MESH_ADDR_UNASSIGNED,
        .flags        = 0x0,
        .iv_index     = 0x0,
        .group_addr   = SSV_BLE_MESH_ADDR_UNASSIGNED,
        .prov_addr    = SSV_BLE_MESH_ADDR_UNASSIGNED,
        .match_len    = 0,
                                  ...
                                  uint16_t app_idx, uint8_t onoff)
{
    ...
    if (!uuid || !SSV_BLE_MESH_ADDR_IS_UNICAST(node_addr) || !elem_num) {
    ...
    for (int i = 0; i < ARRAY_SIZE(nodes_info); i++) {
        ...
        if (!memcmp(node->uuid, uuid, 16)) {
            SSV_LOGW(TAG, "%s: reprovisioned node", __func__);
            node->reprov = true;
    ...
    for (int i = 0; i < ARRAY_SIZE(nodes_info); i++) {
        node = &nodes_info[i];
        if (node->unicast_addr == SSV_BLE_MESH_ADDR_UNASSIGNED) {
    ...
    SSV_LOGE(TAG, "%s: nodes_info is full", __func__);
    return SSV_FAIL;
}
...
bool example_is_node_exist(const uint8_t uuid[16])
{
    ...
    for (int i = 0; i < ARRAY_SIZE(nodes_info); i++) {
        node = &nodes_info[i];
        if (SSV_BLE_MESH_ADDR_IS_UNICAST(node->unicast_addr)) {
            if (!memcmp(node->uuid, uuid, 16)) {
                return true;
            }
        }
...
ssv_ble_mesh_model_t *example_find_model(uint16_t element_addr, uint16_t model_id,
        uint16_t company_id)
{
    ssv_ble_mesh_elem_t *element = NULL;

    if (!SSV_BLE_MESH_ADDR_IS_UNICAST(element_addr)) {
        return NULL;
    }

    element = ssv_ble_mesh_find_element(element_addr);
    if (!element) {
        return NULL;
    }

    if (company_id == CID_NVAL) {
        return ssv_ble_mesh_find_sig_model(element, model_id);
    } else {
        return ssv_ble_mesh_find_vendor_model(element, company_id, model_id);
    }
}

static ssv_err_t example_set_app_idx_to_user_data(uint16_t app_idx)
{
    example_fast_prov_server_t *srv_data = NULL;
    ssv_ble_mesh_model_t *srv_model = NULL;

    srv_model = example_find_model(ssv_ble_mesh_get_primary_element_address(),
                                   SSV_BLE_MESH_VND_MODEL_ID_FAST_PROV_SRV, CID_ESP);
    if (!srv_model) {
        return SSV_FAIL;
    }
...
{
    const ssv_ble_mesh_comp_t *comp = NULL;
    ssv_ble_mesh_elem_t *element = NULL;
    ssv_ble_mesh_model_t *model = NULL;
    int i, j, k;

    comp = ssv_ble_mesh_get_composition_data();
    if (!comp) {
        return SSV_FAIL;
    }
    ...
    for (i = 0; i < comp->element_count; i++) {
        ...
        for (j = 0; j < element->sig_model_count; j++) {
            ...
            if (model->model_id == SSV_BLE_MESH_MODEL_ID_CONFIG_SRV ||
                    model->model_id == SSV_BLE_MESH_MODEL_ID_CONFIG_CLI) {
                continue;
            }
            ...
            for (k = 0; k < ARRAY_SIZE(model->keys); k++) {
                if (model->keys[k] == SSV_BLE_MESH_KEY_UNUSED) {
                    model->keys[k] = app_idx;
                    break;
                }
            }
            if (k == ARRAY_SIZE(model->keys)) {
                SSV_LOGE(TAG, "%s: SIG model (model_id 0x%04x) is full of AppKey",
        ...
        for (j = 0; j < element->vnd_model_count; j++) {
            ...
            if (k == ARRAY_SIZE(model->keys)) {
                SSV_LOGE(TAG, "%s: Vendor model (model_id 0x%04x, cid: 0x%04x) is full of AppKey",
...
ssv_err_t example_add_fast_prov_group_address(uint16_t model_id, uint16_t group_addr)
{
    ...
    for (i = 0; i < comp->element_count; i++) {
        element = &comp->elements[i];
        model = ssv_ble_mesh_find_sig_model(element, model_id);
        if (!model) {
            continue;
        }
        ...
        if (j != ARRAY_SIZE(model->groups)) {
            SSV_LOGW(TAG, "%s: Group address already exists, element index: %d", __func__, i);
            continue;
        }
        for (j = 0; j < ARRAY_SIZE(model->groups); j++) {
            if (model->groups[j] == SSV_BLE_MESH_ADDR_UNASSIGNED) {
                model->groups[j] = group_addr;
                break;
            }
        }
        if (j == ARRAY_SIZE(model->groups)) {
            SSV_LOGE(TAG, "%s: Model is full of group address, element index: %d", __func__, i);
        }
    }
...
ssv_err_t example_delete_fast_prov_group_address(uint16_t model_id, uint16_t group_addr)
{
    ...
    for (i = 0; i < comp->element_count; i++) {
        ...
        for (j = 0; j < ARRAY_SIZE(model->groups); j++) {
            if (model->groups[j] == group_addr) {
                model->groups[j] = SSV_BLE_MESH_ADDR_UNASSIGNED;
                break;
            }
        }
    }
...
{
    ssv_ble_mesh_client_common_param_t common = {0};
    ssv_ble_mesh_cfg_client_set_state_t set = {0};
    ...
    if (add_key) {
        ...
        key = ssv_ble_mesh_get_fast_prov_app_key(info->net_idx, info->app_idx);
#endif
        if (!key) {
            return SSV_FAIL;
        }
    ...
    common.opcode       = SSV_BLE_MESH_MODEL_OP_APP_KEY_ADD;
    common.model        = model;
    ...
    return ssv_ble_mesh_config_client_set_state(&common, &set);
}

ssv_err_t example_send_generic_onoff_get(ssv_ble_mesh_model_t *model,
        example_msg_common_info_t *info)
{
    ssv_ble_mesh_generic_client_get_state_t get = {0};
    ssv_ble_mesh_client_common_param_t common = {0};
    ...
    common.opcode       = SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_GET;
    common.model        = model;
    common.ctx.net_idx  = info->net_idx;
    ...
    return ssv_ble_mesh_generic_client_get_state(&common, &get);
}
...
{
    ssv_ble_mesh_generic_client_set_state_t set = {0};
    ssv_ble_mesh_client_common_param_t common = {0};
    ...
    if (need_ack) {
        common.opcode = SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_SET;
    } else {
        common.opcode = SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK;
    }
    common.model        = model;
    common.ctx.net_idx  = info->net_idx;
    common.ctx.app_idx  = info->app_idx;
    ...
    return ssv_ble_mesh_generic_client_set_state(&common, &set);
}
        ...
        example_fast_prov_info_set_t *set)
{
    ...
    SSV_LOGI(TAG, "min:       0x%04x, max:        0x%04x", set->unicast_min, set->unicast_max);
    SSV_LOGI(TAG, "flags:     0x%02x,   iv_index:   0x%08x", set->flags, set->iv_index);
    SSV_LOGI(TAG, "net_idx:   0x%04x, group_addr: 0x%04x", set->net_idx, set->group_addr);
    SSV_LOGI(TAG, "action:    0x%02x", set->action);
    SSV_LOG_BUFFER_HEX("FAST_PROV_OP: match_val", set->match_val, set->match_len);
    ...
    err = ssv_ble_mesh_client_model_send_msg(model, &ctx,
            SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_SET,
        ...
        uint8_t net_key[16])
{
    ...
    return ssv_ble_mesh_client_model_send_msg(model, &ctx,
            SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NET_KEY_ADD,
        ...
        struct net_buf_simple *node_addr)
{
    ...
    SSV_LOG_BUFFER_HEX("Send node address", node_addr->data, node_addr->len);
    ...
    return ssv_ble_mesh_client_model_send_msg(model, &ctx,
            SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR,
        ...
        example_msg_common_info_t *info)
{
    ...
    return ssv_ble_mesh_client_model_send_msg(model, &ctx,
            SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_GET,
        ...
        uint32_t opcode, struct net_buf_simple *msg)
{
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_STATUS:
        ...
        break;
    default:
        SSV_LOGW(TAG, "%s: Invalid fast prov status opcode 0x%04x", __func__, opcode);
        return SSV_FAIL;
    }

    return ssv_ble_mesh_server_model_send_msg(model, ctx, opcode, msg ? msg->len : 0, msg ? msg->data : NULL);
}