...
void example_send_self_prov_node_addr(struct k_work *work)
{
    ...
    model = example_find_model(ssv_ble_mesh_get_primary_element_address(),
                               SSV_BLE_MESH_VND_MODEL_ID_FAST_PROV_CLI, CID_ESP);
    if (!model) {
        SSV_LOGE(TAG, "%s: Failed to get fast prov client model", __func__);
        return;
    }

    if (node_addr_send.send_succeed == true && node_addr_send.ack_received == false) {
        SSV_LOGW(TAG, "%s: Previous node address message is being sent", __func__);
        return;
    }
    ...
    if (node_addr_send.send_succeed == true) {
        ...
        for (i = 0; i < CONFIG_BLE_MESH_MAX_PROV_NODES; i++) {
            ...
            if (!SSV_BLE_MESH_ADDR_IS_UNICAST(addr)) {
                continue;
            }
            for (j = 0; j < ARRAY_SIZE(addr_already_sent); j++) {
                if (addr == addr_already_sent[j]) {
                    SSV_LOGW(TAG, "%s: node addr 0x%04x has already been sent", __func__, addr);
                    break;
                }
            }
    ...
    if (node_addr_send.addr->len) {
        ...
        if (err != SSV_BM_OK) {
            SSV_LOGE(TAG, "%s: Failed to send node address", __func__);
...
static void example_get_all_node_addr(struct k_work *work)
{
    ...
    if (!node) {
        SSV_LOGE(TAG, "%s: Failed to get node info", __func__);
        return;
    }
    ...
    model = example_find_model(ssv_ble_mesh_get_primary_element_address(),
                               SSV_BLE_MESH_VND_MODEL_ID_FAST_PROV_CLI, CID_ESP);
    if (!model) {
        SSV_LOGE(TAG, "%s: Failed to get model info", __func__);
        return;
    }
    ...
    if (err != SSV_BM_OK) {
        SSV_LOGE(TAG, "%s: Failed to send Fast Prov Node Address Get message", __func__);
        return;
    }
}
#endif
        ...
        ssv_ble_mesh_msg_ctx_t *ctx)
{
    ...
    SSV_LOGW(TAG, "%s: Receive fast prov server status timeout", __func__);
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_SET: {
        ...
        if (err != SSV_BM_OK) {
            SSV_LOGE(TAG, "%s: Failed to send Fast Prov Info Set message", __func__);
            return SSV_FAIL;
        }
        break;
    }
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR:
        if (node_addr_send.addr->len) {
            ...
            err = example_send_fast_prov_self_prov_node_addr(model, &info, node_addr_send.addr);
            if (err != SSV_BM_OK) {
                SSV_LOGE(TAG, "%s: Failed to send Fast Prov Node Addr message", __func__);
                node_addr_send.send_succeed = false;
            } else {
                node_addr_send.send_succeed = true;
            }
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_GET: {
        ...
        err = example_send_fast_prov_all_node_addr_get(model, &info);
        if (err != SSV_BM_OK) {
            SSV_LOGE(TAG, "%s: Failed to send Fast Prov Node Address message", __func__);
            return SSV_FAIL;
        }
        break;
    }
        ...
        uint16_t len, const uint8_t *data)
{
    ...
    SSV_LOG_BUFFER_HEX("fast prov client receives", data, len);

    switch (ctx->recv_op) {
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_STATUS:
        SSV_LOG_BUFFER_HEX("fast prov info status", data, len);
#if !defined(CONFIG_BLE_MESH_FAST_PROV)
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NET_KEY_STATUS:
        SSV_LOGI(TAG, "status_key: 0x%02x, status_act: 0x%02x", data[0], data[1]);
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_ACK: {
        ...
        for (i = 0; i < (length >> 1); i++) {
            ...
            if (SSV_BLE_MESH_ADDR_IS_UNICAST(addr)) {
                ...
                for (j = 0; j < ARRAY_SIZE(addr_already_sent); j++) {
                    if (addr_already_sent[j] == SSV_BLE_MESH_ADDR_UNASSIGNED) {
                        addr_already_sent[j] = addr;
                        break;
                    }
                }
                if (j == ARRAY_SIZE(addr_already_sent)) {
                    SSV_LOGE(TAG, "%s: No place to store the sent node address", __func__);
                    return SSV_FAIL;
                }
            }
        }
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_STATUS: {
        SSV_LOG_BUFFER_HEX("Node address", data, len);
        ssv_ble_mesh_model_t *cli_model = NULL;
        example_node_info_t *node = NULL;
        ssv_err_t err;
        node = example_get_node_info(prim_prov_addr);
        if (!node) {
        ...
        cli_model = example_find_model(ssv_ble_mesh_get_primary_element_address(),
                                       SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_CLI, CID_NVAL);
        if (!cli_model) {
            SSV_LOGE(TAG, "%s: Failed to get Generic OnOff Client Model info", __func__);
            return SSV_FAIL;
        }
        example_msg_common_info_t info = {
            .net_idx = node->net_idx,
        ...
        if (err != SSV_BM_OK) {
            SSV_LOGE(TAG, "%s: Failed to send Generic OnOff Set Unack message", __func__);
            return SSV_FAIL;
        }
        break;
    }
    default:
        SSV_LOGE(TAG, "%s: Invalid fast prov status opcode", __func__);
        return SSV_FAIL;
    }
