...
#include "ssv_ble_mesh_proxy_api.h"
#include "ssv_ble_mesh_config_model_api.h"

#include "ssv_fast_prov_operation.h"
#include "ssv_fast_prov_server_model.h"
...
{
    if (!SSV_BLE_MESH_ADDR_IS_UNICAST(node_addr)) {
        SSV_LOGE(TAG, "%s: Not a unicast address", __func__);
        return SSV_ERR_INVALID_ARG;
    }

    for (int i = 0; i < ARRAY_SIZE(all_node_addr); i++) {
        if (all_node_addr[i] == node_addr) {
            SSV_LOGW(TAG, "%s: Node address 0x%04x already exists", __func__, node_addr);
            return SSV_BM_OK;
        }
    }

    for (int i = 0; i < ARRAY_SIZE(all_node_addr); i++) {
        if (all_node_addr[i] == SSV_BLE_MESH_ADDR_UNASSIGNED) {
    ...
    SSV_LOGE(TAG, "%s: remote node address queue is full", __func__);
    return SSV_FAIL;
}

ssv_ble_mesh_cfg_srv_t *get_cfg_srv_user_data(void)
{
    ssv_ble_mesh_model_t *model = NULL;

    model = example_find_model(ssv_ble_mesh_get_primary_element_address(),
                               SSV_BLE_MESH_MODEL_ID_CONFIG_SRV, CID_NVAL);
    if (!model) {
        SSV_LOGE(TAG, "%s: Failed to get config server model", __func__);
        return NULL;
    }

    return (ssv_ble_mesh_cfg_srv_t *)(model->user_data);
}
...
static void disable_fast_prov_cb(struct k_work *work)
{
    ...
    if (!srv) {
        SSV_LOGE(TAG, "%s: Failed to get fast prov server model user_data", __func__);
        return;
    }

    if (ssv_ble_mesh_set_fast_prov_action(FAST_PROV_ACT_SUSPEND)) {
        SSV_LOGE(TAG, "%s: Failed to disable fast provisioning", __func__);
        return;
    }
}
...
static void enable_gatt_proxy_cb(struct k_work *work)
{
    ...
    if (bt_mesh_atomic_test_and_clear_bit(srv->srv_flags, RELAY_PROXY_DISABLED)) {
        SSV_LOGI(TAG, "%s: Enable BLE Mesh Relay & GATT Proxy", __func__);
        /* For Primary Provisioner, Relay will not be enabled */
        ssv_ble_mesh_proxy_gatt_enable();
        ssv_ble_mesh_proxy_identity_enable();
    }
        ...
        ssv_ble_mesh_msg_ctx_t *ctx, struct net_buf_simple *buf)
{
    ...
    SSV_LOG_BUFFER_HEX("fast prov server recv", buf->data, buf->len);

    switch (ctx->recv_op) {
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_SET: {
        ...
        opcode = SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_STATUS;

        if (srv->set_info) {
            SSV_LOGW(TAG, "%s: Set fast prov info is already in progress", __func__);
        ...
        uint16_t unicast_min   = (ctx_flags & BIT(1)) ? net_buf_simple_pull_le16(buf) : (ssv_ble_mesh_get_primary_element_address() + ssv_ble_mesh_get_element_count());
        ...
        uint16_t group_addr    = (ctx_flags & BIT(6)) ? net_buf_simple_pull_le16(buf) : SSV_BLE_MESH_ADDR_UNASSIGNED;
        uint16_t pri_prov_addr = (ctx_flags & BIT(7)) ? net_buf_simple_pull_le16(buf) : SSV_BLE_MESH_ADDR_UNASSIGNED;
        if (ctx_flags & BIT(8)) {
        ...
        if ((ctx_flags & BIT(1)) || (ctx_flags & BIT(2))) {
            if (!SSV_BLE_MESH_ADDR_IS_UNICAST(unicast_min) || !SSV_BLE_MESH_ADDR_IS_UNICAST(unicast_max)) {
                status_unicast = 0x01;  /* not a unicast address */
            } else if (unicast_min > unicast_max) {
                status_unicast = 0x02;  /* min bigger than max */
            } else if (unicast_min < (ssv_ble_mesh_get_primary_element_address() + ssv_ble_mesh_get_element_count())) {
        ...
        if (ctx_flags & BIT(6)) {
            if (!SSV_BLE_MESH_ADDR_IS_GROUP(group_addr)) {
                net_buf_simple_add_le16(msg, BIT(3));
                net_buf_simple_add_u8(msg, 0x01);   /* not a group address */
                break;
            }
            err = example_add_fast_prov_group_address(SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV, group_addr);
            if (err != SSV_BM_OK) {
                SSV_LOGE(TAG, "%s: Failed to add group address 0x%04x", __func__, group_addr);
                net_buf_simple_add_le16(msg, BIT(3));
        ...
        if (ctx_flags & BIT(7)) {
            if (!SSV_BLE_MESH_ADDR_IS_UNICAST(pri_prov_addr)) {
        ...
        memcpy(&srv->ctx, ctx, sizeof(ssv_ble_mesh_msg_ctx_t));
        srv->set_info = osi_calloc(sizeof(struct fast_prov_info_set));
        if (!srv->set_info) {
            SSV_LOGE(TAG, "%s: Failed to allocate memory", __func__);
        ...
        ssv_ble_mesh_fast_prov_info_t info_set = {
        ...
        err = ssv_ble_mesh_set_fast_prov_info(&info_set);
        if (err != SSV_BM_OK) {
            SSV_LOGE(TAG, "%s: Failed to set fast prov info", __func__);
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NET_KEY_ADD: {
        uint8_t status_net_key, status_action;

        msg = bt_mesh_alloc_buf(2);

        if (srv->state == STATE_PEND) {
            /* Add fast prov net_key, wait for event callback and call ssv_ble_mesh_set_fast_prov_act() to set action */
        } else {
        ...
        opcode = SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NET_KEY_STATUS;
        net_buf_simple_add_u8(msg, status_net_key);
        net_buf_simple_add_u8(msg, status_action);
        break;
    }
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR: {
        if (buf->len % 2) {
            SSV_LOGE(TAG, "%s: Invalid Fast Prov Node Addr message length", __func__);
            return SSV_FAIL;
        }
        ...
        for (; buf->len; ) {
            ...
            SSV_LOGI(TAG, "Node address: 0x%04x", node_addr);
            err = example_store_remote_node_address(node_addr);
            if (err != SSV_BM_OK) {
                SSV_LOGE(TAG, "%s: Failed to store node address 0x%04x", __func__, node_addr);
            }
        }

        opcode = SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_ACK;
        break;
    }
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_GET: {
        ...
        SSV_LOG_BUFFER_HEX("All node address", msg->data, msg->len);

        opcode = SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_STATUS;
        break;
    }
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_GROUP_ADD: {
        uint16_t own_addr = ssv_ble_mesh_get_primary_element_address();
        uint16_t group_addr = net_buf_simple_pull_le16(buf);
        SSV_LOGI(TAG, "%s, group address 0x%04x", __func__, group_addr);
        if (!SSV_BLE_MESH_ADDR_IS_GROUP(group_addr)) {
            return SSV_FAIL;
        }
        for (; buf->len; ) {
            uint16_t dst = net_buf_simple_pull_le16(buf);
            SSV_LOGI(TAG, "%s, dst 0x%04x, own address 0x%04x", __func__, dst, own_addr);
            if (dst == own_addr) {
                err = example_add_fast_prov_group_address(SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV, group_addr);
                if (err != SSV_BM_OK) {
                    SSV_LOGE(TAG, "%s: Failed to add group address 0x%04x", __func__, group_addr);
    ...
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_GROUP_DELETE: {
        uint16_t own_addr = ssv_ble_mesh_get_primary_element_address();
        uint16_t group_addr = net_buf_simple_pull_le16(buf);
        SSV_LOGI(TAG, "%s, group address 0x%04x", __func__, group_addr);
        if (!SSV_BLE_MESH_ADDR_IS_GROUP(group_addr)) {
        ...
        for (; buf->len; ) {
            ...
            SSV_LOGI(TAG, "%s, dst 0x%04x, own address 0x%04x", __func__, dst, own_addr);
            if (dst == own_addr) {
                err = example_delete_fast_prov_group_address(SSV_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV, group_addr);
                if (err != SSV_BM_OK) {
                    SSV_LOGE(TAG, "%s: Failed to delete group address 0x%04x", __func__, group_addr);
                }
                return err;
            }
        }
    ...
    default:
        SSV_LOGW(TAG, "%s: Not a Fast Prov Client message opcode", __func__);
        return SSV_FAIL;
    }

    err = example_send_fast_prov_status_msg(model, ctx, opcode, msg);
    if (err != SSV_BM_OK) {
        SSV_LOGE(TAG, "%s: Failed to send Fast Prov Status message", __func__);
        ...
        uint8_t status_net_idx, uint8_t status_match)
{
    ...
    if (srv->set_info->action & BIT(7)) {
        ...
        srv->prim_prov_addr = ssv_ble_mesh_get_primary_element_address();
    ...
    if (status_net_idx) {
        SSV_LOGW(TAG, "%s: Wait for fast prov netkey to be added", __func__);
    ...
    err = ssv_ble_mesh_set_fast_prov_action(FAST_PROV_ACT_ENTER);
    if (err == SSV_BM_OK) {
        bt_mesh_free_buf(msg);
        return SSV_BM_OK;
    }

    SSV_LOGE(TAG, "%s: Failed to set fast prov action", __func__);
    net_buf_simple_add_le16(msg, BIT(6));
    net_buf_simple_add_u8(msg, 0x01);   /* action failed */

send:
    err = example_send_fast_prov_status_msg(model, &srv->ctx, SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_STATUS, msg);
    if (err != SSV_BM_OK) {
        SSV_LOGE(TAG, "%s: Failed to send Fast Prov Status message", __func__);
        example_free_set_info(srv);
    }
        ...
        ssv_ble_mesh_model_t *model, ssv_ble_mesh_msg_ctx_t *ctx)
{
    ...
    SSV_LOGI(TAG, "%s: opcode 0x%06x", __func__, opcode);

    switch (opcode) {
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_STATUS:
        if (err_code == 0 && srv->set_info && srv->set_info->set_succeed == true) {
            if (!bt_mesh_atomic_test_and_set_bit(srv->srv_flags, RELAY_PROXY_DISABLED)) {
                 ...
                 */
                SSV_LOGW(TAG, "%s: Disable BLE Mesh Relay & GATT Proxy", __func__);
                ssv_ble_mesh_proxy_gatt_disable();
            }
        }
        example_free_set_info(srv);
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NET_KEY_STATUS:
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_ACK:
        if (!bt_mesh_atomic_test_and_set_bit(srv->srv_flags, GATT_PROXY_ENABLE_START)) {
            k_delayed_work_submit(&srv->gatt_proxy_enable_timer, GATT_PROXY_ENABLE_TIMEOUT);
        }
        break;
    case SSV_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_STATUS:
        break;
    default:
        break;
    }
