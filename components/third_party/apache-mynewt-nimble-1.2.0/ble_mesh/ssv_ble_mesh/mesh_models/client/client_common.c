    ...
    struct net_buf_simple *buf, bool need_pub)
{
    ...
    if (!BLE_MESH_ADDR_IS_UNICAST(ctx->recv_dst)) {
        BT_INFO("Unexpected status message 0x%x", ctx->recv_op);
    ...
    if (!data) {
        BT_INFO("%s, Client internal_data is NULL", __func__);
        return NULL;
    }

    if ((node = bt_mesh_client_pick_node(&data->queue, ctx->addr)) == NULL) {
        BT_DBG("Unexpected status message 0x%x, pick_node", ctx->recv_op);
        if (cli->publish_status && need_pub) {
            cli->publish_status(ctx->recv_op, model, ctx, buf);
        }
        return NULL;
    ...
    if (node->op_pending != ctx->recv_op) {
        BT_INFO("Unexpected status message 0x%x, op_pending", ctx->recv_op);
        if (cli->publish_status && need_pub) {
            cli->publish_status(ctx->recv_op, model, ctx, buf);
        }
        return NULL;
    ...
    if (k_delayed_work_remaining_get(&node->timer) == 0) {
        BT_INFO("Unexpected status message 0x%x, remaining_get", ctx->recv_op);
        if (cli->publish_status && need_pub) {
            cli->publish_status(ctx->recv_op, model, ctx, buf);
        }
        return NULL;
...
#define BITMAP_DOWNGRADE_COUNT 3
extern struct ble_mesh_sync_bitmap g_st_acked_bitmap;
extern struct ble_mesh_sync_bitmap g_st_acked_golden_bitmap;
extern struct ble_mesh_sync_bitmap g_st_acked_low_bitmap_value;

u8_t g_acked_low_bitmap_same = 0;

int bitcount(u32_t n)
{
   int count = 0 ;
   while (n)
   {
...
int bitmap_bitcount(struct ble_mesh_sync_bitmap bitmap)
{
   int count = 0;
   count = bitcount(bitmap.low) + bitcount(bitmap.high);

   return count;
}

void judge_golden_bitmap()
{
    if (golden_bitmap_match()) {
        g_acked_low_bitmap_same = 0;
    } else if (bitmap_bitcount(g_st_acked_bitmap)  < bitmap_bitcount(g_st_acked_golden_bitmap)){
        /* lose some device's ack*/
        if (is_bitmap_the_same(g_st_acked_low_bitmap_value, g_st_acked_bitmap))
            g_acked_low_bitmap_same++;
        else
            assign_bitmap(&g_st_acked_low_bitmap_value, g_st_acked_bitmap);

        if (g_acked_low_bitmap_same >= BITMAP_DOWNGRADE_COUNT) {
            assign_bitmap(&g_st_acked_golden_bitmap, g_st_acked_bitmap);
            g_acked_low_bitmap_same = 0;
            printf("bt down %08x %08x\n", g_st_acked_golden_bitmap.high, g_st_acked_golden_bitmap.low);
        }
    } else {
        or_bitmap(&g_st_acked_golden_bitmap, g_st_acked_bitmap);
        printf("bt up %08x %08x\n", g_st_acked_golden_bitmap.high, g_st_acked_golden_bitmap.low);
    }
}
                            ...
                            void *cb_data)
{
    ...
    if (!need_ack) {
        ...
        return bt_mesh_model_send_ssv(opcode, model, ctx, msg, cb, cb_data);
    }
    ...
    if (bt_mesh_client_check_node_in_list(&internal->queue, ctx->addr)) {
        ...
        if ((err = bt_mesh_model_send_ssv(opcode, model, ctx, msg, cb, cb_data)) != 0) {
            BT_INFO("%s, bt_mesh_model_send err %d", __func__, err);
            osi_free(node);
        } else {
            /*need ack msg will go here after send*/
            bt_mesh_list_lock();
