...
static int net_set(const char *name)
{
    ...
    BT_INFO("Recovered DevKey %s", bt_hex(bt_mesh.dev_key, 16));
...
static int seq_set_bak(const char *name)
{
    struct seq_val seq;
    bool exist;
    int err;
    ...
    BT_INFO("%s", __func__);
    if (bt_mesh.seq)
        return 0;/*load /mesh/seq ok before*/

    /*load /mesh/seq/bak information*/
    err = bt_mesh_load_core_settings(name, (u8_t *)&seq, sizeof(seq), &exist);
    if (err) {
    ...
    if (CONFIG_BLE_MESH_SEQ_STORE_RATE > 0) {
         ...
         * to the settings storage.
         */
        bt_mesh.seq += (2*CONFIG_BLE_MESH_SEQ_STORE_RATE -
                        (bt_mesh.seq % CONFIG_BLE_MESH_SEQ_STORE_RATE));
        bt_mesh.seq--;
    }

    BT_INFO("Bak Sequence Number 0x%06x", bt_mesh.seq);
...
static int rpl_set(const char *name)
{
    ...
    for (i = 0; i < length / SETTINGS_ITEM_SIZE; i++) {
        ...
        BT_INFO("RPL 0x%04x: Seq 0x%06x, old_iv %u", src, rpl.seq, rpl.old_iv);
        entry->src = src;
        entry->seq = rpl.seq;
...
static int net_key_set(const char *name)
{
    ...
    for (i = 0; i < length / SETTINGS_ITEM_SIZE; i++) {
        ...
        BT_INFO("NetKeyIndex 0x%03x recovered from storage", net_idx);
        BT_INFO("key 0, %s", bt_hex(&key.val[0], 16));
        BT_INFO("key 1, %s", bt_hex(&key.val[1], 16));
        sub->net_idx = net_idx;
...
static int app_key_set(const char *name)
{
    ...
    for (i = 0; i < length / SETTINGS_ITEM_SIZE; i++) {
        ...
        BT_INFO("AppKeyIndex 0x%03x recovered from storage", app_idx);
        BT_INFO("key 0, %s", bt_hex(key.val[0], 16));
        BT_INFO("key 1, %s", bt_hex(key.val[1], 16));
...
static int hb_pub_set(const char *name)
{
    ...
    BT_INFO("Restore Heartbeat Publication");
...
static int cfg_set(const char *name)
{
    ...
    BT_INFO("Restore configuration state");
    BT_INFO("net_transmit %d, relay %d, relay_retransmit %d",
        stored_cfg.cfg.net_transmit, stored_cfg.cfg.relay, stored_cfg.cfg.relay_retransmit);
    BT_INFO("net_transmit %d, gatt_proxy %d, frnd %d",
        stored_cfg.cfg.beacon, stored_cfg.cfg.gatt_proxy, stored_cfg.cfg.frnd);
    BT_INFO("default_ttl %d", stored_cfg.cfg.default_ttl);
...
int settings_core_load(void)
{
    ...
    //printf("load setting time %llu", time1 - time0);
...
static void schedule_store(int flag)
{
    ...
    BT_INFO("%s flag %d", __func__, flag);
    ...
    if (bt_mesh_atomic_get(bt_mesh.flags) & NO_WAIT_PENDING_BITS) {
        ...
        timeout = (CONFIG_BLE_MESH_RPL_STORE_TIMEOUT);
    } else {
        //timeout = K_SECONDS(CONFIG_BLE_MESH_STORE_TIMEOUT);
        timeout = (CONFIG_BLE_MESH_STORE_TIMEOUT);
    }

    remaining = k_delayed_work_remaining_get(&pending_store);
    if (remaining && remaining < timeout) {
        BT_INFO("Not rescheduling due to existing earlier deadline");
        return;
    }
...
static void store_rpl(struct bt_mesh_rpl *entry)
{
    ...
    BT_INFO("src 0x%04x seq 0x%06x old_iv %u", entry->src, entry->seq, entry->old_iv);
...
static void clear_app_key(u16_t app_idx)
{
    ...
    BT_INFO("clear_app_key AppKeyIndex 0x%03x", app_idx);
...
void bt_mesh_store_app_key(struct bt_mesh_app_key *key)
{
    ...
    BT_INFO("AppKeyIndex 0x%03x", key->app_idx);
