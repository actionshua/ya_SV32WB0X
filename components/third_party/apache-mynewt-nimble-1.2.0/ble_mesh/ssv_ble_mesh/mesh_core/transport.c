...
static void send_ssv_sync(struct bt_mesh_net_tx *tx, struct net_buf_simple *sdu,
                          const struct bt_mesh_send_cb *cb, void *cb_data)

{
    struct net_buf *buf;

    buf = bt_mesh_adv_create(BLE_MESH_SSV_SYNCED_ACK, tx->xmit, BUF_TIMEOUT);
    if (!buf) {
        BT_ERR("%s, Out of network buffers", __func__);
        return;
    }

    net_buf_add_le16(buf, 0x039b);//ad typr d0xff vendor
    net_buf_add_le16(buf, bt_mesh_primary_addr());//tx node address
    net_buf_add_le32(buf, bt_mesh.seq);
    net_buf_add_le32(buf, g_st_acked_bitmap.low);
    net_buf_add_le32(buf, g_st_acked_bitmap.high);
...
static int send_unseg_ssv(u32 opcode, struct bt_mesh_net_tx *tx, struct net_buf_simple *sdu,
                      const struct bt_mesh_send_cb *cb, void *cb_data)
{
    struct net_buf *buf;
    int ret= 0;
...
send:
    if (opcode == SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS) {
        //rx side
        //printf("ssv send_seg ack opcode %x\n", opcode);
        buf->ack = 1;
        buf->remote_addr = tx->ctx->addr;
        clean_ssv_ack_info(tx->ctx->addr);
        //g_synced_ack_from = 0;
        //g_synced_ack_seq = 0;
        //g_synced_ack_bitmap = 0;
    } else if (opcode == SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
        //tx side onff message
        //printf("ssv need_synced_adv opcode %x\n", opcode);
        buf->need_synced_adv = 1;
    }

    ret = bt_mesh_net_send(tx, buf, cb, cb_data);
    if (opcode == SSV_BLE_MESH_MODEL_OP_GEN_ONOFF_SET) {
        tx->xmit = SSV_SYNCED_ACK_XMIT_COUNT;
        //printf("send us sync adv\n");
        send_ssv_sync(tx, sdu, cb, cb_data);
    }
...
static int send_seg_ssv(u32 opcode, struct bt_mesh_net_tx *net_tx, struct net_buf_simple *sdu,
                    const struct bt_mesh_send_cb *cb, void *cb_data)
{
    u8_t seg_hdr, seg_o;
    u16_t seq_zero;
    ...
    for (seg_o = 0U; sdu->len; seg_o++) {
        ...
        if ((seg->len + 2 + 4) > BLE_HS_ADV_MAX_SZ) {
            printf("send_seg len err %d\n", seg->len);
            ASSERT(0);//length dbg
            return -100;
        }

        if (opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST) {
            seg->segment = 1;
            seg->segment_seq = seg_o;
            seg->remote_addr = net_tx->ctx->addr;
            seg->need_synced_adv = 1;
        } else if (opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST_STATUS) {
            seg->segment = 1;
            seg->segment_seq = seg_o;
            seg->remote_addr = net_tx->ctx->addr;
            seg->ack = 1;
            net_tx->xmit = SSV_SYNCED_ACK_XMIT_COUNT;
            clean_ssv_ack_info(net_tx->ctx->addr);
        }
    ...
    if (opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST) {
        net_tx->xmit = SSV_SYNCED_ACK_XMIT_COUNT;
        send_ssv_sync(net_tx, sdu, cb, cb_data);
    }
                    ...
                    const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    for (seg_o = 0U; sdu->len; seg_o++) {
        ...
        printf("send_seg %x %x\n",
            net_tx->ctx->model->op->opcode, SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_STATUS);
        if (net_tx->ctx->model->op->opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_STATUS) {
            seg->segment = 1;
            seg->segment_seq = seg_o;
            seg->remote_addr = net_tx->ctx->addr;
            seg->need_synced_adv = 1;
            printf("sseg_o %d\n", seg->segment_seq);
        } else if (net_tx->ctx->model->op->opcode == SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_SET) {
            seg->segment = 1;
            seg->segment_seq = seg_o;
            seg->remote_addr = net_tx->ctx->addr;
            seg->ack = 1;
                       ...
                       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (tx->ctx->send_rel) {
        err = send_seg_ssv(opcode, tx, msg, cb, cb_data);
        //vendor msg
        //printf("trans_send opcode %x %x\n",opcode, SSV_BLE_MESH_VND_MODEL_OP_TEST_PERF_TEST);
    } else {
        err = send_unseg_ssv(opcode, tx, msg, cb, cb_data);
    }
                    ...
                    u8_t aszmic, struct net_buf_simple *buf)
{
    ...
    BT_DBG("sdu_recv len %u: %s", buf->len, bt_hex(buf->data, buf->len));
                     ...
                     struct net_buf_simple *buf, u64_t *seq_auth)
{
    ...
    if (!tx) {
        BT_DBG("No matching TX context for ack");/*test item 10, show too many in node2~node50*/
        return -EINVAL;
    }
