...
static char device_name[DEVICE_NAME_SIZE] = "SSV-BLE-MESH";
...
static int node_id_adv(struct bt_mesh_subnet *sub)
{
    ...
    if (err) {
        if (err == BLE_HS_ENOMEM)
            BT_DBG("Failed to advertise using Node ID (err %d) BLE_HS_ENOMEM", err);
        else
...
static int net_id_adv(struct bt_mesh_subnet *sub)
{
    ...
    if (err) {
        BT_DBG("Failed to advertise using Network ID (err %d)", err);
        return err;
    }
