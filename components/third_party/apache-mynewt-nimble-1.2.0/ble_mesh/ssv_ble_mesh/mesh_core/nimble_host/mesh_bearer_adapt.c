...
{
    bt_mesh_atomic_clear_bit(bt_mesh_dev.flags, BLE_MESH_DEV_HAS_PUB_KEY);
    return 0;
}

bool bt_mesh_check_public_key(const u8_t key[64])
{
    struct ssv_mbedtls_ecp_point pt;
    ssv_mbedtls_ecp_group grp = {0};
    bool rc = false;
    ...
    ssv_mbedtls_ecp_point_init(&pt);
    ssv_mbedtls_ecp_group_init(&grp);

    /* Below 3 steps are to validate public key on curve secp256r1 */
    if (ssv_mbedtls_ecp_group_load(&grp, SSV_MBEDTLS_ECP_DP_SECP256R1) != 0) {
        BT_ERR("%s, mbedtls_ecp_group_load fail", __func__);
        goto exit;
    }

    if (ssv_mbedtls_ecp_point_read_binary(&grp, &pt, pub, 65) != 0) {
        BT_ERR("%s, mbedtls_ecp_point_read_binary fail", __func__);
        goto exit;
    }

    if (ssv_mbedtls_ecp_check_pubkey(&grp, &pt) != 0) {
        BT_ERR("%s, mbedtls_ecp_check_pubkey fail", __func__);
        goto exit;
    }
