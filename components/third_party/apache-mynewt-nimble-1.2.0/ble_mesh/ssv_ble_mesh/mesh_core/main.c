...
extern void ble_att_svr_hide_range(uint16_t start_handle, uint16_t end_handle);
                 ...
                 const struct bt_mesh_comp *comp)
{
    ...
    if (IS_ENABLED(CONFIG_BLE_MESH_SETTINGS)) {
        ...
        if(bt_mesh_is_provisioned())
        {
            printf("\33[32mmesh is provisioned (hide mesh provision handler)\33[0m\r\n");
            ble_att_svr_hide_range(16,21);
            //bt_mesh_proxy_prov_disable(0);
        }
    }
#endif
