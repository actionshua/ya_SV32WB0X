...
int bt_mesh_net_beacon_update(struct bt_mesh_subnet *sub)
{
    ...
    if (sub->kr_flag) {
        BT_INFO("NetIndex %u Using new key", sub->net_idx);
        keys = &sub->keys[1];
    } else {
        BT_INFO("NetIndex %u Using current key", sub->net_idx);
        keys = &sub->keys[0];
    }

    BT_INFO("flags 0x%02x, IVI 0x%08x", flags, bt_mesh.iv_index);
...
void bt_mesh_rpl_show(void)
{
    ...
    BT_ERR("my seq %d", bt_mesh.seq);
    for (i = 0; i < ARRAY_SIZE(bt_mesh.rpl); i++) {
        struct bt_mesh_rpl *rpl = &bt_mesh.rpl[i];

        if (rpl->src) {
            BT_ERR("src %d, seq %d", rpl->src, rpl->seq);
        }
    }
}
...
bool bt_mesh_net_iv_update(u32_t iv_index, bool iv_update)
{
    ...
    if (bt_mesh_atomic_test_bit(bt_mesh.flags, BLE_MESH_IVU_IN_PROGRESS)) {
        ...
        if (iv_update) {
            /* Nothing to do */
            BT_INFO("Already in IV Update in Progress state");
            return false;
        }
    } else {
        ...
        if (iv_index == bt_mesh.iv_index) {
            BT_INFO("Same IV Index in normal mode");
            return false;
        }
        ...
        if (!iv_update) {
            /* Nothing to do */
            BT_INFO("Already in Normal state");
            return false;
        }
    }
...
do_update:
    ...
    if (iv_update) {
        bt_mesh.iv_index = iv_index;
        BT_WARN("IV Update state entered. New index 0x%08x",
               bt_mesh.iv_index);

        bt_mesh_rpl_reset();
    } else {
        BT_WARN("Normal mode entered");
        bt_mesh.seq = 0U;
    }
                       ...
                       bool proxy)
{
    ...
    if ((buf->len + 2 + 4) > BLE_HS_ADV_MAX_SZ) {/*4 is MIC*/
        printf("bt_mesh_net_encode1 %d\n", buf->len);
        ASSERT(0);//length dbg
        return -100;
    }
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if ((buf->len + 2 + 4) > BLE_HS_ADV_MAX_SZ) {//4  /* 4 is MIC*/
        printf("bt_mesh_net_send 1 %d\n", buf->len);
        ASSERT(0);//length dbg
        err = -100;
        goto done;
    }
    ...
    if ((buf->len + 2) > BLE_HS_ADV_MAX_SZ) {
        printf("bt_mesh_net_encode ed %d\n", buf->len);
        ASSERT(0);//length dbg
        err = -100;
        goto done;
    }
...
{
    BT_INFO("NID 0x%02x net_idx 0x%04x", NID(data), sub->net_idx);
    BT_INFO("IVI %u net->iv_index 0x%08x", IVI(data), bt_mesh.iv_index);
                              ...
                              struct bt_mesh_net_rx *rx)
{
    ...
    /*count = bt_mesh_get_stored_adv_count();
    if (count > CONFIG_BLE_MESH_RELAY_DONT_SEND_CRITERIA) {
        BT_INFO("adv queue %u don't relay\n", count);
        return;
    }*/
    #if defined(CONFIG_BLE_MESH_RELAY_ADV_BUF)
    if (bt_mesh_get_stored_relay_count() >= (CONFIG_BLE_MESH_RELAY_ADV_BUF_COUNT - 1))
        return;
    #endif
                       ...
                       struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
    ...
    if(buf->len > BLE_HS_ADV_MAX_SZ) {
        BT_ERR("Drop Decryption Payload len %u", buf->len);
        return -EBADMSG;
    }
...
extern u8_t g_relay_enable;

void bt_mesh_net_recv(struct net_buf_simple *data, s8_t rssi,
                      enum bt_mesh_net_if net_if)
{
    NET_BUF_SIMPLE_DEFINE(buf, BLE_MESH_SSV_ADV_DATA_SIZE);
    struct bt_mesh_net_rx rx = { .rssi = rssi };
    struct net_buf_simple_state state;

    BT_DBG("rssi %d net_if %u", rssi, net_if);

    if (data->len > BLE_MESH_ADV_DATA_SIZE) {
        BT_WARN("Dropping too long mesh packet (len %u)", data->len);
        return;
    }
...
#if CONFIG_BLE_MESH_NODE
    if (bt_mesh_is_provisioned()) {
        if (!g_relay_enable && (net_if == BLE_MESH_NET_IF_ADV))
            return;
