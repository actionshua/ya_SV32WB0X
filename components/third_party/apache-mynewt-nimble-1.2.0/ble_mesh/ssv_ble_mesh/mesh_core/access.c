        ...
        *model = &models[i];
        ...
        if (BLE_MESH_ADDR_IS_GROUP(dst) ||
            ...
            if (!bt_mesh_model_find_group(*model, dst)) {
                BT_DBG("%s, OpCode 0x%08x dst addr is group or virtual and not in this model",
                    __func__, opcode);
                continue;
            }
        }
        ...
        if (!model_has_key(*model, app_idx)) {
            BT_DBG("%s, OpCode 0x%08x has no this app idx",
                    __func__, opcode);
            continue;
        }
        ...
        for (op = (*model)->op; op->func; op++) {
            if (op->opcode == opcode) {
                BT_DBG("%s, OpCode 0x%08x find op", __func__);
                return op;
            }
        }
    }
...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
    ...
    for (i = 0; i < dev_comp->elem_count; i++) {
        ...
        BT_DBG("%s, OpCode 0x%08x element %d", __func__, opcode, i);
...
static int model_send_ssv(u32 opcode, struct bt_mesh_model *model,
                      ...
                      const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    return bt_mesh_trans_send_ssv(opcode, tx, msg, cb, cb_data);
}
                       ...
                       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    return model_send_ssv(opcode, model, &tx, false, msg, cb, cb_data);
}
...
int bt_mesh_model_publish(struct bt_mesh_model *model)
{
    ...
    if (pub->addr == BLE_MESH_ADDR_UNASSIGNED) {
        BT_DBG("%s, Unassigned model publish address", __func__);
        return -EADDRNOTAVAIL;
    }
