                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (status == STATUS_SUCCESS) {
        bt_mesh_cfg_server_state_change_t change;
...
#if CONFIG_BLE_MESH_BLOCK_TRANSMIT_SET
        BT_ERR("net_transmit_set do not set 0x%x", buf->data[0]);
#else
...
#if CONFIG_BLE_MESH_BLOCK_TRANSMIT_SET
            BT_ERR("relay_set do not set 0x%x", buf->data[0]);
#else
