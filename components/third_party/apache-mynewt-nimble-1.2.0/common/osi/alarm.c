...
ATTRIBUTE_SECTION_MESH_BLE static struct alarm_t alarm_cbs[ALARM_CBS_NUM];/*run here now*/
#else
...
osi_alarm_t *osi_alarm_new(const char *alarm_name, osi_alarm_callback_t callback, void *data, period_ms_t timer_expire)
{
    ...
    if (!timer_id) {
        OSI_TRACE_ERROR("%s %s alarm_cbs exhausted\n", __func__, alarm_name);
        timer_id = NULL;
        goto end;
    }

    ssv_timer_create_args_t tca;
    tca.callback = (ssv_timer_cb_t)alarm_cb_handler;
    tca.arg = timer_id;
    tca.dispatch_method = SSV_TIMER_TASK;
    tca.name = alarm_name;
    ...
    ssv_err_t stat = ssv_timer_create(&tca, &timer_id->alarm_hdl);
    if (stat != SSV_BM_OK) {
...
static osi_alarm_err_t alarm_free(osi_alarm_t *alarm)
{
    ...
    ssv_timer_stop(alarm->alarm_hdl);
    ssv_err_t stat = ssv_timer_delete(alarm->alarm_hdl);
    if (stat != SSV_BM_OK) {
...
static osi_alarm_err_t alarm_set(osi_alarm_t *alarm, period_ms_t timeout, bool is_periodic)
{
    ...
    ssv_err_t stat;
    if (is_periodic) {
        stat = ssv_timer_start_periodic(alarm->alarm_hdl, (uint64_t)timeout_us);
    } else {
        stat = ssv_timer_start_once(alarm->alarm_hdl, (uint64_t)timeout_us);
    }
    if (stat != SSV_BM_OK) {
    ...
    alarm->deadline_us = is_periodic ? 0 : (timeout_us + ssv_timer_get_time());//--ssv
    //alarm->deadline_us = is_periodic ? 0 : (timeout_us);//++ssv
...
osi_alarm_err_t osi_alarm_cancel(osi_alarm_t *alarm)
{
    ...
    ssv_err_t stat = ssv_timer_stop(alarm->alarm_hdl);
    if (stat != SSV_BM_OK) {
...
period_ms_t osi_alarm_get_remaining_ms(const osi_alarm_t *alarm)
{
    ...
    osi_mutex_lock(&alarm_mutex, OSI_MUTEX_MAX_TIMEOUT);
    dt_us = alarm->deadline_us - ssv_timer_get_time();
    osi_mutex_unlock(&alarm_mutex);
...
{
    return (uint32_t)(ssv_timer_get_time() / 1000);
}
