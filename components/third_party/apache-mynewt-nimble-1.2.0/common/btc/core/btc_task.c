...
#if (GATTS_INCLUDED || GATTC_INCLUDED)
#include "btc/btc_gap_ble.h"
#include "btc/btc_gatt_common.h"
#endif
...
#define BTC_TASK_PRIO                   OS_TASK_MIDDLE//(OS_TASK_PRIOBASE + 4)

osi_thread_t *btc_thread = NULL;

static const btc_func_t profile_tab[BTC_PID_NUM] = {
//#ifdef CONFIG_BT_BLUEDROID_ENABLED
    //[BTC_PID_MAIN_INIT]   = {btc_main_call_handler,       NULL                    },//ignore
#if BLE_INCLUDED
...
#if 0//(GATTS_INCLUDED == TRUE)
    [BTC_PID_BLUFI]       = {btc_blufi_call_handler,      btc_blufi_cb_handler    },
#endif  ///GATTS_INCLUDED == TRUE
    //[BTC_PID_DM_SEC]      = {NULL,                        btc_dm_sec_cb_handler   },//todo
//#endif//CONFIG_BT_BLUEDROID_ENABLED end
...
#if GATTS_INCLUDED == TRUE && GATT_DYNAMIC_MEMORY == TRUE
    if ((btc_creat_tab_env_ptr = (ssv_btc_creat_tab_t *)osi_malloc(sizeof(ssv_btc_creat_tab_t))) == NULL) {
        goto error_exit;
    }
    memset((void *)btc_creat_tab_env_ptr, 0, sizeof(ssv_btc_creat_tab_t));
...
{
    if (btc_thread) {
        BTC_TRACE_WARNING("%s btc_thread already create\n", __func__);
        return BT_STATUS_SUCCESS;
    }
