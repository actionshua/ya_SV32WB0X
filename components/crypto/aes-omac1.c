		     ...
		     const u8 *addr[], const size_t *len, u8 *mac)
{
    ...
    ssv_mbedtls_aes_setkey_enc(&ctx, key, key_len * 8);
	...
	while (left >= AES_BLOCK_SIZE) {
		...
		if (left > AES_BLOCK_SIZE)
			ssv_mbedtls_internal_aes_encrypt(&ctx, cbc, cbc);
		left -= AES_BLOCK_SIZE;
	}

	memset(pad, 0, AES_BLOCK_SIZE);
	ssv_mbedtls_internal_aes_encrypt(&ctx, pad, pad);
	gf_mulx(pad);
	...
	for (i = 0; i < AES_BLOCK_SIZE; i++)
		pad[i] ^= cbc[i];
	ssv_mbedtls_internal_aes_encrypt(&ctx, pad, mac);
