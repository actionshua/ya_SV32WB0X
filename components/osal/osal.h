...
#else
    #define OS_TASK_WIFI_SYSTEM     (4)
	#define OS_TASK_PRIOBASE        (0)
	#define OS_TASK_HIGH_PRIO       (6)
	#define OS_TASK_MIDDLE_PRIO     (3)
...
/**
 * create task with stack in PSRAM.
 *
 * @param   task                    	task function
 * @param   name                    	task name
 * @param   stackSize                   task stack size in WORDS (ex: task stack size 512 bytes = 128 WORDS)
 ...
 * @retval  1                         	task create success
 */
u8 OS_TaskCreateFromPsram( OsTask task, const char *name, u16 stackSize, void *param, u32 pri, OsTaskHandle *taskHandle );
/**
...
/**
 * get softtimer remain expiry time.
 *
 * @param   timer                    	timer handle
 *
 * @retval  the timer expiry time (ms).
 */
u32 OS_TimerGetExpiryTime( OsTimer timer );
...
OS_STATUS OS_MsgQEnqueueFront( OsMsgQ MsgQ, OsMsgQEntry *MsgItem );
/**
 ...
 * init psram heap
 *
 * @retval  0							psram heap init failed
 * @retval  1                  			psram heap init success
 */
