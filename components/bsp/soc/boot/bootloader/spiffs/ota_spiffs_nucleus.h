 ...
 *
 */
#ifndef OTA_SPIFFS_NUCLEUS_H_
#define OTA_SPIFFS_NUCLEUS_H_
...
#else
#define _spiffs_rd(fs, op, fh, addr, len, dst) \
    otaspiffs_phys_rd((fs), (addr), (len), (dst))
#define _spiffs_wr(fs, op, fh, addr, len, src) \
    otaspiffs_phys_wr((fs), (addr), (len), (src))
#endif
...
s32_t otaspiffs_phys_count_free_blocks(
    spiffs *fs);

s32_t otaspiffs_obj_lu_find_entry_visitor(
    spiffs *fs,
...
s32_t otaspiffs_erase_block(
    spiffs *fs,
    spiffs_block_ix bix);

#if SPIFFS_USE_MAGIC && SPIFFS_USE_MAGIC_LENGTH
s32_t otaspiffs_probe(
    spiffs_config *cfg);
#endif // SPIFFS_USE_MAGIC && SPIFFS_USE_MAGIC_LENGTH

// ---------------

s32_t otaspiffs_obj_lu_scan(
    spiffs *fs);

s32_t otaspiffs_obj_lu_find_free_obj_id(
    spiffs *fs,
    spiffs_obj_id *obj_id,
    const u8_t *conflicting_name);

s32_t otaspiffs_obj_lu_find_free(
    spiffs *fs,
    spiffs_block_ix starting_block,
    int starting_lu_entry,
    spiffs_block_ix *block_ix,
...
s32_t otaspiffs_obj_lu_find_id(
    spiffs *fs,
    spiffs_block_ix starting_block,
    int starting_lu_entry,
    spiffs_obj_id obj_id,
...
s32_t otaspiffs_obj_lu_find_id_and_span(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_span_ix spix,
...
s32_t otaspiffs_obj_lu_find_id_and_span_by_phdr(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_span_ix spix,
    spiffs_page_ix exclusion_pix,
...
s32_t otaspiffs_page_allocate_data(
    spiffs *fs,
    spiffs_obj_id obj_id,
...
s32_t otaspiffs_page_delete(
    spiffs *fs,
    spiffs_page_ix pix);

// ---------------

s32_t otaspiffs_object_create(
    spiffs *fs,
    spiffs_obj_id obj_id,
...
s32_t otaspiffs_object_update_index_hdr(
    spiffs *fs,
    spiffs_fd *fd,
    spiffs_obj_id obj_id,
    spiffs_page_ix objix_hdr_pix,
...
void otaspiffs_cb_object_event(
    spiffs *fs,
...
s32_t otaspiffs_object_open_by_id(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_fd *f,
    spiffs_flags flags,
...
s32_t otaspiffs_object_open_by_page(
    spiffs *fs,
    spiffs_page_ix pix,
    spiffs_fd *f,
    spiffs_flags flags,
...
s32_t otaspiffs_object_append(
    spiffs_fd *fd,
    u32_t offset,
    u8_t *data,
    u32_t len);

s32_t otaspiffs_object_modify(
    spiffs_fd *fd,
    u32_t offset,
    u8_t *data,
    u32_t len);

s32_t otaspiffs_object_read(
    spiffs_fd *fd,
    u32_t offset,
    u32_t len,
    u8_t *dst);

s32_t otaspiffs_object_truncate(
    spiffs_fd *fd,
    u32_t new_len,
    u8_t remove_object);

s32_t otaspiffs_object_find_object_index_header_by_name(
    spiffs *fs,
    const u8_t name[SPIFFS_OBJ_NAME_LEN],
    spiffs_page_ix *pix);
...
s32_t otaspiffs_gc_check(
    spiffs *fs,
    u32_t len);

s32_t otaspiffs_gc_erase_page_stats(
    spiffs *fs,
    spiffs_block_ix bix);

s32_t otaspiffs_gc_find_candidate(
    spiffs *fs,
    spiffs_block_ix **block_candidate,
    int *candidate_count,
    char fs_crammed);

s32_t otaspiffs_gc_clean(
    spiffs *fs,
    spiffs_block_ix bix);

s32_t otaspiffs_gc_quick(
    spiffs *fs, u16_t max_free_pages);

// ---------------

s32_t otaspiffs_fd_find_new(
    spiffs *fs,
    spiffs_fd **fd,
    const char *name);

s32_t otaspiffs_fd_return(
    spiffs *fs,
    spiffs_file f);

s32_t otaspiffs_fd_get(
    spiffs *fs,
    spiffs_file f,
    spiffs_fd **fd);
...
s32_t otaspiffs_lookup_consistency_check(
    spiffs *fs,
    u8_t check_all_objects);

s32_t otaspiffs_page_consistency_check(
    spiffs *fs);

s32_t otaspiffs_object_index_consistency_check(
    spiffs *fs);
