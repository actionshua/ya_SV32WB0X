#ifndef _SOC_ASSERT_H_
#define _SOC_ASSERT_H_
...
{ \
    ...
    if (!(x)) \
    { \
        extern uint8_t __log_buf[]; \
        extern uint32_t __log_size; \
        snprintf((char *)__log_buf,(uint32_t)(&__log_size),"Assert!! file: %s, function: %s, line: %d\n\t" #x, __FILE__, \
        	__FUNCTION__, __LINE__); \
        printf("%s\n",(char *)__log_buf);\
        stop_and_halt(); \
    } \
