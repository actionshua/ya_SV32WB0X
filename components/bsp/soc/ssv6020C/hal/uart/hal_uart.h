...
void hal_uart_register_isr (UART_IE_E INT_ENABLE, UART_ISR ISR);

void hal_uart_init (void);

void hal_uart_deinit (void);

int32_t hal_uart_set_format (int32_t BaudRate, UART_WORD_LEN_E DataBits, UART_STOP_BIT_E StopBits, UART_PARITY_E Parity);

void hal_uart_set_fifo (UART_INT_RXFIFO_TRGLVL_E INT_RXFIFO_TRGLVL, uint32_t INT_TTHR_L, uint32_t INT_TTHR_H);

void hal_uart_set_loop_back (void);

void hal_uart_unset_loop_back (void);

uint32_t hal_uart_direct_read_byte (void);

void hal_uart_direct_write_byte (uint32_t data);

int32_t hal_uart_write_fifo (uint8_t const *Buffer, int32_t NumberOfBytes, UART_BLOCKING_MODE_E BlockingMode);

int32_t hal_uart_read_fifo (uint8_t *Buffer, int32_t NumberOfBytes, UART_BLOCKING_MODE_E BlockingMode);

void hal_uart_sw_rst (void);

uint32_t hal_uart_get_line_status (void);
