...
typedef enum _HSUART_INTERRUPT_ENABLE_E {
    ...
    HSUART_ENABCTXTHR               = 0x4,  ///< Tx FIFO threshold
    HSUART_ENABDMARXEND             = 0x6,  ///< Rx DMA done
    HSUART_ENABDMATXEND             = 0x7,  ///< Tx DMA done
...
uint32_t hal_hsuart_direct_read_byte (void);


void hal_hsuart_register_isr_ex (HSUART_HW_ID_E hw, HSUART_INTERRUPT_ENABLE_E INT_ENABLE, HSUART_ISR ISR);

void hal_hsuart_init_ex (HSUART_HW_ID_E hw);

void hal_hsuart_deinit_ex (HSUART_HW_ID_E hw);

int32_t hal_hsuart_set_format_ex (HSUART_HW_ID_E hw, int32_t BaudRate, HSUART_WLS_E DataBits, HSUART_STB_E StopBits, HSUART_PARITY_E Parity);

void hal_hsuart_set_fifo_ex (HSUART_HW_ID_E hw, HSUART_INT_RX_FIFO_TRIG_LV_E INT_RX_TRIG_LV);

void hal_hsuart_enable_dma_ring_buffer_ex (HSUART_HW_ID_E hw, uint8_t *DMA_TX_STR_ADDR, uint8_t *DMA_TX_END_ADDR, uint8_t *DMA_RX_STR_ADDR, uint8_t *DMA_RX_END_ADDR);

void hal_hsuart_disable_dma_ring_buffer_ex (HSUART_HW_ID_E hw);

void hal_hsuart_set_hardware_flow_control_ex (HSUART_HW_ID_E hw, uint32_t RTS_AUTO_TH_L, uint32_t RTS_AUTO_TH_H);

void hal_hsuart_set_loop_back_ex (HSUART_HW_ID_E hw);

int32_t hal_hsuart_write_fifo_ex (HSUART_HW_ID_E hw, uint8_t const *Buffer, int32_t NumberOfBytes, HSUART_BLOCKING_MODE_E BlockingMode);

int32_t hal_hsuart_read_fifo_ex (HSUART_HW_ID_E hw, uint8_t *Buffer, int32_t NumberOfBytes, HSUART_BLOCKING_MODE_E BlockingMode);

int32_t hal_hsuart_get_dma_rx_length_ex (HSUART_HW_ID_E hw);

int32_t hal_hsuart_write_dma_ring_buffer_ex (HSUART_HW_ID_E hw, uint8_t Character);

int32_t hal_hsuart_read_dma_ring_buffer_ex (HSUART_HW_ID_E hw);

int32_t hal_hsuart_write_dma_ex (HSUART_HW_ID_E hw, uint8_t *buf, int32_t buf_size);

int32_t hal_hsuart_read_dma_ex (HSUART_HW_ID_E hw, uint8_t *buf, int32_t buf_size);

void hal_hsuart_sw_rst_ex (HSUART_HW_ID_E hw);

uint32_t hal_hsuart_get_line_status_ex (HSUART_HW_ID_E hw);

uint32_t hal_hsuart_get_modem_status_ex (HSUART_HW_ID_E hw);

uint32_t hal_hsuart_direct_read_byte_ex (HSUART_HW_ID_E hw);
