...
#if (STRIP_LIGHT == 0)
extern st_ya_stripLigths_workData ya_stripLigths_workData;
#endif
...
void ya_stripLightsDisplay_CWtoRGB(unsigned char*r, unsigned char*g, unsigned char*b, unsigned char c, unsigned char w)
{
	int index,len;
	#if (FLOAT_SUPPORT == 1)
	float C_Adj, W_Adj;
 ...
 void ya_stripLightsDisplay_getRGB(ya_display_stripsLight_t *pcolorInfo)
{
	...
	for(i=0;i<len;i++)
	{
		if(pcolorInfo->colorInfo[i].type == COLORTYPE_COLOR)
		{
			...
			c=pcolorInfo->colorInfo[i].white_temp;
			w=100- pcolorInfo->colorInfo[i].white_temp;
		#if (FLOAT_SUPPORT == 1)
			c=c*(pcolorInfo->colorInfo[i].white_bright)*2.55/100;
			w=w*(pcolorInfo->colorInfo[i].white_bright)*2.55/100;  	
		#else
			c=c*(pcolorInfo->colorInfo[i].white_bright)*255/10000;
			w=w*(pcolorInfo->colorInfo[i].white_bright)*255/10000;  				
		#endif
...
#else
			if(ya_stripLigths_workData.workMode ==  WORKMODE_WHITE || ya_stripLigths_workData.workMode == WORKMODE_SCENE)
			{
				if(pcolorInfo->colorInfo[i].white_bright < 10)	
					pcolorInfo->colorInfo[i].white_bright = 10;
			}
	#if (CW_PWM == 1)
			c=pcolorInfo->colorInfo[i].white_temp;
			w=100- pcolorInfo->colorInfo[i].white_temp;
		...
		#endif
	#else
			c=(pcolorInfo->colorInfo[i].white_temp)*255/100;
			w=(pcolorInfo->colorInfo[i].white_bright)*255/100;
	#endif
			pcolorInfo->colorInfo[i].color_H = 0;
...
void ya_stripLightsDisplay_start(ya_display_stripsLight_t *pcolorInfo)
{
	...
	if(pcolorInfo->changeType == STATIC_TYPE)
	{
		pCtrlInfoGroup->timerIntervalWaiting = 1;
		pCtrlInfoGroup->timerIntervalHold = 1;
		pCtrlInfoGroup->timerIntervalChangeDown = 1;
		//ÀûÓÃÁÁ¶È¿ØÖÆÑÕÉ«±ä»¯µÄËÙ¶ÈÑÕÉ«ÁÁ¶ÈµÍ±ä»¯¿ì£¬ÁÁ¶È¸ß±ä»¯Âý
		if(pcolorInfo->colorInfo[0].type == COLORTYPE_COLOR)		
		{			
			pCtrlInfoGroup->changeSpeed = pcolorInfo->colorInfo[0].color_B;	
			//limit bright
			if(pcolorInfo->colorInfo[0].color_B < 10)
				pcolorInfo->colorInfo[0].color_B = 10;
			//limit speed
			if(pCtrlInfoGroup->changeSpeed < 50)
				pCtrlInfoGroup->changeSpeed = 50;
			pCtrlInfoGroup->timerIntervalChangeUp =  pCtrlInfoGroup->changeSpeed ;
			#if (FLOAT_SUPPORT == 1)
			pCtrlInfoGroup->changeSpeed = (pCtrlInfoGroup->changeSpeed/100.0f)*(YA_STRIPLIGHTSDISPLAY_COLORCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp);
			#else		
			pCtrlInfoGroup->changeSpeed = (pCtrlInfoGroup->changeSpeed/100)*(YA_STRIPLIGHTSDISPLAY_COLORCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp);		
			#endif
		}
		else		
		{			
			pCtrlInfoGroup->changeSpeed = pcolorInfo->colorInfo[0].white_bright;	
			//limit bright
			if(pcolorInfo->colorInfo[0].white_bright < 10)
				pcolorInfo->colorInfo[0].white_bright = 10;
			//limit speed
			if(pCtrlInfoGroup->changeSpeed < 50)
				pCtrlInfoGroup->changeSpeed = 50;
			pCtrlInfoGroup->timerIntervalChangeUp =  pCtrlInfoGroup->changeSpeed ;
			...
			pCtrlInfoGroup->changeSpeed = (pCtrlInfoGroup->changeSpeed /100.0f)*(YA_STRIPLIGHTSDISPLAY_COLORCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp);
			#else		
			pCtrlInfoGroup->changeSpeed = pCtrlInfoGroup->changeSpeed * (YA_STRIPLIGHTSDISPLAY_COLORCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp) /100;	
			#endif
		}
	...
	{	
		...
		pCtrlInfoGroup->timerIntervalChangeDown = 1;
		#if (FLOAT_SUPPORT == 1)
		pCtrlInfoGroup->changeSpeed = (pcolorInfo->changeSpeed/100.0f)*(YA_STRIPLIGHTSDISPLAY_PWMCHANGE_MAXTIME);
		#else
		pCtrlInfoGroup->changeSpeed = (pcolorInfo->changeSpeed/100)*(YA_STRIPLIGHTSDISPLAY_PWMCHANGE_MAXTIME);
		#endif
	}
	...
	{	
		...
		pCtrlInfoGroup->timerIntervalChangeDown = 1;
		#if (FLOAT_SUPPORT == 1)
		pCtrlInfoGroup->changeSpeed = (pcolorInfo->changeSpeed/100.0f)*(YA_STRIPLIGHTSDISPLAY_PWMCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp);
		#else
		pCtrlInfoGroup->changeSpeed = (pcolorInfo->changeSpeed/100)*(YA_STRIPLIGHTSDISPLAY_PWMCHANGE_MAXTIME/pCtrlInfoGroup->timerIntervalChangeUp);
		#endif
	}
	...
	for(i=0;i<pCtrlInfoGroup->pwmGroupLen;i++)
	{
		...
		pChannelInfo->pwmDutyNew= pcolorInfo->colorInfo[i].color_H*100;// ·Å´ó100

		pChannelInfo = &pCrlInfo->pwmChannelInfo[PWM_G];
		pChannelInfo->pwmDutyNew= pcolorInfo->colorInfo[i].color_S*100;// ·Å´ó100

		pChannelInfo = &pCrlInfo->pwmChannelInfo[PWM_B];
		pChannelInfo->pwmDutyNew= pcolorInfo->colorInfo[i].color_B*100;// ·Å´ó100
		
		pChannelInfo = &pCrlInfo->pwmChannelInfo[PWM_COOL];
		pChannelInfo->pwmDutyNew= pcolorInfo->colorInfo[i].white_temp*100;// ·Å´ó100
		
		pChannelInfo = &pCrlInfo->pwmChannelInfo[PWM_WARM];
		pChannelInfo->pwmDutyNew= pcolorInfo->colorInfo[i].white_bright*100;// ·Å´ó100
	}
...
int ya_stripLightsDisplay_loop(void)
{
	...
	if(pCtrlInfoGroup->displayState == PWM_DISPLAY_UP)
	{
		if(++pCtrlInfoGroup->timerCounter <= pCtrlInfoGroup->timerIntervalChangeUp)
		{
			...
			{
				pCtrlInfoGroup->timerCounter = 0;
				if(pCtrlInfoGroup->changeType == STATIC_TYPE)
				{
					pCtrlInfoGroup->displayState= PWM_DISPLAY_IDLE;
					pCtrlInfoGroup->pwmGroupIndex = 0;
