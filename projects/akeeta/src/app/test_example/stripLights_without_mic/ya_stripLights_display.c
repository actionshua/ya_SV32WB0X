...
void ya_stripLightsDisplay_getChangeStep(void)
{
	...
	for(i=0;i<5;i++)
	{
		...
		#if (PWM_CW_CTL_TYPE == 0)
		if(i == PWM_WARM)
		{
			//on
			if((pCtrlInfoGroup->pwmDutyCur[PWM_COOL] == 0)&&(pCrlInfo->pwmChannelInfo[PWM_COOL].pwmDutyNew > 0))
				pChannelInfo->pwmDutyStep = temp;
			//off
			else if((pCtrlInfoGroup->pwmDutyCur[PWM_COOL] > 0)&&(pCrlInfo->pwmChannelInfo[PWM_COOL].pwmDutyNew == 0))
			{
				pChannelInfo->pwmDutyNew = pCtrlInfoGroup->pwmDutyCur[i];
				pChannelInfo->pwmDutyStep = 0;
			}
		}
		#endif
