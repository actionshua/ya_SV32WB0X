...
/* write half-word */
/* could use outh write half-word to target memory */
int outh(unsigned int address, unsigned int data)
{
#ifdef SPI_BURN
...
int outb(unsigned int address, unsigned int data)
{
#ifdef SPI_BURN
  char send_data[10],recv_data[2];
  int retval __attribute__((unused));
...
/* read half-word */
/* could use inh read half-word from target memory */
int inh(unsigned int address)
{
#ifdef SPI_BURN
  int data;
...
void reset_dut()
{
    fprintf(pLogFile, "try to reset system\n");
    outw(0xC0000000, (1<<21));
