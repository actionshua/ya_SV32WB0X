...
void flash_32kblock_erase(unsigned int addr) {
	unsigned int s32cmd;
	flash_write_enable();
	
	addr = ENDIAN_CHANGE(addr);
	s32cmd = (ERASE_32KBLOCK_CMD | addr);
	 
	flash_write_cmd(s32cmd,ERASE_32KBLOCK_TLEN,ERASE_32KBLOCK_RLEN);
	 
	flash_wait_spi_busy();
	flash_wait_spi_status_register();
}

void flash_64kblock_erase(unsigned int addr) {
	unsigned int s32cmd;
	flash_write_enable();
	
	addr = ENDIAN_CHANGE(addr);
	s32cmd = (ERASE_64KBLOCK_CMD | addr);
	 
	flash_write_cmd(s32cmd,ERASE_64KBLOCK_TLEN,ERASE_64KBLOCK_RLEN);
...
void flash_page_program(unsigned int flash_addr, unsigned int flash_len, unsigned char* src_arr)
{
	...
	fastout(SRAM_ADDR_CMD_DATAOUT_ADD4, flash_len, (char*)src_arr);
	flash_write_enable();
